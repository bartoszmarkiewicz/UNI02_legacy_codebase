...
unsigned int pmppwm;
extern unsigned char DVNTtoPWM(unsigned char dvnt);
void main(void)
{
//------------------
//------------------
...
//------------------
	WritePWM2(0);
	if(M._PW_START||eeprom_read(_ADRKRES))
	{
        if_eeprom_write(_ADRKRES,0);        //zeruj flage wymuszonej kalibracji wentylatora po programowym restarcie oprogramowania
...
//----------------------------
	...
	//----------------
        ...
        /*if(GetBit2(RSDT1.inSTAT1,_KSET))  //wymuszenie resetu plytki sterujacej przyciskiem SET
        {
            while(1);
        }*/
	//----------------
...
void InitPFN(const unsigned char pos)
{
	...
	if(MGSD._CON)
	{
		BitSet2(&RSDT1.outSTAT0,_PGPRG,0);		//aktywacja procedury inicjalizacji danych
		BitSet2(&RSDT1.outSTAT0,_PGRES,0);
		while(!GetBit2(RSDT1.inSTAT0,_INITDTA)&&MGSD._CON)	//procedura jeszcze nieaktywna?
		{
			CLRWDT();
			RefLCDData();			//aktualizacja danych wysylanych do modulu LCD
		...
		BitSet2(&RSDT1.outSTAT0,_PGPRG,1);		//deaktywacja procedury inicjalizacji danych (w tym momencie nastepuje aktualizacja danej)
		BitSet2(&RSDT1.outSTAT0,_PGRES,0);
		while(GetBit2(RSDT1.inSTAT0,_INITDTA)&&MGSD._CON)	//procedura jeszcze aktywna?
		{
			CLRWDT();
			RefLCDData();			//aktualizacja danych wysylanych do modulu LCD
			ReceptDataLCD();		//aktualizacja danych pobieranych z modulu LCD
...
void KalibracjaWentylatora(void)
{
	...
	{        
		...
		if(_VNT) PomiarVNT();	//predkosc obrotowa wentylatora
        else ClrTachoDta();
		ReceptDataLCD();		//aktualizacja danych pobieranych z modulu LCD
		RefLCDData();			//aktualizacja danych wysylanych do modulu LCD        
	};
...
void RefLCDData(void)
{
	...
	BitSet2(&RSDT1.outSTAT3,_VNT_ON,_VNT);									//stan pracy wentylatora (0-postoj, 1-paraca)
	BitSet2(&RSDT1.outSTAT3,_ROZ,M._ROZRUCH);								//znacznik aktywnej procedury rozruchu
	BitSet2(&RSDT1.outSTAT3,_ErrIN,DtIN._err_IN);							//awaria czujnika IN
	BitSet2(&RSDT1.outSTAT3,_PMP_PWM,(DtKNF.rdpm!=0)?1:0);					//pompa PWM
	BitSet2(&RSDT1.outSTAT3,_TRB_ECO,((DtKNF.teco!=0)&&(DtKNF.robg!=0))?1:0);	//tryb pracy ECO
	...
	BitSet2(&RSDT1.outSTAT4,_PR_ODP,M._PROC_ODP);							//aktywacja blokady na odpowietrzenie ukladu
    ...
    BitSet2(&RSDT1.outSTAT4,_PGD_TP,(DtKNF.pgdr==2)?1:0);                   //rodzaj regulatora pogodowego (0-tradycyjny, 1-rozszerzony)
	...
	BitSet2(&RSDT1.outKSTAT0,_ZCFM,(_KNF)?0:1);								//stan zworki konfiguracyjnej
	BitSet2(&RSDT1.outKSTAT0,_KC1F,(DtKNF.tpkt==_UNICO)?1:0);				//znacznik rodzaju kotla (1=jednofunkcyjny)
...
void DecSTR(void)
{
    ...
    else //(w stanie spoczynku pompa pracuje w obiegu CW)
    {
        if(!M._ZMKCOCW)
        {
            ...
            if(!M._PRACA&&M._WYLACZ&&!M._PROC_ODP&&(DtKNF.tpkt==_MONOHB)&&(!DtCO._err_CO&&!DtCO._ptwp_CO)&&M._pob_CW) ToObiegCW();
            else
            if(!M._PRACA&&M._WYLACZ&&!M._PROC_ODP&&(DtKNF.tpkt==_UNICO)&&(!DtCO._err_CO&&!DtCO._ptwp_CO)&&((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA))&&DtCW._wpz_CW) ToObiegCW();
            else
            ...
            else	//kociol w spoczynku
            {
                switch(DtKNF.tpkt)		//dla danego rodzaju kotla
                {
                    ...
                    {
                        //nieaktywna pompa?
                        if(!M._PRACA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&(!_POMPA||M._RUSZ_48Z3)) ToObiegCW();
                        break;
                    }
                    case _UNICO:
                    {					
                        //b.niska temperatura CW lub nieaktywna pompa?
                        //if(!M._PRACA&&!M._PROC_ODP&&(RdPrt(S_ON_PMP_CW)||(!M._WYLACZ&&!M._RUSZ_48&&!_POMPA&&!M._PROC_ODP))) ToObiegCW();
                        ...
                        {
                            if(!M._WYLACZ&&!M._RUSZ_48&&!_POMPA&&!M._PROC_ODP) ToObiegCO();
                        }
                        break;
                    }
                    default:			//domyslny monotermiczny
                    {
                        //nieaktywna pompa?
                        if(!M._PRACA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!_POMPA) ToObiegCW();
                        break;
                    }
                }
            }
	...
	{	
		switch(DtKNF.tpkt)								//dla danego rodzaju kotla
		{
			case _MONOHB:
			{
				if(!_POMPA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M._RUSZ_48Z3&&!M._PRACA)
				{
			...
			case _UNICO:
			{
				if(!_POMPA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M._PRACA)
				{
					//temperatura CO/CW poza dozwolonym zakresem
	...
	if(!M._ROZRUCH&&_GRZEJ&&M._pob_CO&&!M._pob_SRW&&!M._AKTYWNE_KNF&&!M._pob_KCO&&!M._pob_KCW&&!M._pob_CW&&!M._pob_DzCW&&!M._pob_AL)
	{
    ...
    st=(!M._ROZRUCH&&M._LOW_PMP)?1:0;
    if(!st) lwp=0;
    else
    {
        if(!bst&&bst!=st)                   //zmiana stanu na 1?
        {
            lwp=(RSDT1.outPG>_PRGVNT)?1:0;        //spelniony warunek predkosci w trakcie zmiany stanu?
        }
    }
    bst=st;
	if(!M._PRACA||M._RUN_MNMOD||(!M._PRACA&&M._VWYBIEG)||(lwp))
	{
		M._MODULUJ=0;	//wylacz blok modulacji
		if(M._RUN_MNMOD)
		{
	...
	if(M._pob_KCW&&!M._ROZRUCH&&_GRZEJ) WritePWM(ProcToMod(DtKNF.dmxcw));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxcw),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCW);
	if(M._pob_KCO&&!M._ROZRUCH&&_GRZEJ) WritePWM(ProcToMod(DtKNF.dmxco));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxco),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCO);
#if _SRVTRB==1														//aktywny tryb serwisowy kompilacji?
	if(M._pob_SRW&&!M._ROZRUCH&&_GRZEJ) WritePWM(mypwm);
#else
	if(M._pob_SRW&&!M._ROZRUCH&&_GRZEJ)
	{
		if(GetBit2(RSDT1.inSTAT1,_SRVMAX)) WritePWM(ProcToMod(99));//WritePWM(ProcToMod(DtKNF.mxco));//WritePWM(ProcToMod(99));
		else WritePWM(ProcToMod(0));
	}
#endif
	if(M._pob_DzCO&&!M._ROZRUCH&&_GRZEJ) WritePWM(PWM.MDMOD);//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxco),PWM.BufPWM));//WritePWM(PWM.MDMOD);
	if(M._pob_DzCW&&!M._ROZRUCH&&_GRZEJ) WritePWM(PWM.MDMOD);//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxcw),PWM.BufPWM));//WritePWM(PWM.MDMOD);	
}
...
void DecRozruch(void)
{
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		...
		case _MONOHB:
		{
			if(!M._ROZRUCH&&!_GRZEJ&&!M._BLOKUJ_ODP)
...
#if _I0_PMP==1                     
					if(!M._ZMKCOCW) _POMPA=0;					//wylacz pompe
#else
                    _POMPA=0;
#endif                    
					M._WYLACZ=0;
		...
		case _UNICO:
		{
			switch(M.WYL)
			{
				...
				case 2:
				{
					if(M._RPM_CW)	//obieg CW
					{
						if(RdPrt(S_ON_PMP_CW)) return;	//niska graniczne temp w obiegu CW
		...
		case _MONOHB:
		{
			switch(M.ZKP)
			{
				...
				case 2:
				{
					...
					if(M._ROZRUCH||_GRZEJ||M._WYLACZ||M._RUSZ_48||M._RUSZ_48Z3) _POMPA=1;			//wlacz pompe
...
void KontrolaWentylatora(void)
{
	...
	else
	{
		...
		{	
			...
			{
				BitSet2(&RSDT1.outSTAT2,_KALIB,0);
				_VNT=0;
				WritePWM(0);
				M.ERR_BTY=0x07;
				PrintErr(0x07,1);
...
void Proba48Z3(void)
{
	...
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		case _MONOHB:								//uklady sterowania z hydroblokiem?
		{
			...
			{
				if(_POMPA||M._ZMKCOCW||M._PROC_ODP) return;
...
void Odpowietrzenie(void)
{
	...
	if(M._BLOKUJ_ODP&&M._PROC_ODP)	//trwa procedura odpowietrzenia?
	{
        ...
        if((GetBit2(RSDT1.inSTAT1,_KPLUS)&&GetBit2(RSDT1.inSTAT1,_KMINUS))||(PFN._off&&DtKNF.enknf&&GetBit2(RSDT1.inSTAT1,_KSET)&&GetBit2(RSDT1.inSTAT1,_KOFF))||M._LIN_ENDO||(M.STRB==1)||(M.STRB==2))	//manualne wyjscie z procedury    
		{
			M._BLOKUJ_ODP=0;				//zakoncz procedure
			M._PROC_ODP=0;
			M.LICZNIK_ODP=0;
...
{ 
	...
	if(((M.ERR_BTY>=0x01)&&(M.ERR_BTY<=0x03))||(M.ERR_BTY==0x07)||(M.ERR_BTY==0x0D)) eeprom_write(_ADRERR,M.ERR_BTY);//WriteEEPROM(_ADRERR,M.ERR_BTY); 	//zapis kodu bledu do pamieci
	M._RUSZ_POMPE=0;
	M._ROZRUCH=0;
	M._MODULUJ=0;
