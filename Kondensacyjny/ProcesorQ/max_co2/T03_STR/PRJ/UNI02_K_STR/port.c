...
#define	_NVPORT	27		/*liczba obslugiwanych portow/sygnalow*/
...
static const Opoznienie Nastawa[_NVPORT]={	
									...
									20,20,		//port nr 25 czas lo/hi oraz hi\lo 
									20,20		//port nr 26 czas lo/hi oraz hi\lo                                    
								 };
/*---------------------------------------------------------------------------------*/
...
void inline RefStan(void)
{
	...
	SPort[6].Input=(PrCW.S_CW)?1:0;						//port/sygnal poddany filtracji    
	//port 7
    ...
    SPort[11].Input=((PrCW.S_CW)?1:0)||((M._LIN_CW)?1:0)||((M._IOT_CW)?1:0);			//port/sygnal poddany filtracji
	//port 12
    if(!M._LIN_BUF&&!M._LIN_BPSS)
    {
#if _TST_RT==0
	...
	//port 25
	SPort[25].Input=(M._LIN_UP)?1:0;					//port/sygnal poddany filtracji    
	//port 26
#if _TST_RT==0    
	SPort[26].Input=((!((M._AKTYWNE_PGD2)?1:0))&&(((_RT_GetValue())?1:0)||((M._I10V_RT)?1:0)||((M._IOT_RT)?1:0)))||(((M._AKTYWNE_PGD2)?1:0)&&((M._pob_PGDRT)?1:0));					//port/sygnal poddany filtracji    
#else
    SPort[26].Input=((!((M._AKTYWNE_PGD2)?1:0))&&(1||((M._I10V_RT)?1:0)||((M._IOT_RT)?1:0)))||(((M._AKTYWNE_PGD2)?1:0)&&((M._pob_PGDRT)?1:0));
#endif
