...
void inline SetTData(void)
{
    ...
    */
#if _TSPRZ_CW==1    
    BufI2C.Bf[1]=PrCW.impCW%100;//imp/sek
	BufI2C.Bf[2]=PrCW.impCW/100;//

	BufI2C.Bf[3]=PrCW.przCCW%100;//ltr/min
	BufI2C.Bf[4]=PrCW.przCCW/100;//
    dt=(PrCW.S_CW==1 ? 10 : 0);
    BufI2C.Bf[5]=dt;//przCW (0-off,1-on))
#else
#if _TEST_COM==1
	BufI2C.Bf[1]=zzz;//DtI2C.CO;//rgl.fz;//PWM.MINPWM/100;//dt;//M.VVNT;//zzz;//dt;//Kvnt[0].dvnt/100;//DtI2C.CO;//M.rVVNT/100;//xxy;//DtI2C.CO;
#else
    BufI2C.Bf[1]=DtI2C.PCO;//(M._RPM_CW)?1:0;//DtI2C.PCO;//PWM.APROC;//PWM.MINPWM%100;//dt2;//xxy;//dt2;//xxy;//DtI2C.PCO;//Kvnt[0].dvnt%100;//DtI2C.PCO;//M.rVVNT%100;//M.VVNT;//vvv;//DtI2C.PCO;//M.VVNT%100
#endif
    BufI2C.Bf[2]=DtI2C.CO;
	BufI2C.Bf[3]=DtI2C.PCW;//(M.CONTSETP);//(M._pob_CO&&DtCO._su_CO)?1:0;//DtI2C.PCW;//(M.rVVNT)%100;//dt;//DtI2C.PCW;//Kvnt[0].dpwm%100;//DtI2C.PCW;//((!((M.FL3.BT3._AKTYWNE_PGD2)?1:0))&&(((_RT_GetValue())?1:0)||((M.FL5.BT5._LIN_RT)?1:0)||((M.FL6.BT6._I10V_RT)?1:0)||((M.FL7.BT7._IOT_RT)?1:0)));//DtZW.ZW;//DtI2C.PCW;//RSDT1.inCONTSETP;//M.CONTSETP;//DtI2C.PCW;//M.CONTSETP%100;//DtI2C.PCW;//PWM.BufPWM%100;//DtI2C.PCW;//DtCO.HstCo;//DtI2C.PCW;//mmm%100;//(vvvv>=0)?vvvv:0;//DtI2C.PCW;//PMP.APROC;//DtI2C.PCW;
	BufI2C.Bf[4]=DtI2C.CW;//(M.rVVNT)/100;//DtI2C.CW;//Kvnt[0].dpwm/100;//DtI2C.CW;//RSDT1.inECO;//M.FL1.BT1._pob_PGDRT;//DtI2C.CW;//DtCO.PGDPCO;//DtI2C.CW;//PWM.BufPWM/100;//DtI2C.CW;//(DtCO.FL0.BT0._ad_CO)?1:0;//DtI2C.CW;//mmm/100;//(vvvv<=0)?(-1)*vvvv:0;//DtI2C.CW;//DtIN.DIN;//DtI2C.CW;    
...
void inline FormatDataIMY(void)
{
	...
	InBitSet(&DtI2C.ST_AD1,b_ERR_CW,0);    
#else
    ...
    else
    {
        ...
        if(M._KPOT)         //konsola z potencjometrami?
        {	
            InBitSet(&DtI2C.PORTC,b_KPOT, 1);
            if(M._KKOM)     //nacisnieto przycisk specjalny?
            {
                InBitSet(&DtI2C.ST_AD3,b_F_LO_CSN,1);    
            }
