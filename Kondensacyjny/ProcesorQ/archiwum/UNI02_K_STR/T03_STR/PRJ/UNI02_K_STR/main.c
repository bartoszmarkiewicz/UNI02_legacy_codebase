...
void KolejkaPobudzen(void)
{
	...
	else
	{
		...
		else 
		{
			...
			M._pob_SRW=0;
			if(M._AKTYWNE_AL) //grzanie na zasobnik nie jest zablokowane i aktywowano funkcje antylegionella?
			{
//				if(_AL_PCW>DtCW.CW) M._pob_AL=1;	//zalacz wymuszenie grzania gdy temperatura w zasobniku jest nizsza od wymaganej            
//				else 
//				{
...
void DecSTR(void)
{
	...
	if(M._pob_SRW&&!M._ROZRUCH&&_GRZEJ_GetValue())
	{
		if(InGetBit(RSDT1.inSTAT1,_SRVMAX)) WritePWM3H(ProcToMod(99));//WritePWM3H(ProcToMod(DtKNF.mxco));//WritePWM3H(ProcToMod(99));
