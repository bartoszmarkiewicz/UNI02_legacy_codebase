...
const CDtPZK CPZK[_KNPAR]=
{
	...
	50,89,0b01010100,			//20 temp CO przy grzaniu CW zasobnika
...
#if _KEY_BBOARD==0   
            kk=(PFNf_aktywne_LKNF||RdPrt(S_KUP))?1:0;       //kkk
			key=PresKeyEX(RdPrt(S_OFF)&&RdPrt(S_SET)&&!kk,&ky[SS_OFF_SET]);     //kkk
#endif
#if _KEY_BBOARD==1      
            kk=(PFNf_aktywne_LKNF||RdPrt(S_KUP_CO))?1:0;        //kkk
...
#if _KEY_BBOARD==0  
                bnrp=nrp;
				ReadParam(&bnrp,&zm,&zp,0,DtKNF.KNPAR-1,1,0);           //kkk         
                nrp=bnrp;
#endif
#if _KEY_BBOARD==1  
                bnrp=nrp;
