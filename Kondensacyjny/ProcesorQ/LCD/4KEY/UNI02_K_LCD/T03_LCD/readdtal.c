...
#if (_ENRDTOXLID!=0)
unsigned char ReadDataXONLID(unsigned char k,volatile StPin *ky,signed int *dt,unsigned char *ovf, unsigned char tid, signed int min, signed int max, const unsigned char step,const unsigned char step2,const unsigned char fst)  //kkk
{
	static bit dtmo=0;
	static unsigned char ktu,iu,pp,es2;
	char key=0;
...
//-------
	if(key=ExxPresKey(k,ky,_RDTOFSXLID,ktu)) 
	{
		if(key==1||((key==2)&&!fst))
		{
			...
			{
				//if(!*dt||((*dt-pp)<min)) *ovf=1;
                if((*dt-pp)<min) *ovf=1;        //kkk
				else *ovf=0;
				//if(*dt&&((*dt-pp)>min)) *dt=*dt-pp;
                if((*dt-pp)>min) *dt=*dt-pp;        //kkk
				else *dt=min;
			}
			else
			{
