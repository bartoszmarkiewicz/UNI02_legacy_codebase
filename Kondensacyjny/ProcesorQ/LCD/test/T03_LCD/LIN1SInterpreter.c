...
unsigned char LIN1SingleResponse_RD(void)
{
    ...
    {      
        ...
        case _CLIN_RRD_SETPP:                                   //informacja zwrotna pobudzen i wartosci oczekiwanych (na potrzeby BYPASS)  //jjj
        {
            FTL1.PCI_LENGTH=0x02;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40
            if(PFNf_EnF10V||(OTMaster.Ref._Control_Setpoint&&PFNf_FOT))                           //aktywny interfejs 10V lub OT?
            {
                FTL1.WRD0.byte1=PFN.IINSETP&0b01111111;             //odeslij wartosc oczekiwana z interfejsow 10V/OT
            }
            else
            {
                FTL1.WRD0.byte1=PFN.SETPmax;
            }
            if(FTL1.WRD0.byte1)
            {
                if(PFN._FOUTRT)                                 //aktywne pobudzenie RT?
                {
                    FTL1.WRD0.byte1|=0b10000000;                //ustaw bit pobudzenia RT (informacja zwrotna na potrzeby BYPASS)
                }
            }
            FTL1.WRD1.byte0=0;
            FTL1.WRD1.byte1=0;
        ...
        case _CLIN_RRD_OTNST:                                   //informacja zwrotna nastaw z interfejsu OT (na potrzeby BYPASS)  //jjj
        {
            ...
            if(PFNf_FOT)    //aktywny interfejs OT?
            {
                ...
                {
                    PFNf_new_IPCO=0;
                    FTL1.WRD0.byte1=PFN.IPCO;            //odeslij wartosc PCO z interfejsu OT
                }
                else
                {
                    FTL1.WRD0.byte1=0;
                ...
                {
                    PFNf_new_IPCW=0;
                    FTL1.WRD1.byte0=PFN.IPCW;            //odeslij wartosc PCW z interfejsu OT
                }
                else
                {
                    FTL1.WRD1.byte0=0;
