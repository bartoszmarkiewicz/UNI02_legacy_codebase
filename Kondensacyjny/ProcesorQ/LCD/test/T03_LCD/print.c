...
//void PrintLN2(const unsigned char laddr)
//{
//#if LCD4D==0
//	SetStatus(&(PWD[_WS_TCO].St),tDEC|tSDID|tRDID|tFOCUS,tALL,_KZ_TCO,_KZ_BAR);
//	SetData(&(PWD[_WS_TCO].Dt),0,tm);												//tempetratura IN
//#else
//	SetStatus(&(PWD[_WS_PRM].St2),tLED|tRDID|tFOCUS,tALL,_KZ_SRV,_KZ_PGD|_KZ_TCO|_KZ_TCW|_KZ_BAR);
//	SetData(&(PWD[_WS_PRM].Dt2),led_L,led_n);
//    if(laddr)
//    {
//        SetStatus(&(PWD[_WS_PRM].St),tDEC|tRDID|tFOCUS,tALL,_KZ_SRV,_KZ_PGD|_KZ_TCO|_KZ_TCW|_KZ_BAR);	//tempetratura IN
//        SetData(&(PWD[_WS_PRM].Dt),0,laddr);
//    }
//    else
//    {
//        SetStatus(&(PWD[_WS_PRM].St),tLED|tRDID|tFOCUS,tALL,_KZ_SRV,_KZ_PGD|_KZ_TCO|_KZ_TCW|_KZ_BAR);	//tempetratura IN
//        SetData(&(PWD[_WS_PRM].Dt),KRESKA,KRESKA);
//    }
//#endif
//}
//---------------------------------------------------------
//---------------------------------------------------------
//signed char GetSignedVar(unsigned char wr, const unsigned char mrg_min, const unsigned char mrg_max)
//{
//    volatile signed char dt=0,zro=0,min=0,max=0; 
//
//    min=mrg_min;
//    max=mrg_max;
//    if(max<min) max=min;
...
//    return dt; 
//}
//wyswietla wartosc parametru ze znakiem (zakres od -99 do 99)
//void PrintSgWPR(signed char wr)     //
//{
//    volatile signed char dt=0;
//    dt=wr;
//#if LCD4D==0
//	SetData(&(PWD[_WS_PRM].Dt),0,lo_wr);
//	SetStatus(&(PWD[_WS_PRM].St),tDEC|tSDID|tFOCUS,tALL,_KZ_SRV,0);
...
/*void PrintNstCO(const unsigned char nst,const unsigned char kt,const unsigned char only)
{
    ...
    //SetStatus(&(DWD[2].St),fl|fk|tDEC|tSDID|tRDID|tFOCUS,tALL,fm|_KZ_DCO,fn|_KZ_BAR|_KZ_PGD);
	//SetStatus(&(DWD[3].St),fl|tLED|tRDID,tALL,0,fo|_KZ_TCO|_KZ_DCO|_KZ_BAR|_KZ_PGD);
	MarkMinMax(0,1);
#if LCD4D!=0
	if(!only)
	{	
