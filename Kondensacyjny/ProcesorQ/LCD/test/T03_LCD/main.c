        ...
        if((PFNf_LIN_KSK||PFNf_LIN_STR||PFNf_LIN_PWD||PFNf_LIN_BUF||PFNf_LIN_BPSS)&&PFNf_FLIN)             //interfejs LIN w trybie STR lub KSK?    //jjj
        {
            ...
            else
            {
                ...
                else
                {
                    if(PFNf_LIN_BPSS)
                    {
                        PFN.INSETP=PFN.IINSETP;                         //wartosc nastawy dla przewyzszenia
                    }
                }
...
{
	volatile static unsigned char obg_cw=0,obg_co=0,zmn2=0,pcw,eco,inst=0;      //mmm
    signed int bfdta, EnEco;   //kkk    //mmm
...
//-----------------
	if(PFNf_set_ncw)	//wybrano nastawe CW?
	{
        ...
        EnEco=(PFNf_ofzima&&PFNf_pmpPWM&&PFNf_trbECO&&!PFNf_errIN&&!PFNf_obgCW)?1:0;        //mmm
		...
		{
			//if(PFNf_mod_ncw) ReadParam((unsigned int *)(&PFN.PCW),&zmn,&zps,PFN.minPCW-1,PFN.maxPCW,0,1);   //kkk
			if(PFNf_mod_ncw)
            {
                bfdta=(signed int)PFN.PCW;//kkk
                ReadParam(&bfdta,&zmn,&zps,PFN.minPCW-1,PFN.maxPCW,0,1);   //kkk 
                PFN.PCW=(unsigned int)bfdta;//kkk                
            }
		}
...
//-----------------
	if(PFNf_set_eco)	//wybrano nastawe parametru ECO?
	{
        ...
        if(PFNf_mod_eco) 
        {
            //ReadParam((signed int *)(&PFN.ECO),&zmn,&zps,1,9,0,1);     //kkk
            bfdta=(signed int)PFN.ECO;//kkk
            ReadParam(&bfdta,&zmn,&zps,1,9,0,1);     //kkk
...
//-----------------    //mmm
    ...
    if(!PFNf_set_ncw&&!PFNf_set_nco&&!PFNf_set_eco)
    {
        inst=0;
    }
//-----------------
...
void NastawaCOCW(void)//unsigned char one)
{
	volatile static unsigned char obg_cw=0,obg_co=0,zmn2=0,pcw,pco,eco,inst=0;      //mmm
    signed int bfdta, EnEco;   //kkk    //mmm
...
//-----------------
	if(PFNf_set_ncw)	//wybrano nastawe CW?
	{
		...
		if(!PFNf_EnAleg)
		{
            ...
            bfdta=(signed int)PFN.PCW;//kkk
            ReadParamCW(&bfdta,&zmn,&zps,PFN.minPCW,PFN.maxPCW,0,1);    //kkk
            PFN.PCW=(unsigned int)bfdta;//kkk
		}
		else
		{
            ...
            bfdta=(signed int)PFN.PCW;//kkk
            ReadParamCW(&bfdta,&zmn,&zps,PFN.minPCW-1,PFN.maxPCW,0,1);  //kkk
            PFN.PCW=(unsigned int)bfdta;//kkk
		}
		if(!PFNf_b_set_ncw)	//nacisnieto przycisk set?
		{
...
//-----------------
	if(PFNf_set_nco)	//wybrano nastawe CO?
	{
        ...
        {
            bfdta=(signed int)PFN.PCO;//kkk
            ReadParamCO(&bfdta,&zmn,&zps,PFN.minPCO,PFN.maxPCO,0,1);     //kkk
            PFN.PCO=(unsigned int)bfdta;//kkk
        }      
#else
#if (_EXT_BOARD==1)     //nowa wersja plytki wyswietlacza z interfejsem 10V  
...
//-----------------
	if(PFNf_set_eco)	//wybrano nastawe parametru ECO?
	{
        ...
        {
            bfdta=(signed int)PFN.ECO;//kkk
            ReadParamCO(&bfdta,&zmn,&zps,1,9,0,1);  //kkk
            PFN.ECO=(unsigned int)bfdta;//kkk
        }
		if(!PFNf_b_set_eco)	//nacisnieto przycisk set?
		{
...
{    
    ...
    //if((PFNf_ofzima||PFNf_oflato)&&!PFNf_ofoff&&!PFNf_opKNF)
...
//    {
//
//        if(GetPoz(_KL_NCWCO,1)) ClrMarkMinMax();
//        else
//        {
//            MarkMinMax2(1,0);
//        }
