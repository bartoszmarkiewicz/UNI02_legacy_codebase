...
{
	volatile unsigned char key,fst,min_par,tm,kk;
	...
	switch(DtLKNF.KRK)
	{
...
#if _KEY_BBOARD==0             
            kk=(PFNf_aktywne_KNF||!PFNf_FLIN||!PFNf_off)?1:0;       //kkk
            key=PresKeyEX(RdPrt(S_BAR)&&!RdPrt(S_RES)&&!kk,&ky[SS_BAR_NRES]); //kkk
#endif
#if _KEY_BBOARD==1             
		...
		case 2:
		{
            ...
            //----------------------------------------------------------------
#if _KEY_BBOARD==0                
                if(!RdPrt(S_BAR)) DtLKNF._keyBAR=0;
                if(!RdPrt(S_RES)) DtLKNF._keyRES=0;
#endif  
#if _KEY_BBOARD==1                
...
#endif            
            if(!DtLKNF._keyBAR&&!DtLKNF._keyRES)
            {
    #if _KEY_BBOARD==0  
                bnrp=nrp;
                ReadParamONE(&bnrp,&zm,&zp,min_par,DtLKNF.KNPAR,1,0);                
                nrp=bnrp;
    #endif
    #if _KEY_BBOARD==1  
                bnrp=nrp;
...
#if _KEY_BBOARD==0 
            bfdta=LPZK.bfdta;
			ReadParamONE(&bfdta,&zm,&zp,LPZK.min,LPZK.max,1,fst);            
            LPZK.bfdta=bfdta;
#endif            
#if _KEY_BBOARD==1         
            bfdta=LPZK.bfdta;
