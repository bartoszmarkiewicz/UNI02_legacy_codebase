...
extern volatile tpMOD2 L2MOD;   //kkk
//unsigned char ccc;
//--------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------
void Mod2KParToOutDtaBuf(void)       //kkk
{
    L2MOD.bfOUT[0]=L2MOD.outDTA[_MOD_OUT0_STATUS];              
    L2MOD.bfOUT[1]=L2MOD.outDTA[_MOD_OUT0_NRMOD];  
    L2MOD.bfOUT[2]=L2MOD.outDTA[_MOD_OUT0_NRPAR]; 
    L2MOD.bfOUT[3]=L2MOD.outDTA[_MOD_OUT0_COUNT];         
    L2MOD.bfOUT[4]=L2MOD.outDTA[_MOD_OUT0_DTA0];  
    
    L2MOD.bfOUT[5]=L2MOD.outDTA[_MOD_OUT1_STATUS];  
    L2MOD.bfOUT[6]=L2MOD.outDTA[_MOD_OUT1_NRMOD]; 
    L2MOD.bfOUT[7]=L2MOD.outDTA[_MOD_OUT1_NRPAR];         
    L2MOD.bfOUT[8]=L2MOD.outDTA[_MOD_OUT1_DTA0];              
    L2MOD.bfOUT[9]=L2MOD.outDTA[_MOD_OUT1_DTA1]; 
    
    L2MOD.bfOUT[10]=L2MOD.outDTA[_MOD_OUT2_STATUS];  
    L2MOD.bfOUT[11]=L2MOD.outDTA[_MOD_OUT2_NRMOD];        
    L2MOD.bfOUT[12]=L2MOD.outDTA[_MOD_OUT2_NRPAR]; 
    
    L2MOD.bfOUT[13]=L2MOD.outDTA[_MOD_OUT3_STATUS];  
    L2MOD.bfOUT[14]=L2MOD.outDTA[_MOD_OUT3_NRMOD];  
    L2MOD.bfOUT[15]=L2MOD.outDTA[_MOD_OUT3_NRPAR]; 
    if(L2MOD._clrREF)
    {
        L2MOD._clrREF=0;
        L2MOD.inREF0=0;
...
void Mod2InDtaToKParBuf(const unsigned char n)       //kkk
{
    if(L2MOD.inREF0>=n&&L2MOD.inREF1>=n&&L2MOD.inREF2>=n&&L2MOD.inREF3>=n)
    {
        L2MOD.inREF=1;
        L2MOD._clrREF=1;
        L2MOD.inDTA[_MOD_IN0_NRMOD]=L2MOD.bfIN[0];              
        L2MOD.inDTA[_MOD_IN0_NRPAR]=L2MOD.bfIN[1];  
        L2MOD.inDTA[_MOD_IN0_NPAR]=L2MOD.bfIN[2]; 
        L2MOD.inDTA[_MOD_IN0_FORMAT]=L2MOD.bfIN[3];         

        L2MOD.inDTA[_MOD_IN1_NRMOD]=L2MOD.bfIN[4];              
        L2MOD.inDTA[_MOD_IN1_NRPAR]=L2MOD.bfIN[5];  
        L2MOD.inDTA[_MOD_IN1_DTA0]=L2MOD.bfIN[6]; 
        L2MOD.inDTA[_MOD_IN1_DTA1]=L2MOD.bfIN[7];         

        L2MOD.inDTA[_MOD_IN2_NRMOD]=L2MOD.bfIN[8];              
        L2MOD.inDTA[_MOD_IN2_NRPAR]=L2MOD.bfIN[9];  
        L2MOD.inDTA[_MOD_IN2_DTA0]=L2MOD.bfIN[10];  
        L2MOD.inDTA[_MOD_IN2_DTA1]=L2MOD.bfIN[11];        

        L2MOD.inDTA[_MOD_IN3_NRMOD]=L2MOD.bfIN[12];              
        L2MOD.inDTA[_MOD_IN3_NRPAR]=L2MOD.bfIN[13];  
        L2MOD.inDTA[_MOD_IN3_DTA0]=L2MOD.bfIN[14];  
        L2MOD.inDTA[_MOD_IN3_DTA1]=L2MOD.bfIN[15];     
    }
}
//--------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------
...
unsigned char LIN1SingleResponse_WR(void)
{
    ...
    volatile unsigned char adr;     //nnn
    ...
    switch(FRL1.IDN)                        //identyfikator komendy
    {
        ...
        case _CLIN_WRD_LIFADDR:              //zmien adres LIN plytki sterujacej UNI-02 (nowy adres w parametr1) po spelnieniu warunku      //nnn
        {
            ...
            switch(krok)        //kkk
            {
                ...
                {                    
                    PFNf_fsetLTrb=(FRL1.WRD1.byte0&0b10000000)?1:0;
                    if(!PFNf_fsetLTrb)      //nadawanie adresu z potwierdzeniem przyciskiem?
                    {
                        PFNf_fOne=0;
                        PFNf_fProcess=0;
                        PFNf_fIgnore=0;  
                        adr=FRL1.WRD1.byte0&0b01111111;
                        //wstepny warunek
                        if((FRL1.IDN==_CLIN_WRD_LIFADDR))  //warunkowe przypisanie adresu odbiornikowi z adresem neutralnym?
                        {
    ...
    #if _KEY_BBOARD==0                         
                            if(PFNf_setKey&&RdPrt(S_BAR))   //przytrzymany przycisk SET?
                            {
    ...
    #if _KEY_BBOARD==0                         
                            if(!RdPrt(S_BAR))               //puszczony przycisk SET?
                            {
                                PFNf_setKey=0;
                                return _LIN1_NORESP;        //zignoruj polecenie
                            }
                            ...
                            if(PFN.ALIN==adr)   //adres zgodny z parametrem1?
                            {
                    ...
                    else         //automatyczne nadawanie adresu 
                    {
                        adr=FRL1.WRD1.byte0&0b01111111;
                        if((FRL1.IDN==_CLIN_WRD_LIFADDR))  //warunkowe przypisanie adresu odbiornikowi z adresem neutralnym?
                        {
                            PFNf_fProcess=1;
                            if(PFNf_fOne) //oczekiwany adres 0x01?
                            {
                                if(adr!=0x01)
                                {
                                    PFNf_fProcess=0;
                            ...
                            if(!PFNf_fIgnore)
                            {
                                if(PFN.ALIN==adr)   //adres zgodny z parametrem1?
                                {
                                    PFNf_fOne=0;
                                    PFNf_fProcess=0;
                                    PFNf_fIgnore=1;                 //ignoruj kolejne polecenia
                                    PFNf_fsetLaddr=1;               //ustaw tryb wyswietlania adresu lin
                                    StartRTdS(_RTALIN);             //timer do autowyjscia z trybu                                  
                                    if(FRL1.NAD) return _LIN1_IFINAL;
                                    else return _LIN1_NORESP;
                ...
                case 3:         //3.Przygotowanie danych dla LIN
                {
                    ...
                    {
                        if(RSDT1.outLDTA1==RSDT1.inLDTA1)
                        {
                            if(PFN.ALIN==RSDT1.inLDTA1)
                            {
                                if(PFNf_fsetLTrb)
                                {
                                    PFNf_fIgnore=1;             //ignoruj kolejne polecenia
        ...
        case _CLIN_WRD_PCW:     //zmiana nastawy CW
        {
            ...
            else
            {
                ...
                {
                    if(!PFNf_off&&!PFNf_fnserw&&!PFNf_aleg&&!PFNf_set_fnserw&&!PFNf_set_aleg&&!PFNf_set_ncw&&!PFNf_set_nco&&!PFNf_set_eco&&!PFNf_str_ncw&&!PFNf_str_nco)
                    {                    
                        if(PFN.PCW!=FRL1.WRD1.byte0)
                        {
                            PFNf_newPCW=1;
        ...
        case _CLIN_WRD_FSR:      //aktywuj/deaktywuj funkcje serwisowa
        {
            ...
            if(!PFNf_fsetLaddr&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF&&!PFNf_opgres)     //blokada nieaktywna?    //kkk
            {
                if(FRL1.WRD1.byte0)
                {
        ...
        {
            if((PFNf_LIN_STR&&PFNf_FLIN)||(PFNf_LIN_KSK&&PFNf_FLIN)||(PFNf_LIN_PWD&&PFNf_FLIN)||(PFNf_LIN_BUF&&PFNf_FLIN))    //LIN w trybie pracy sterowanie?   //jjj
            {
...
unsigned char LIN1SingleResponse_RD(void)
{
    ...
    {      
        ...
        case _CLIN_RRD_LIFADDR:                 //nnn            
        {
            if(FRL1.IDN==_CLIN_RRD_LIFADDR)      //nie odpowiadaj jesli komenda zapisu adresu nie zostala zrealizowana  //nnn
            {
                PFNf_fshowLaddr=1;                  //ustaw tryb wyswietlania adresu lin
                StartRTdS(_RTALIN);                 //timer do autowyjscia z trybu                
                if(PFNf_fProcess||!PFNf_fIgnore) return _LIN1_NORESP;        //ignoruj wywolanie
            }
            ...
            //----------------------------------------
            //ID1
            //bit7-flaga wsparcia dla skroconej multiramki,bit6-oczekiwanie na sygnal czasu,bit5-znacznik odpowiedzi jednego z modulow LIN
            //bit4...bit0 - wersja oprogramowania 
            ...
            //----------------------------------------
            //ID2
            //bit7-oczekiwanie na dane z pionow zasilania,bit6-funkcja obiegu CW (0-aktywna),bit5-czujnik pogodowy (0-aktywny)
            //bit4...bit0 - wersja oprogramowania 2                    
            FTL1.WRD1.byte1=(PFN.vUNI02&0b00011111);            //data1: wersja oprogramowania plytki UNI-02 (0 - brak wsparcia dla identyfikacji wersji)
            //-----------------------
            //ID3      
            //bit7...bit5 - liczba aktywnych podsystemow
            //bit4 - oczekiwanie na parametry konfiguracyjne modulu
            //bit3...bit0 - aktualna konfiguracja sterownika            
            FTL1.WRD2.byte0=(PFNf_EnAleg)?0x01:0x02;            //data2: aktualna konfiguracja sterownika (1F/2F)   
            
            //bit4 - oczekiwanie na parametry konfiguracyjne modulu (KPAR)
            if(LIN2SGetMasterIDO()&&L2MOD.KNMODS)                //odbiornik zdolny do wysylki danych KPAR? //kkk
            {
                FTL1.WRD2.byte0=(FTL1.WRD2.byte0&0b00001111);
                if(LIN2GetModKnfDtaFlg())                       //ustawiona flaga oczekiwania na dane KPAR?
                {
                    FTL1.WRD2.byte0|=0b00010000;                //aktywuj oczekiwanie
                }
                else
                {
                    FTL1.WRD2.byte0=(FTL1.WRD2.byte0&0b00001111);    
            ...
            //-----------------------
            //ID4
            //bit7...bit4-rodzaj plytki wyswietlacza, bit3...bit0 -identyfikator warstwy sprzetowej             
            dt=0x01;                                            //(1-p³ytka UNI-02)                                          
            dt=dt|(0x02<<4);                                    //(0-4 key, 1-7key, 2-Pot)
        ...
        case _CLIN_RRD_PCO:                 //wygeneruj wartosc PCO
        {
            ...
            if(!FRL1f_RSFrame)                 //ramka niskiego poziomu?
            {
                ...
                CMSGDta.DTA6=FRL1.WRD1.byte1;       //Relatywna granica mocy maksymalnej dla grzania co
                if(CMSGDta.DTA6&0b10000000)        //aktywna granica mocy?
                {
                    KFN.LN_MXCO=CMSGDta.DTA6;
                }             
                else
                {
                    KFN.LN_MXCO=0;
        ...
        case _CLIN_RRD_FSR:                //wygeneruj informacje o funkcji serwisowej 
        {
            ...
            else
            {
                ...
                if(!PFNf_ferr&&!PFNf_fmsg&&!PFNf_fsetLaddr&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF&&!PFNf_opgres&&PFNf_zima&&!PFNf_obgCW&&!PFNf_off&&!PFNf_aleg)    //kkk
                {               
        ...
        case _CLIN_RRD_ANL:                 //wygeneruj informacje o stanie funkcji antylegionella
        {
            ...
            else
            {
                ...
                if(!PFNf_ferr&&!PFNf_fmsg&&!PFNf_fsetLaddr&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF&&!PFNf_opgres&&PFNf_EnAleg&&!PFNf_off&&!PFNf_fnserw)    //kkk
                {               
                    FTL1.WRD0.byte1=0;                          //funkcja nieaktywna   
                }
                else 
        ...
        case _CLIN_RRD_INT:                                     //wygeneruj informacje zbiorcza o aktywnosci interfejsach 10V,OT,LIN
        {
            ...
            if(PFNf_LIN_DPGD&&PFNf_FLIN) dt=dt|0b00000001;                                          //deaktywacja wbudowanego regulatora pogodowego za po¶rednictwem interfejsu LIN
            //+++
            ...
            if(PFNf_OT_DPGD&&PFNf_FOT) dt=dt|0b00001000;                                            //deaktywacja wbudowanego regulatora pogodowego za po¶rednictwem interfejsu OT
            
            FTL1.WRD1.byte0=dt;
            //------------------------------
            //docelowa wartosc modulacji           
            if((PFNf_LIN_KSK&&PFNf_FLIN)||(PFNf_LIN_STR&&PFNf_FLIN)||(PFNf_LIN_PWD&&PFNf_FLIN)||(PFNf_LIN_BUF&&PFNf_FLIN))   //jjj
            {
                if(!PFNf_LIN_PWD)
                {
        ...
        case _CLIN_RRD_SETPP:                                   //informacja zwrotna pobudzen i wartosci oczekiwanych (na potrzeby BYPASS)  //jjj  //fff
        {
            FTL1.PCI_LENGTH=0x06;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40
            FTL1.WRD0.byte1=PFN.IINSETP&0b01111111;             //odeslij wartosc oczekiwana z interfejsow 10V/OT na potrzeby regulacji temperatury
            FTL1.WRD0.byte1|=0b10000000;                        //flaga aktualizacji IISETP
            //------------------------------
            //nastawy PCO/PCW z interfejsu OT
            if(PFNf_FOT)    //aktywny interfejs OT?
            {
                if(PFNf_new_IPCO)   //nowa nastawa PCO?
                {
                    PFNf_new_IPCO=0;
                    FTL1.WRD1.byte0=PFN.IPCO&0b01111111;            //odeslij wartosc PCO z interfejsu OT
                    FTL1.WRD1.byte0|=0b10000000;
            }
            else
            {
                ...
                if(PFNf_new_IPCW)   //nowa nastawa PCW?
            {
                    PFNf_new_IPCW=0;
                    FTL1.WRD1.byte1=PFN.IPCW&0b01111111;            //odeslij wartosc PCW z interfejsu OT
                    FTL1.WRD1.byte1|=0b10000000;
                }
                else
                {
            ...
            //------------------------------
            //pobudzenia zewnetrzne RT
            FTL1.WRD2.byte0=0;    
            if(PFNf_stkRT)                                  //aktywne pobudzenie styk RT?
            {
                FTL1.WRD2.byte0|=0b00000001;                //ustaw bit pobudzenia stk RT
            }     
            if(PFNf_pobPGRT)                                  //aktywne pobudzenie PGD RT?
            {
                FTL1.WRD2.byte0|=0b00000010;                //ustaw bit pobudzenia PGD RT
            }            
            if(PFNf_EnF10V&&PFNf_10VRT)                         //aktywne pobudzenie 10V RT?
            {
                FTL1.WRD2.byte0|=0b00000100;                //ustaw bit pobudzenia 10V RT
            }                     
            if(PFNf_FOT&&PFNf_OTRT)                         //aktywne pobudzenie OT RT?
            {
                FTL1.WRD2.byte0|=0b00001000;                //ustaw bit pobudzenia OT RT 
            }
            if(PFNf_EnF10V)                                 //aktywny interfejs 10V?
            {
                FTL1.WRD2.byte0|=0b00010000;                //ustaw bit aktywnosci interfejsu 10V
            }
            if(PFNf_FOT)                                    //aktywny interfejs OT?
            {
                FTL1.WRD2.byte0|=0b00100000;                //ustaw bit aktywnosci interfejsu OT 
            }            
            if(PFN._FOUTRT)                                 //aktywne dowolne z pobudzen RT?
            {
                FTL1.WRD2.byte0|=0b10000000;                //ustaw bit pobudzenia RT (informacja zwrotna na potrzeby BYPASS)
            }           
            //------------------------------            
            //nastawy MXCO z interfejsu OT
            if(PFNf_FOT)    //aktywny interfejs OT?
            {
                if(PFNf_new_IMXCO)   //nowa nastawa PCO?
                {
                    PFNf_new_IMXCO=0;
                    FTL1.WRD2.byte1=KFN.IOT_MXCO&0b01111111;            //odeslij wartosc MXCO z interfejsu OT
                    FTL1.WRD2.byte1|=0b10000000;
                }
                else
                {
                    FTL1.WRD2.byte1=0;
        ...
        case _CLIN_RRD_KPAR0:                                    //zapis wartosci wskazanego parmetru do modulu automatyki  //kkk
        {
            FTL1.PCI_LENGTH=0x06;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)            
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40    
            Mod2KParToOutDtaBuf();
            FTL1.WRD0.byte1=L2MOD.bfOUT[0];                      //status
            FTL1.WRD1.byte0=L2MOD.bfOUT[1];                        //dta0
            FTL1.WRD1.byte1=L2MOD.bfOUT[2];                        //dta1
            FTL1.WRD2.byte0=L2MOD.bfOUT[3];                        //dta2    
            FTL1.WRD2.byte1=L2MOD.bfOUT[4];                        //dta3   
            if(!FRL1f_RSFrame)                 //ramka niskiego poziomu?
            {           
                L2MOD.bfIN[0]=FRL1.WRD1.byte0;                      //status
                L2MOD.bfIN[1]=FRL1.WRD1.byte1;                        //dta0
                L2MOD.bfIN[2]=FRL1.WRD2.byte0;                        //dta1
                L2MOD.bfIN[3]=FRL1.WRD2.byte1;                        //dta2 
            }

            if(L2MOD.inREF0<0xff) 
            {
                L2MOD.inREF0=L2MOD.inREF0++;
            }
            else
            {
                L2MOD.inREF0=1;
        ...
        case _CLIN_RRD_KPAR1:                                    //zapis wartosci wskazanego parmetru do modulu automatyki  //kkk
        {
            FTL1.PCI_LENGTH=0x06;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)            
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40   
            
            FTL1.WRD0.byte1=L2MOD.bfOUT[5];                       //nr parametru (1..n), 0-ignorowane
            FTL1.WRD1.byte0=L2MOD.bfOUT[6];                        //nr parametru
            FTL1.WRD1.byte1=L2MOD.bfOUT[7];
            FTL1.WRD2.byte0=L2MOD.bfOUT[8];    
            FTL1.WRD2.byte1=L2MOD.bfOUT[9];   
            if(!FRL1f_RSFrame)                 //ramka niskiego poziomu?
            {           
                L2MOD.bfIN[4]=FRL1.WRD1.byte0;
                L2MOD.bfIN[5]=FRL1.WRD1.byte1;
                L2MOD.bfIN[6]=FRL1.WRD2.byte0;
                L2MOD.bfIN[7]=FRL1.WRD2.byte1;
            } 
            if(L2MOD.inREF1<0xff) 
            {
                L2MOD.inREF1=L2MOD.inREF1++;
            }
            else
            {
                L2MOD.inREF1=1;
        ...
        case _CLIN_RRD_KPAR2:                                    //zapis wartosci wskazanego parmetru do modulu automatyki  //kkk
        {
            FTL1.PCI_LENGTH=0x04;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)            
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40
            
            FTL1.WRD0.byte1=L2MOD.bfOUT[10];                       //nr parametru (1..n), 0-ignorowane
            FTL1.WRD1.byte0=L2MOD.bfOUT[11];                        //nr parametru
            FTL1.WRD1.byte1=L2MOD.bfOUT[12];
            FTL1.WRD2.byte0=0;    
            FTL1.WRD2.byte1=0;   
            if(!FRL1f_RSFrame)                 //ramka niskiego poziomu?
            {           
                L2MOD.bfIN[8]=FRL1.WRD1.byte0;
                L2MOD.bfIN[9]=FRL1.WRD1.byte1;
                L2MOD.bfIN[10]=FRL1.WRD2.byte0;
                L2MOD.bfIN[11]=FRL1.WRD2.byte1; 
            }  
            if(L2MOD.inREF2<0xff) 
            {
                L2MOD.inREF2=L2MOD.inREF2++;
            }
            else
            {
                L2MOD.inREF2=1;
        ...
        case _CLIN_RRD_KPAR3:                                    //zapis wartosci wskazanego parmetru do modulu automatyki  //kkk
        {
            FTL1.PCI_LENGTH=0x04;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)            
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40  

            FTL1.WRD0.byte1=L2MOD.bfOUT[13];                       //nr parametru (1..n), 0-ignorowane
            FTL1.WRD1.byte0=L2MOD.bfOUT[14];                        //nr parametru
            FTL1.WRD1.byte1=L2MOD.bfOUT[15];
            FTL1.WRD2.byte0=0;    
            FTL1.WRD2.byte1=0;   
            if(!FRL1f_RSFrame)                 //ramka niskiego poziomu?
            {           
                L2MOD.bfIN[12]=FRL1.WRD1.byte0;
                L2MOD.bfIN[13]=FRL1.WRD1.byte1;
                L2MOD.bfIN[14]=FRL1.WRD2.byte0;
                L2MOD.bfIN[15]=FRL1.WRD2.byte1; 
            }        
            if(L2MOD.inREF3<0xff) 
            {
                L2MOD.inREF3=L2MOD.inREF3++;
            }
            else
            {
                L2MOD.inREF3=1;
            }  
            Mod2InDtaToKParBuf(2);               //aktualizacja zbiorcza L2MOD.inDTA[] 
            break; 
        }         
...
unsigned char LIN1StartMultiResponse_RD(void)
{
    ...
    switch(FRL1.IDN)                        //identyfikator komendy
    {
        ...
        case _CLIN_RRD_STD2:                 //odeslij do KSK rozszerzona paczke danych opisujaca stan sterownika
        {
            ...
            if(PFNf_LIN_KSK||PFNf_LIN_STR||PFNf_LIN_PWD||PFNf_LIN_BUF)  //jjj 
            {
                if(PFNf_LIN_KSK||PFNf_LIN_STR)  //jjj
                {
                ...
                {
                    CMSGPar._NST_PCO=0;
                    if(!PFNf_LIN_PWD)       //praca bez przewyzszenia (bez sprzegla)?
                    {
                        if((CMSGPar.PAR1>=PFN.SETPmin)&&(CMSGPar.PAR1<=PFN.SETPmax))
                        {
                            PFN.INSETP=CMSGPar.PAR1;
                    ...
                    else                    //praca z przewyzszeniem (ze sprzeglem)
                    {
        ...
        case _CLIN_RRD_KNF:
        {
            ...
            LIN2GetKnfPar(FRL1.WRD1.byte0, FRL1.WRD1.byte1);        //odczytaj liczbe modolow ogolem, liczbe parametrow najblizszego modulu     //kkk
            //-----------------------------------
            //Odpowiedz zwrotna LCD
            //++++++++++++++++++++++++++++
