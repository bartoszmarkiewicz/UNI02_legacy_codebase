...
unsigned char OTMSGReadData(void)
{
    ...
    switch(OT.RFrame.data_id)
    {
        ...
        //case 14:  //Maximum relative modulation level setting //ooo    
...
unsigned char OTMSGWriteData(void)
{
    ...
    switch(OT.RFrame.data_id)
    {
        ...
        {
            SetWriteAckRespData();
            OTMaster.Ref._Maximum_Mod_Level=1;            
            OTMaster.Maximum_Mod_Level=F88ToData((signed int) OT.RFrame.word0.war);  
            if(OTMaster.Maximum_Mod_Level<0)
            {
                OTMaster.Maximum_Mod_Level=0;
            }      
            if(OTMaster.Maximum_Mod_Level>100)
            {
                OTMaster.Maximum_Mod_Level=100;
            }            
            dt2=(unsigned char) OTMaster.Maximum_Mod_Level;
            if(!PFNf_LIN_BPSS)      //fff
            {                    
                PFNf_new_IMXCO=0;
                KFN.OT_MXCO=dt2;                   //ustaw wartosc [%] ograniczenia mocy]                  
                KFN.OT_MXCO|=0b10000000;           //ustaw flage ogranicznika mocy
                KFN.IOT_MXCO=dt2; 
            }
            else
            {
