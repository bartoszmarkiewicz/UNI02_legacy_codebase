	...
	switch(MGSD.MTRS)
	{
		...
		case 1:
			if(tnDat)
			{
				...
				{
					if(((RSTim()>_TS_TSTR)&&!UADtf_TData)||(RSTim()>_TS_TOUT))			//5 uplynol maksymalny czas oczekiwania na zak. nadawania?	^^^
					{
						MGSD.MTRS=4;
						break;
					}
...
void SaveDataToRS(unsigned char tDTA)
{
	...
	switch(tDTA)
	{
		...
		case _RD_STD2:				//identyfikator paczki danych
		{
            ...
            SetRSTBuf(14,RSDT1.inLNSTAT2);  //+++            
            SetRSTBuf(15,RSDT1.inMXCO);     //ooo            
			SetRSCHK(ndat);					//ndat - CHK z bajtow [0..ndat] i zapisuje na poz. ndat
			//ndat - CHK
			break;
		}        
...
/*---------------------------------------------------------------------------------*/
//Zeruj licznik ramek konfiguracji
void ClrNKFN(void)                      //%%%%
{
    MGSD.KL=0;
}
//Odczytaj licznik ramek konfiguracji
unsigned char GetNKFN(void)             //%%%%
{
    return MGSD.KL;
}
/*---------------------------------------------------------------------------------*/
		...
		case 2:
			...
			if(MGSDf_TransOK)										//transmisja przebiegla bez problemu?
			{
				...
				if(VerifyRSACK()&&VerifyRSCHK(UADt.RNData))			//odpowidz odbiornika pozytywna?
				{
					...
					MGSD.KTR=0;
					RCSTA2bits.CREN=1;
                    if(tDTA==_RD_KNF || tDTA==_RD_KNF2)             //%%%%
                    {
                        if(MGSD.KL<0xFF)
                        {
                            MGSD.KL++;                              //licznik wysylek ramek konfiguracji
                        }                    
                    }
					return 1;
				}
