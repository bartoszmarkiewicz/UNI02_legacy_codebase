...
#if _KEY_BBOARD==0
void ReadParam(signed int *param,unsigned char *zmiana,unsigned char *zapis,signed int min, signed int max, unsigned char rckl, unsigned char fst)      //kkk
...
#endif
#if _KEY_BBOARD==1
void ReadParamCW(signed int *param,unsigned char *zmiana,unsigned char *zapis,signed int min, signed int max, unsigned char rckl, unsigned char fst)
{
	static signed int pnst;
	unsigned char stp,ovi,ovd;
    signed int zkr=0;
...
//#endif								
//}
void ReadParamCO(signed int *param, unsigned char *zmiana,unsigned char *zapis, signed int min, signed int max, unsigned char rckl, unsigned char fst)      //kkk
{
	static signed int pnst;
	unsigned char stp,ovi,ovd;
    signed int zkr=0;
...
//--------------------------FUNKCJE SPECJALNE------------------------------
        ...
        else
        {
            ...
            {            
                ...
                if((RTdS(_RTPCS)>=(2*tm/4))&&(RTdS(_RTPCS)<((3*tm)/4)))
                {
                    PFNf_prn_cs=0;
                    PFNf_prn_in=0;
                    PFNf_prn_pp=1;
            ...
            else
            {
                ...
                if((RTdS(_RTPCS)>=(tm/5))&&(RTdS(_RTPCS)<(2*tm/5)))		//nadszedl czas podtrzymania wskazu temp wlotowej?
                {
                    PFNf_prn_cs=0;
                    PFNf_prn_in=1;
                    PFNf_prn_pp=0;
                ...
                if((RTdS(_RTPCS)>=(2*tm/5))&&(RTdS(_RTPCS)<((3*tm)/5)))
                {
                    PFNf_prn_cs=0;
                    PFNf_prn_in=0;
                    PFNf_prn_pp=1;
                ...
                if((RTdS(_RTPCS)>=(3*tm/5))&&(RTdS(_RTPCS)<((4*tm)/5)))
                {
                    PFNf_prn_cs=0;
                    PFNf_prn_in=0;
                    PFNf_prn_pp=0;
