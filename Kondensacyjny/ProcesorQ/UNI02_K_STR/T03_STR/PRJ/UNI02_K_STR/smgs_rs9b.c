		...
		case _RD_STD2:				//identyfikator paczki danych            
		{
            ...
            switch(nbty)
            {
                ...
                case 17:                            //ooo
                    ...
                    break;                    
                default:
                    if(nbty>=17) RSDT1.vSTD=3;      //ooo
                    else RSDT1.vSTD=3;              //wersja ramki danych
                    break;
            }  
            ...
            RSDT1.inLNSTAT2=GetRSRBuf(14);    
            RSDT1.inMXCO=GetRSRBuf(15);            //ooo
            if(RSDT1.vSTD<3)
            {
                RSDT1.inLNSTAT2=0;
                RSDT1.inMXCO=0;                 //ooo
...
#else
                    if((RSDT1.inLDTA1>=1)&&(RSDT1.inLDTA1<=5)||(RSDT1.inLDTA1>=9)&&(RSDT1.inLDTA1<=12))
#endif
		            {
...
{
    if(RSDT1.vKNF&&(RSDT1.vKNF<7))               //kompatybilnosc wsteczna
    {
        if(n>35) n=35;        //dla sterownikow z oprogramowaniem <17 wielkosc bufora odbioru to 40 bajtow
    }   
    return n;
}
...
void SaveDataToRS(unsigned char tDTA)
{
	...
	switch(tDTA)
	{
		...
		{
			ndat=GetNDAT(4);								//liczba danych wysy³anych (+ CHK)
		...
		case _RD_STD:                           //identyfikator paczki danych           
		{
			ndat=GetNDAT(31);                            //30liczba danych wysy³anych (+ CHK)
            chkndat=SetNDatCHK(ndat);			//oblicz CHK dla liczby danych
			SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
			SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
		...
		case _RD_STD2:                          //identyfikator paczki danych            
		{
			ndat=GetNDAT(39);                            //liczba danych wysylanych (+ CHK)
            SetExNDatCHK(ndat); 
			SetRSTBuf(0,_LCD_ADDR);				//0 - adres master           
			//SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
			...
			SetRSTBuf(36,RSDT1.outSETPPmax);
			SetRSTBuf(37,RSDT1.outPRZ);            
			SetRSCHK(ndat);			//ndat - CHK z bajtow [0..ndat] i zapisuje na poz. ndat
			//ndat - CHK
			break;
		}        
		case _RD_KNF:                                           //identyfikator paczki danych           
		{
			ndat=GetNDAT(30);                                        //liczba danych wysy³anych (+ CHK)
            chkndat=SetNDatCHK(ndat);			//oblicz CHK dla liczby danych
			SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
			SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
		...
		case _RD_LIN:                                           //identyfikator paczki danych
		{
		    switch(RSDT1.inLIDN)                                //komenda LIN
		    {
				...
				{
		            ndat=GetNDAT(7);                             //liczba danych wysy³anych (+ CHK)
		            chkndat=SetNDatCHK(ndat);           //oblicz CHK dla liczby danych
		            SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
		            SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
		            SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
		        ...
		        {
                    ndat=GetNDAT(10);                                            //liczba danych wysy³anych (+ CHK)
                    chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                    SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                    SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                    SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
		        ...
		        case _LIN_GEEST:         //status wylaczenia awaryjnego z blokada
		        {
                    ...
                    {
                        ndat=GetNDAT(28);                                            //liczba danych wysy³anych (+ CHK)
                        chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                        SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                        SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                        SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
		        ...
		        case _LIN_GHIST:         //bufor historii zdarzen
		        {
                    ...
                    {
                        ndat=GetNDAT(6+2);                                            //liczba danych wysy³anych (+ CHK)
                        chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                        SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                        SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                        SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
		        ...
		        {
                    ndat=GetNDAT(6);                                            //liczba danych wysy³anych (+ CHK)
                    chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                    SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                    SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                    SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
		        ...
		        {
                    ndat=GetNDAT(9);                                            //liczba danych wysy³anych (+ CHK)
                    chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                    SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                    SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                    SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
                ...
                case _LIN_WPP:         //zapisz wartosc parametru P
		        {
                    ndat=GetNDAT(8);                                            //liczba danych wysy³anych (+ CHK)
                    chkndat=SetNDatCHK(ndat);                           //oblicz CHK dla liczby danych
                    SetRSTBuf(0,_LCD_ADDR);				//0 - adres master
                    SetRSTBuf(1,chkndat);				//1 - liczba przesy³anych danych (ndat)
                    SetRSTBuf(2,_RD_LIN);				//2 - identyfikator paczki danych
