...
//------------------
	...
	if((M.ERR_BTY=ReadEEPROM(_ADRERR))) 		//uruchomienie po wylaczeniu awaryjnym?
	{
		...
		if(((M.ERR_BTY>=0x01)&&(M.ERR_BTY<=0x03))||(M.ERR_BTY==0x07)||(M.ERR_BTY==0x0D)||(M.ERR_BTY==0x0E))	//dozwolony kod bledu? //
		{
			PrintErr(M.ERR_BTY,1);			//wyswietl kod bledu
			ErrPTG();						//przejdz do petli wylaczenia awaryjnego
		}
...
//------------------	
	...
	_GRZEJ_SetLow();
#else
	M.POB_GRZ=0;					//wentylator na stale wylaczony
	M.POB_VNT=0;    
#endif
#if _HIRESVNT==0            
	WritePWM3(0);
...
#else
		if(M.POB_VNT) PomiarVNT();	//predkosc obrotowa wentylatora        
#endif
        else ClrTachoDta();
		ReceptDataLCD();		//aktualizacja danych pobieranych z modulu LCD
		RefLCDData();			//aktualizacja danych wysylanych do modulu LCD        
...
{
    volatile unsigned char wr;//,plm_cw,plm_co;
	...
	RSDT1.outRDZ0=(KodSterownika()&0b00011111)|((DtKNF.alin&0b00000111)<<5);         //kod sterownika i adres LIN plutki UNI-02 RSDT1.outRDZ0=KodSterownika()
        ...
        wr=0;//RSDT1.outRDZ2&0b11100000;
        wr=wr|((DtKNF.alin&0b00001000)<<2);   
        RSDT1.outRDZ2=wr&0b11100000;
...
//        RSDT1.outRDZ2=wr;
...
#endif
    //plm_cw=((M._RPM_CW||M._pob_UP)&&RdPrt(S_PLM))?1:0;
    //plm_co=(!plm_cw&&!M._RPM_CW&&RdPrt(S_PLM))?1:0;
	InBitSet(&RSDT1.outSTAT0,_PLMCO,M._PRACA&&!M._RPM_CW&&RdPrt(S_PLM));		//plomien CO
    //InBitSet(&RSDT1.outSTAT0,_PLMCO,M._PRACA&&plm_co);		//plomien CO
	InBitSet(&RSDT1.outSTAT0,_PLMCW,M._PRACA&&M._RPM_CW&&RdPrt(S_PLM));		//plomien CW
    //InBitSet(&RSDT1.outSTAT0,_PLMCW,M._PRACA&&plm_cw);		//plomien CW
...
#else
	InBitSet(&RSDT1.outSTAT3,_VNT_ON,M.POB_VNT);									//stan pracy wentylatora (0-postoj, 1-paraca)    
#endif
    ...
    InBitSet(&RSDT1.outSTAT5,_STK_CW2,(RdPrt(S_TIM))?1:0);                        //styk wejscia pobudzenia CW/timer zasobnika (0-rozwarty, 1-zwarty)
    ...
    if(M._AKTYWNE_PGD2&&M._pob_PGDRT)                        //praca z modulacj? w obiegu co - tryb stabilizacji?
    {
        wr=1;
    }
    else
    ...
    InBitSet(&RSDT1.outSTAT5,_POB_PGRT,wr);                                 //pobudzenie RT od regulatora pogodowego    
    if((M._PRACA&&!M._RPM_CW)&&(M._MODULUJ&&rgl.fz))                        //praca z modulacj? w obiegu co - tryb stabilizacji?
    {
        wr=1;
    }
    else
    ...
    InBitSet(&RSDT1.outSTAT5,_FZA_REG,wr);                                    //faza stabilizacji regulatora    
    InBitSet(&RSDT1.outSTAT5,_FEN_PWD,1);                                    //flaga gotowosci pracy na sprzeglo    
    
	InBitSet(&RSDT1.outKSTAT0,_KZCFM,(_KNF_GetValue())?0:1);								//stan zworki konfiguracyjnej
    ...
    if(PrCW.przCW<=25.5)                              //przeplyw ponizej wartosci granicznej?
    {
        wr=(unsigned char)(PrCW.przCW*10);
    }
    else
    {
        wr=255;                                       //dla wiekszych przeplywow nadaj wartosc graniczna
    }        
    RSDT1.outPRZ=wr;                                  //przeplyw cw (*10)
    //------------------------------------------
    ...
    {
        M._KEY7=0;
        if(InGetBit(RSDT1.inSTAT1,_POT))
        {
            M._KPOT=1;                                                          //znacznik konsoli 7key
            if(InGetBit(RSDT1.inSTAT1,_KSET))                                  //nacisnieto przycisk funkcji specjalnych?
            {
                M._KKOM=1;
            }
    ...
    //------------------------------------------
    if(M._LCD&&(RSDT1.inMXCO&0b10000000))     //oktywne zdalne ograniczenie mocy?     //ooo
    {
        M._setMXCO=1;
        M.MXCO=RSDT1.inSTAT1&0b01111111;
        if(M.MXCO>100)
        {
            M.MXCO=100;
        }
...
//                        10V---
    ...
    M._LIN_PWD=InGetBit(RSDT1.inLNSTAT,_LN_PWD)&&M._LIN;                   //znacznik przewyzszenia temperatury    
    M._LIN_BUF=InGetBit(RSDT1.inLNSTAT,_LN_BUF)&&M._LIN;                   //znacznik pracy na bufor ciepla   
    
    M._LIN_UP=InGetBit(RSDT1.inLNSTAT2,_LN_UP)&&M._LIN;   					//ustaw pobudzenie RT z interfejsu LIN
    M._LIN_BPSS=InGetBit(RSDT1.inLNSTAT2,_LN_BPSS)&&M._LIN;   			    //ustaw bypass dla pobudzenia RT
     
    //if(!M._LIN_KSK&&!M._LIN_STR)
    if(M._LIN_BPSS||(!M._LIN_KSK&&!M._LIN_STR))    
    {
        M._I10V=InGetBit(RSDT1.inINSTAT,_F10V)&&M._LCD;   						//ustaw znacznik aktywnego interfejsi 10V
        M._I10V_RT=InGetBit(RSDT1.inINSTAT,_10V_RT)&&M._I10V;     
    }
    ...
    InBitSet(&RSDT1.outKSTAT0,_PWD,M._LIN_PWD);
    InBitSet(&RSDT1.outKSTAT0,_BUF,M._LIN_BUF);
    InBitSet(&RSDT1.outKSTAT0,_UPCW,M._pob_UP);    
    InBitSet(&RSDT1.outKSTAT0,_BPSS,M._setBPSS);
    InBitSet(&RSDT1.outKSTAT0,_OUT_RT,RdPrt(S_OUT_RT));    
}
//------------------
...
//				}
			...
			else
			{
				...
				{
					//grzanie grzania na parametrach wysokich (UP)
					M._pob_CW=0;                    
					if(RdPrt(S_UP)&&((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA)))
					{
						M._pob_UP=1;	//aktywacja pobudzenia
                        M._pob_CO=0;
					...
					else    
                    {
                        ...
                        if(M._pob_UP)           //wyjscie od pracy na parametrach wysokich?
                        {
        					if(M._PRACA)        //aktywne grzanie?
        					{
			...
			if(DtKNF.aleg)
			{
				...
				if(RTM(_SRVRTANL)>=1)						//timer godzinowy wyzwalania funkcji ANTYLEGIONELLA
				{
					...
					if(M.LICZNIK_ANL>=_NGODZIN_SRVAL) 		//licznik godzin do wyzwolenia funkcji ANTYLEGIONELLA
					{
                        ...
                        {
                            //if((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA)) M._AKTYWNE_AL=1;		//aktywacja funkcji ANTYLEGIONELLA po czasie odliczonym przez timer.
                            if(((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA))&&(DtCW.PCW>=DtCW.minPCW)) M._AKTYWNE_AL=1;		//aktywacja funkcji ANTYLEGIONELLA po czasie odliczonym przez timer.   
                        }                        
	...
	if(DtKNF.tpkt==_UNICO)	//obowiazuje tylko dla kotlow z obsluga zasobnika
	{
		...
		if(!M._AKTYWNE_AL)					//mozliwa realizacja funkcji ANTYLEGIONELLA?
		{
			...
			if(DtKNF.aleg)
			{
				...
				{
					StartRTG(_RTANL);
					if(M.LICZNIK_ANL + 1 >= _NGODZIN_AL) 		//licznik godzin do wyzwolenia funkcji ANTYLEGIONELLA
					{
						M.LICZNIK_ANL=0;
						//if((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA)) M._AKTYWNE_AL=1;		//aktywacja funkcji ANTYLEGIONELLA po czasie odliczonym przez timer.
                        if(!(RdPrt(S_ERR_CW)||RdPrt(S_ERR_CO)))	//***************************
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{ 
						M._RUSZ_NTC_CW=0;
						if((DtKNF.rdpm!=0)&&!M._LIN_KSK&&!M._LIN_PWD&&!M._LIN_BUF&&!M._pob_UP)
						{
...
#else
	if(!M._RUSZ_PLOMIEN&&!RdPrt(S_PLM)&&!M._ROZRUCH&&M.POB_GRZ)        
#endif
	{
...
//------------------
//------------------
//Obsluga zdalnego, wzglednego obnizenia mocy       //ooo
unsigned char GetWMXCO(unsigned char mxco)
{
    volatile unsigned int wmxco=0;
    
    if(M._setMXCO)      //aktywne zdalne, wzgledne obnizenie mocy?
    {
        wmxco=DtKNF.def_mxco*M.MXCO;
        wmxco=wmxco/100;
        if(wmxco>100) wmxco=100;
        
        if(mxco>wmxco)
        {
            return (unsigned char)wmxco;
        }       
    }
    return mxco;
}
...
void DecSTR(void)
{
...
#else
	if(!M._ROZRUCH&&M.POB_GRZ&&DtCO._ldDCO) M._RUN_MNMOD=1;
	else M._RUN_MNMOD=0;    
#endif
	//--------------
...
#endif
    {
        if(!M._ZMKCOCW)
        {
            ...
            //wymuszenie dla pracy na parametrach wysokich?
            if(M._PRACA&&M._pob_UP) ToObiegCO();        //eee
            else    
            ...
            if(!M._PRACA&&!M._BLOKUJ_15S&&!M._PROC_ODP&&(RdPrt(S_ON_PMP)||M._RUSZ_48))
            {
                ...
                else                                        //pozycja ZIMA
                {
                    if(DtKNF.tpkt==_UNICO)
                    {
                        ...
                        {
                            //ToObiegCW();                
                            if((!M._pob_KCW && !M._pob_CW && !M._pob_DzCW && !M._pob_AL)&&(M._pob_UP||M._pob_CO)) ToObiegCO();                //brak pobudzenia CW, pobudzenie RT, temp CO poza zakresem
                            else ToObiegCW();                            
                        }
                        else
                        { 
            ...
            if(!M._PRACA&&!M._PROC_ODP&&M._pob_UP&&!DtCO._su_UP) ToObiegCO();       //eee
            else                
            //aktywne RT i temp. CO wyzsza od warunku zalaczenia kotla?	
            //if(!M._PRACA&&!M._BLOKUJ_15S&&!M._PROC_ODP&&((M._pob_CO&&!DtCO._su_CO)||(M._pob_UP&&!DtCO._su_UP))) ToObiegCO();
            if(!M._PRACA&&!M._BLOKUJ_15S&&!M._PROC_ODP&&M._pob_CO&&!DtCO._su_CO) ToObiegCO();       //eee
            else	//kociol w spoczynku
            {
#if _DEMO==0                
                switch(DtKNF.tpkt)		//dla danego rodzaju kotla
                ...
                switch(DtKNF.tpkt)		//dla danego rodzaju kotla
                {
                    ...
                    {
                        //nieaktywna pompa?
                        if(!M._PRACA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&(!M.POB_PMP||M._RUSZ_48Z3)) ToObiegCW();
                        break;
                    }
                    case _UNICO:
                    {					
                        //b.niska temperatura CW lub nieaktywna pompa?
                        //if(!M._PRACA&&!M._PROC_ODP&&(RdPrt(S_ON_PMP_CW)||(!M._WYLACZ&&!M._RUSZ_48&&!M.POB_PMP&&!M._PROC_ODP))) ToObiegCW();
                        if(!M._PRACA&&!M._PROC_ODP&&(RdPrt(S_ON_PMP_CW)))   //b.niska temperatura CW?
                        {
                            ToObiegCW();    
                        }
                        ...
                        {
                            if(!M._WYLACZ&&!M._RUSZ_48&&!M.POB_PMP&&!M._PROC_ODP) ToObiegCO();
                        }
                        break;
                    }
                    default:			//domyslny monotermiczny
                    {
                        //nieaktywna pompa?
                        if(!M._PRACA&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M.POB_PMP) ToObiegCW();
                        break;
                    }
                }                
#endif
	...
	{	
		...
		switch(DtKNF.tpkt)								//dla danego rodzaju kotla
		{
			case _MONOHB:
			{
				if(!_POMPA_GetValue()&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M._RUSZ_48Z3&&!M._PRACA)
				{
					...
					//decyzja o zlaczeniu pompy dla RT lub UP i temp. CO wyzszej od warunku zalaczenia kotla
					if((PFN.poz!=_PFOFF) && (PFN.poz!=_PFLTO) && ((M._pob_CO&&!DtCO._su_CO)||(M._pob_UP&&!DtCO._su_UP)))
					{
						M._WYLACZ=1;				//uaktywnij wybieg pompy	
						_POMPA_SetHigh();
					}
		...
		switch(DtKNF.tpkt)								//dla danego rodzaju kotla
		{
			case _MONOHB:
			{
				if(!M.POB_PMP&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M._RUSZ_48Z3&&!M._PRACA)
				{
					//temperatura CO/CW poza dozwolonym zakresem
					if(RdPrt(S_ON_PMP))
					{
			...
			case _UNICO:
			{
				if(!M.POB_PMP&&!M._WYLACZ&&!M._PROC_ODP&&!M._RUSZ_48&&!M._PRACA)
				{
                    //jest spelniony war temp. pobudzenie CW ale temp. CO jest za wysoka
                    if((DtCW._wpz_CW||M._pob_AL||M._pob_DzCW||M._pob_KCW)&&(PFN.poz!=_PFOFF)&&(!RdPrt(S_ERR_CO)&&!DtCO._ptwp_CO))
                    {
	...
	if(M._PRACA&&(M._pob_UP||M._pob_CO||M._pob_DzCO||M._pob_DzCW)&&!M._pob_SRW&&!M._AKTYWNE_KNF&&!M._pob_KCO&&!M._pob_KCW&&!M._pob_CW&&!M._pob_AL)
	{
...
#else
    if(!M._ROZRUCH&&M.POB_GRZ&&M._pob_CO&&!M._pob_SRW&&!M._AKTYWNE_KNF&&!M._pob_KCO&&!M._pob_KCW&&!M._pob_CW&&!M._pob_DzCW&&!M._pob_AL)        
#endif
	{
        if((M.STRB==1)||(M.STRB==2))                //tryb produkcyjny?
        {
        ...
        else
        {
            if(DtKNF.tmxco2&&(RTM(_RTMMAXCO)<DtKNF.tmxco2))
            {
                if(!PWM._TMAXCO)
                {
                    //DtKNF.mxco=DtKNF.mxco2;		//czasowa praca z moca obnizona  zdefiniowana w konfiguracji
                    DtKNF.mxco=GetWMXCO(DtKNF.mxco2);
            ...
            else
            {
                ...
                {
                    //DtKNF.mxco=DtKNF.def_mxco;	//praca z domyslna moca maksymalna zdefiniowana w konfiguracji
                    DtKNF.mxco=GetWMXCO(DtKNF.def_mxco);
                    ModParam();					//oblicz parametry kluczowe modulatora
                }
                PWM._TMAXCO=0;
                //mmm=0;
	...
	//--------------
	//aktywacja/deaktywacja bloku modulacji plomieniem dla pobudzen RT, UP i CW
	if(M._pob_UP||M._pob_CO||M._pob_CW||M._pob_AL)
	{
#if _DEMO==0        
		if(!M._ROZRUCH&&_GRZEJ_GetValue())
#else
		if(!M._ROZRUCH&&M.POB_GRZ)            
#endif
		{
			if(!M._MODULUJ)
			{
...
#else   //DEMO
	if(M._pob_KCW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3(ProcToMod(DtKNF.dmxcw));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxcw),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCW);
	if(M._pob_KCO&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3(ProcToMod(DtKNF.dmxco));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxco),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCO);
#if _SRVTRB==1														//aktywny tryb serwisowy kompilacji?
	if(M._pob_SRW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3(mypwm);
#else
	if(M._pob_SRW&&!M._ROZRUCH&&M.POB_GRZ)
	{
		if(InGetBit(RSDT1.inSTAT1,_SRVMAX)) WritePWM3(ProcToMod(99));
		else WritePWM3(ProcToMod(0));
	}
#endif
	if(M._pob_DzCO&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3(PWM.MDMOD);//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxco),PWM.BufPWM));//WritePWM(PWM.MDMOD);
	if(M._pob_DzCW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3(PWM.MDMOD);//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxcw),PWM.BufPWM));//WritePWM(PWM.MDMOD);	    
#endif
#else
	//deaktywacja bloku modulacji plomieniem dla awarii modulatora lub dla szybkiego narostu temp.
    if(!M._PRACA||M._RUN_MNMOD||(!M._PRACA&&M._VWYBIEG))
	//if(!M._PRACA||M._RUN_MNMOD||(!M._PRACA&&M._VWYBIEG)||(!M._ROZRUCH&&M._LOW_PMP))
	{
		M._MODULUJ=0;	//wylacz blok modulacji
		if(M._RUN_MNMOD)
		{
...
#else   //DEMO
	if(M._pob_KCW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3H(ProcToMod(DtKNF.dmxcw));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxcw),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCW);
	if(M._pob_KCO&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3H(ProcToMod(DtKNF.dmxco));//WritePWM(PWM.BufPWM=ModToDVNT(ProcToVNT(DtKNF.dmxco),DtKNF.mmin,DtKNF.mmax,PWM.BufPWM));//WritePWM(PWM.MMAXCO);
#if _SRVTRB==1														//aktywny tryb serwisowy kompilacji?
	if(M._pob_SRW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3H(mypwm);
#else
	if(M._pob_SRW&&!M._ROZRUCH&&M.POB_GRZ)
	{
		if(InGetBit(RSDT1.inSTAT1,_SRVMAX)) WritePWM3H(ProcToMod(DtKNF.mxco));//WritePWM3H(ProcToMod(99));
		else WritePWM3H(ProcToMod(0));
	}
#endif
	if(M._pob_DzCO&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3H(ProcToMod(0));//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxco),PWM.BufPWM));//WritePWM(PWM.MDMOD);
	if(M._pob_DzCW&&!M._ROZRUCH&&M.POB_GRZ) WritePWM3H(ProcToMod(0));//WritePWM(ProcToMod(0));//WritePWM(ModToMin());//WritePWM(PWM.BufPWM=ModToDVNT(DtKNF.mmin,DtKNF.mmin,ProcToVNT(DtKNF.mxcw),PWM.BufPWM));//WritePWM(PWM.MDMOD);	     
#endif
#endif    
}
...
void ToObiegCO(void)
{
	...
	switch(DtKNF.tpkt)		//dla danego rodzaju kotla
	{
		...
		case _MONOHB:
		{		
			if(!M._ZMKCOCW&&!M.POB_KCOCW) 			//aktywny obieg CW?
			{
				M._RPM_CW=0;					//wyzeruj znacznik obiegu CW
				M._ZMKCOCW=1;					//uruchom procedure zmiany obiegu CW na CO
				M.ZKP=0;              
...
void ToObiegCW(void)
{
	...
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		...
		case _MONOHB:
		{		
			if(!M._ZMKCOCW&&M.POB_KCOCW) 			//aktywny obieg CO?
			{
				M._RPM_CW=1;					//ustaw znacznik obiegu CW
				M._ZMKCOCW=1;					//uruchom procedure zmiany obiegu CO na CW
				M.ZKP=0;
	...
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		...
		case _MONOHB:
		{
...
#else
			if(!M._ROZRUCH&&!M.POB_GRZ&&!M._BLOKUJ_ODP)                
#endif
			...
			if(!M._RUSZ_POMPE&&!M._RUSZ_CISN&&!M._RUSZ_NCISN)
			{
				...
				//if(M._pob_UP&&DtCO._su_UP&&!M._BLOKUJ_15S)                      //grzanie z parametrami wysokimi
                if(M._pob_UP)                      //grzanie z parametrami wysokimi  //eee    
				{
                    if(DtCO._su_UP)
                    {
//                        M._BLOKUJ_15S=1;
...
#endif
			{
				if(!M._pob_KCW&&!M._pob_AL&&!M._pob_KCO&&!M._pob_SRW&&!M._pob_CW&&!M._pob_UP&&!M._pob_CO&&!M._pob_DzCO&&!M._pob_DzCW)
				{
					SetWylacz();
					return;
				}
				...
				//if(M._pob_UP&&(DtCO._ad_UP||M._BLOKUJ_15S))		//grzanie na parametrach wysokich 
                if(M._pob_UP)		//grzanie na parametrach wysokich     //eee
				{
		 			if(DtCO._ad_UP) SetWylacz();						
				}		
				else                    
...
void SetWylacz(void)
{
	...
	//-----
	M._WYLACZ=1;							//uaktywnij procedure sterowania wybiegiem pompy
	if(M.POB_VNT) M._VWYBIEG=1;					//dla kotla turbo uaktywnij wybieg wentylatora    
#endif
	M.ROZ=0;
	M.MNM=0;
	M._ROZRUCH=0;
...
void BlokadaL3(void)
{
	switch(M.BL3)
	{
		...
		{
			PrintL3();
			if((PFN.poz!=_PFZMA&&!M._ZMKCOCW)||M._pob_CW||M._pob_UP||(DtKNF.tpkt==_UNICO&&DtCW._wpz_CW)) //przelacznik funkcyjny na pozycji innej niz ZIMA lub aktywne?
			{
				M._BLOKUJ_3M=0;
				M.BL3=0;
				EndL3();
...
#else
#if _I0_PMP==1                     
					if(!M._ZMKCOCW) M.POB_PMP=1;					//zalacz pompe
#else
                    M.POB_PMP=1;
#endif                    
#endif
		...
		case 3:
		{
			...
			if(RdPrt(S_PLM)&&!_GRZEJ_GetValue())			//obecny plomien mimo ze _GRZEJ=0?
			{
				...
				ErrPTG();
                    ...
                    M.POB_PMP=1;
#endif        
			if(RdPrt(S_PLM)&&!M.POB_GRZ)			//obecny plomien mimo ze _GRZEJ=0?
			{
				if(RTS(_RTROZ)<_TBPL0) return;
				M.ERR_BTY=0x06;
				PrintErr(0x06,1);					//wyswietl kod bledu
			...
			switch(M.WYL)
			{
				...
				{	
					...
					else			//obieg CO
					{
                        if(!M._pob_UP)
                        {
                            if(RTS(_RTWYL)<DtKNF.tmpm) return;  				//uplynal maksymalny wybieg pompy dla pobudzenia CO?
                            M.WYL=2;
                        }
                        else
                        {
                            if(RTS(_RTWYL)<_TzsWBW) return;                     //uplynal maksymalny wybieg pompy dla pobudzenia UP?
                            M.WYL=2;
                        }
                        return;
					}
				...
				case 2:
				{
					if(!M._RPM_CW)	//obieg CO
					{
                        ...
                        if(M._pob_UP)
                        {
                            if(!RdPrt(S_ERR_CO)&&!DtCO._su_UP) return; 		//osiagnieto wartosc wylaczenia po pracy w ukladzie UP? 
                        }                        
					}
					else            //obieg CW
					{
...
#else
#if _I0_PMP==1                     
					if(!M._ZMKCOCW) M.POB_PMP=0;					//wylacz pompe
#else
                    M.POB_PMP=0;
#endif                    
#endif
...
void DetekcjaPrzeplywuWG(void) 
{
	...
	{
		if(!prz&&!M._DE_PMP)							//zmiana stanu przeplywu?
		{
	 		M._DE_PMP=1;
			M.DPR=0;
		}
		if(M._DE_PMP)
		{
			switch(M.DPR)
			{
				...
				{	
					...
					if(RTS(_RTDPR)<_TDPR1) return;		//oczekuj przez zadany czas na przeplyw WG
...
#else
                   if(M.POB_KCOCW==1) M.POB_KCOCW=0;
                   else M.POB_KCOCW=1;
#endif
					M.ZKP=2;
				}
...
#else
					//warunek ponownego zalaczenia pompy
					if(M._ROZRUCH||M.POB_GRZ||M._WYLACZ||M._RUSZ_48||M._RUSZ_48Z3) M.POB_PMP=1;			//wlacz pompe
					if(DtKNF.robg==_OTWARTY) 							//uniko z obiegiem otwartym?
					{
						if(M._RUSZ_POMPE) M.POB_PMP=1;						//trwa oczekiwanie na powrot czujnika przeplywu WG do prawidlowej pozycji
					}                    
#endif
#endif
				}
...
#else
    if(!M._ZMKCOCW&&!M.POB_PMP)     //pompa nie pracuje?
    {
        M._LOW_PMP=0;
    }    
#endif
...
void ReloadPlomien(void)
{
	...
	if(RTS(_RTRPL2)>_TPLPR2)					//po zadanym czasie wyzeruj licznik_zanik�w_plomienia/min
	{
		M.LICZNIK_PL2=0;
	}
}
...
{
    if(!M.POB_GRZ)     //brak sygnalu grzej i brak plomienia
    {
        M.STN_PLM=1;         //odwrotna logika
        StartRTS(_RTSPLM);
    }
    else
    {
        if(RTS(_RTSPLM)>=3) //po 3 sek. zasymuluj plomien
        {
            M.STN_PLM=0;     //odwrotna logika
        }
    }
}
#endif
...
void KontrolaZatkania(void){
    ...
    else
    {
        ...
        if(!M._AKTYWNE_KNF && M._PRACA && RdPrt(S_PLM)&&(PWM.BufPWM3 >= (lastPWM - 5)) && (PWM.BufPWM3 <= (lastPWM + 5))) {
            ...
            if(stablePWMcount >= 200 && (M.VVNT >= (int)(0.5*(maxRPM+DtKNF.mmin)-2))) { // np. 5 cykli z rz?du ten sam PWM 
                ...
                float e = 0.0;
                if(M.rVVNT>0){
                    e = (M.VVNT*100)/M.rVVNT;
                }
                if ((e > 0.99 && e < 1.01) && M.rVVNT > blad){
...
#else
	if(!M.POB_VNT||!PWM.BufPWM3) 									//wentylator stoi
	{
		StartRTS(_RTRCI);
	}
	else
...
#else
	if(M._RUSZ_48||M._RUSZ_48Z3) return;
	if(M.POB_PMP) StartRTM(_SRVRTANL);
	else
	{
		if(M._PROC_ODP) return;
		if(RTM(_SRVRTANL)>=10)
...
void Proba48Z3(void)
{
	...
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		...
		{
			if(M._RUSZ_48Z3||M._RUSZ_48) return;
			if(M.POB_PMP&&!M.POB_KCOCW) StartRTG(_RTR48Z3); //pompa pracuje w obiegu WU?
			else
			{
				if(M.POB_PMP||M._ZMKCOCW||M._PROC_ODP) return;
				if(RTG(_RTR48Z3)>=_TPR48Z3)
				{
					M.R48Z3=0;
					M._RUSZ_48Z3=1;
	...
	switch(DtKNF.tpkt)								//dla danego rodzaju kotla
	{
		...
		{
			if(M._RUSZ_48Z3||M._RUSZ_48) return;
			if(M.POB_PMP&&!M.POB_KCOCW) StartRTM(_RTMAL); //pompa pracuje w obiegu WU?
			else
			{
				if(M.POB_PMP||M._ZMKCOCW||M._PROC_ODP) return;
				if(RTM(_RTMAL)>=9)
...
void Odpowietrzenie(void)
{
	...
	if(M._BLOKUJ_ODP&&M._AKTYWNE_KNF)
	{
		...
		M.ODP=0;
		ToObiegCO();		//przejdz na obieg CO
		M.POB_PMP=1;			//wlacz wybieg pompy
		M._WYLACZ=1;
		M.WYL=0;
		return;
	}
...
{ 
	...
	if(((M.ERR_BTY>=0x01)&&(M.ERR_BTY<=0x03))||(M.ERR_BTY==0x07)||(M.ERR_BTY==0x0D)||(M.ERR_BTY==0x0E)) if_WriteEEPROM(_ADRERR,M.ERR_BTY);//zapis kodu bledu do pamieci $$$
	M._RUSZ_POMPE=0;
	M._ROZRUCH=0;
	M._MODULUJ=0;
	...
	if(M.POB_VNT) M._VWYBIEG=1;		//aktywacja wybiegu wentylatora
    else M.VVNT=0;              //zeruj wartosc pomiaru predkosci wentylatora    
#endif
	M._RUSZ_PWR=0;
    M._SYNCH_PGD=0;    
	...
	while(1)
	{
...
#else
        if(!M.POB_VNT&&!M._VWYBIEG) M.VVNT=0;    //zeruj wartosc pomiaru predkosci wentylatora        
#endif
		...
		switch(DtKNF.tpkt)								//dla danego rodzaju kotla
		{
			case _MONOHB:
			{
				...
				if(!M._ZMKCOCW&&!M._WYLACZ&&!M._RUSZ_48&&(!M.POB_PMP||M._RUSZ_48Z3)) ToObiegCW();			//nieaktywna pompa?
				//aktywacja wybiegu, zalaczenie pompy		
				if(!M._RUSZ_PWR&&!M._ZMKCOCW&&!M.POB_PMP&&!M._WYLACZ&&!M._RUSZ_48&&!M._RUSZ_48Z3)
				{
					if(!M._RPM_CW&&RdPrt(S_ON_PMP))	 									//temperatura CO/CW poza dozwolonym zakresem?
					{
						M._WYLACZ=1;	//uaktywnij wybieg pompy							 									//uaktywnij procedure wybiegu pompy
			...
			case _UNICO:
			{
                ...
                else
                {
                    ...
                    if(!M._ZMKCOCW&&!M.POB_PMP&&!M._WYLACZ&&!M._RUSZ_48) ToObiegCW();			//nieaktywna pompa?
                }
				//aktywacja wybiegu, zalaczenie pompy		
				if(!M._RUSZ_PWR&&!M._ZMKCOCW&&!M.POB_PMP&&!M._WYLACZ&&!M._RUSZ_48)
				{
					if((!M._RPM_CW&&RdPrt(S_ON_PMP))||(M._RPM_CW&&RdPrt(S_DZ_CW)))		//temperatura CO/CW poza dozwolonym zakresem?
					{
						M._WYLACZ=1;	//uaktywnij wybieg pompy							 									//uaktywnij procedure wybiegu pompy
