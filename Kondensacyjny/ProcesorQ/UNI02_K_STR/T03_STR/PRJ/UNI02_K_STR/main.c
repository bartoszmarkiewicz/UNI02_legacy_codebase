...
void KontrolaZatkania(void){
    ...
    else
    {
        ...
        if(!M._AKTYWNE_KNF && M._PRACA && RdPrt(S_PLM) && (PWM.BufPWM3 >= (lastPWM - 10)) && (PWM.BufPWM3 <= (lastPWM + 10))) {
            stablePWMcount++;
            if(stablePWMcount >= 50 && (M.VVNT >= (int)(0.9*maxRPM))) { // np. 5 cykli z rz?du ten sam PWM 
