...
//wskaz CO
	...
	else
	{
		...
		else //co>pco
		{
            ...
            else            //przewyzszenie temperaturowe
            {
                pco=_MAX_PWCO;                
            }
			...
			switch(DtKNF.tpkt)		//dla danego rodzaju kotla
			{
				...
				case _BITERM2:		//bitermiczny (maxiterm)
				{
                    switch(DtKNF.opdg)
                    {
                        ...
                        case 2:                            
                        {
                            if((M.CONTSETP+hist)>(pco+F1_AD_CO))	//nastawa+hist > pco+F2_AD_CO?
                            {
                                hist=F1_AD_CO;
                            }                            
                            break;
				...
				case _MONOHB:
				{
                    switch(DtKNF.opdg)
                    {
                        ...
                        case 2:                            
                        {
                            if((M.CONTSETP+hist)>(pco+F2_AD_CO))	//nastawa+hist > pco+F2_AD_CO?
                            {
                                hist=F2_AD_CO;
                            }                            
                            break;
				...
				case _UNICO:				//kociol unico
				{
                    switch(DtKNF.opdg)
                    {
                        ...
                        case 2:                            
                        {
                            if((M.CONTSETP+hist)>(pco+F3_AD_CO))	//nastawa+hist > pco+F2_AD_CO?
                            {
                                hist=F3_AD_CO;
                            }                            
                            break;
...
void PomiarCO(void)
{
		...
		case _BITERM2:		//bitermiczny (maxiterm)
		{
            ...
            else    //aktywne przewyzszenie temperaturowe
            {
                switch(DtKNF.opdg)
                {
                    ...
                    case 2:                            
                    {
                        if((M.CONTSETP+DtCO.HstCo)>F1_PWT_CO)	//nastawa+hist > F2_PW_CO?
                        {
                            if(DtCO.CO>=F1_PWT_CO) DtCO._ad_CO=1;
                            else DtCO._ad_CO=0;
                        }
                        else	
                        {
                    ...
                    case 1:
                    {
                        if((M.CONTSETP+DtCO.HstCo)>F1_PPWT_CO)	//nastawa+hist > F2_PPW_CO?
                        {
                            if(DtCO.CO>=F1_PPWT_CO) DtCO._ad_CO=1;
                            else DtCO._ad_CO=0;
                        }
                        else	
                        {
    ...
    switch(DtKNF.opdg)
    {
        ...
        {           
            ...
            {
                if(DtCO.CO<=_ON_WSRW_CO) DtCO._su_SRW_CO=1;
                else DtCO._su_SRW_CO=0;			
                if(DtCO.CO>_OFF_WSRW_CO) DtCO._ad_SRW_CO=1;
                else DtCO._ad_SRW_CO=0;                    
            }            
            break;
        }
...
void PomiarCW(void)
{
    ...
    //granica powyzej ktorej zawor 3DR wysterowany jest tylko na obieg CO, powrót do normalnej pracy po spadku temp. ponizej tego progu
...
#if (_EXT_BOARD>=3)        
        if(M._I10V_RT||M._LIN_STR||M._LIN_KSK||M._IOT_STR||M._LIN_PWD)      //nastawa zdalna?
        {          
            M.CONTSETP=RSDT1.inCONTSETP;
            if(!M._LIN_PWD)  
            {
    ...
    switch(DtKNF.opdg)
    {
        ...
        {                
            ...
            else 
            {
                ...
                {
                    DtCO.PPCO=_OFF_WSRW_CO;
                    RSDT1.outPPCO=_OFF_WSRW_CO;                    
                }
                M.CONTSETP=DtCO.PPCO;
                M.SETPmax=DtCO.PPCO; 
                M.SETPPmax=M.SETPmax;                
        ...
        case 0:            
        {
            if(DtCO.PCO)		//kt>0?
            {
                if(DtZW.SZW<25)	//Tzw<25?
                {
                    ...
                    m*=100;
                    m/=1485;	//(25-10)*99
                    if((_MIN_PCO+m/100)>pco) p=pco;
                    else
                    {    
                        p=_MIN_PCO+m/100;
                        if(p<_MIN_PCO) p=_MIN_PCO;
                        if((p+DtKNF.prop)<pco) p=p+DtKNF.prop;             //przesuniecie rownolegle
                        else p=pco;
                    }
