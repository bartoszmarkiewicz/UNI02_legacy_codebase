		...
		switch(DtKNF.KRK)
		{
			...
			case 4:	//zapis/wyjscie z procedury
			{
				if(!bf_SAVE_PAR)
				{
                    ...
                    if_eeprom_write(_ADRKRES,0);                    //domyslnie zeruj flage wymuszonej kalibracji wentylatora po programowym restarcie oprogramowania
                    ...
                    if(M._KNF_KAL)
                    {
                        if_eeprom_write(_ADRKRES,1);                //ustaw flage wymuszonej kalibracji wentylatora po programowym restarcie oprogramowania
...
unsigned char SaveKnfPar(void)
{
	if(RefKnfPar())
	{
		...
		if(DtKNF.kodkt!=eeprom_read(_ADRWKK))		//zmieniono typ kotla
		{
			SetTpKtParam();							//ustaw param. ukryte dla wybr. kotla
			ModParam();								//oblicz parametry kluczowe modulatora
 			eeprom_write(_ADRWKK,DtKNF.kodkt);
		}
        ...
        {
            if(DtKNF.rfan!=eeprom_read(_ADRRFAN))
            {
                eeprom_write(_ADRRFAN,DtKNF.rfan);
        ...
        {
            if(DtKNF.tstr!=eeprom_read(_ADRTSTR)) eeprom_write(_ADRTSTR,DtKNF.tstr);
        } 
        if((DtKNF.tL3>MinMax[24].min)&&(DtKNF.tL3<MinMax[24].max)||DtKNF.tL3==MinMax[24].min||DtKNF.tL3==MinMax[24].max)
        {
            if(DtKNF.tL3!=eeprom_read(_ADRTL3)) eeprom_write(_ADRTL3,DtKNF.tL3);
        }   
        ...
        {
            if(DtKNF.pgdg!=eeprom_read(_ADRPGDG)) eeprom_write(_ADRPGDG,DtKNF.pgdg);
        }   
        if((DtKNF.pgdo>MinMax[27].min)&&(DtKNF.pgdo<MinMax[27].max)||DtKNF.pgdo==MinMax[27].min||DtKNF.pgdo==MinMax[27].max)
        {
            if(DtKNF.pgdo!=eeprom_read(_ADRPGDO)) eeprom_write(_ADRPGDO,DtKNF.pgdo);
        }
        ...
        {
            if(DtKNF.cwmg!=eeprom_read(_ADRCWMG)) eeprom_write(_ADRCWMG,DtKNF.cwmg);    //histereza wylaczenia CW(2F)              
        }        
...
{
	//-----
	DtKNF.kodkt=eeprom_read(_ADRWKK);						//kod kotla
	...
	DtKNF.mxco=eeprom_read(_ADRMCO);						//moc maksymalna CO	
	DtKNF.dmxco=DtKNF.mxco;	
	DtKNF.def_mxco=DtKNF.mxco;		
	RSDT1.outMMAXG=DtKNF.mxco;
	DtKNF.tmxco2=eeprom_read(_ADRTMCO2);					//czas pracy przy obnizonej mocy w trybie CO
	RSDT1.outTMMAXG2=DtKNF.tmxco2;
	DtKNF.mxco2=eeprom_read(_ADRMCO2);						//wartosc obnizonej mocy w trybie CO
	...
	DtKNF.teco=eeprom_read(_ADRTECO);						//aktywacja/deaktywacja trybu ECO
	RSDT1.outTECO=DtKNF.teco;
	//-----
	DtIN.Eco=eeprom_read(_ADRECO);							//wspolczynnik ECO
...
void SetTpKtParam(void)
{
	switch(DtKNF.kodkt)
	{
		...
		case 10://1780.87.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN10;
			DtKNF.mmax=_DTMMAX10;
			break;
		}
		case 11://1800.86.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN11;
			DtKNF.mmax=_DTMMAX11;
			break;
		}
		case 12://1780.86.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN12;
			DtKNF.mmax=_DTMMAX12;
			break;
		}
		case 13://1800.85.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN13;
			DtKNF.mmax=_DTMMAX13;
			break;
		}
		case 14://1780.85.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN14;
			DtKNF.mmax=_DTMMAX14;
			break;
		}
		case 15://1800.84.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN15;
			DtKNF.mmax=_DTMMAX15;
			break;
		}
		case 16://1780.84.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN16;
			DtKNF.mmax=_DTMMAX16;
			break;
		}
		case 17://1800.83.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN17;
			DtKNF.mmax=_DTMMAX17;
			break;
		}
		case 18://1780.83.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN18;
			DtKNF.mmax=_DTMMAX18;
			break;
		}
		case 19://1800.82.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN19;
			DtKNF.mmax=_DTMMAX19;
			break;
		}
		case 20://1780.82.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN20;
			DtKNF.mmax=_DTMMAX20;
			break;
		}
		case 21://1800.81.00.00
		{
			DtKNF.tpkt=_UNICO;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN21;
			DtKNF.mmax=_DTMMAX21;
			break;
		}
		case 22://1780.81.00.00
		{
			DtKNF.tpkt=_MONOHB;
			DtKNF.czpr=_BRAK;
			DtKNF.tpwm=_TERMET;
			DtKNF.mmin=_DTMMIN22;
			DtKNF.mmax=_DTMMAX22;
			break;
		}
	...
	BitSet2(&RSDT1.outKSTAT0,_KC1F,(DtKNF.tpkt==_UNICO)?1:0);
}
/*---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------*/
...
{
    return eeprom_read(_ADRECO);
}
void WriteEEECO(void)
{
    if(DtIN.Eco!=eeprom_read(_ADRECO))
    {    
        eeprom_write(_ADRECO,DtIN.Eco);
    }
}
/*---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------*/
...
{
	eeprom_write(_ADRVN0,v);
	eeprom_write(_ADRPW0,p);
}
unsigned char ReadEEVNT0_V()
{
	return eeprom_read(_ADRVN0);
}
unsigned char  ReadEEVNT0_P()
{
	return eeprom_read(_ADRPW0);
}
//------
void WriteEEVNT1(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN1,v);
	eeprom_write(_ADRPW1,p);
}
unsigned char ReadEEVNT1_V()
{
	return eeprom_read(_ADRVN1);
}
unsigned char ReadEEVNT1_P()
{
	return eeprom_read(_ADRPW1);
}
//------
void WriteEEVNT2(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN2,v);
	eeprom_write(_ADRPW2,p);
}
unsigned char ReadEEVNT2_V()
{
	return eeprom_read(_ADRVN2);
}
unsigned char ReadEEVNT2_P()
{
	return eeprom_read(_ADRPW2);
}
//------
void WriteEEVNT3(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN3,v);
	eeprom_write(_ADRPW3,p);
}
unsigned char ReadEEVNT3_V()
{
	return eeprom_read(_ADRVN3);
}
unsigned char ReadEEVNT3_P()
{
	return eeprom_read(_ADRPW3);
}
//------
void WriteEEVNT4(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN4,v);
	eeprom_write(_ADRPW4,p);
}
unsigned char ReadEEVNT4_V()
{
	return eeprom_read(_ADRVN4);
}
unsigned char ReadEEVNT4_P()
{
	return eeprom_read(_ADRPW4);
}
//------
void WriteEEVNT5(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN5,v);
	eeprom_write(_ADRPW5,p);
}
unsigned char ReadEEVNT5_V()
{
	return eeprom_read(_ADRVN5);
}
unsigned char ReadEEVNT5_P()
{
	return eeprom_read(_ADRPW5);
}
//------
void WriteEEVNT6(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN6,v);
	eeprom_write(_ADRPW6,p);
}
unsigned char ReadEEVNT6_V()
{
	return eeprom_read(_ADRVN6);
}
unsigned char ReadEEVNT6_P()
{
	return eeprom_read(_ADRPW6);
}
//------
void WriteEEVNT7(unsigned char v,unsigned char p)
{
	eeprom_write(_ADRVN7,v);
	eeprom_write(_ADRPW7,p);
}
unsigned char ReadEEVNT7_V()
{
	return eeprom_read(_ADRVN7);
}
unsigned char ReadEEVNT7_P()
{
	return eeprom_read(_ADRPW7);
}
/*---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------*/
...
/*void WriteEESTAT(void)
{
    ...
    if_eeprom_write(_ADRSTEE+8,RSDT1.outrCO);			//rzeczywista wartosc temp
    if_eeprom_write(_ADRSTEE+9,RSDT1.outrCW);			//rzeczywista wartosc temp    
    if_eeprom_write(_ADRSTEE+10,RSDT1.outCS);
    if_eeprom_write(_ADRSTEE+11,RSDT1.outIN);
    if_eeprom_write(_ADRSTEE+12,RSDT1.outVNT);
    if_eeprom_write(_ADRSTEE+13,RSDT1.outPCO);
    if_eeprom_write(_ADRSTEE+14,RSDT1.outPCW);
    if_eeprom_write(_ADRSTEE+15,RSDT1.outPPCO);
    if_eeprom_write(_ADRSTEE+16,RSDT1.outPPCW);
    if_eeprom_write(_ADRSTEE+17,RSDT1.outERR);
    if_eeprom_write(_ADRSTEE+18,RSDT1.outAWR);
    if_eeprom_write(_ADRSTEE+19,RSDT1.outPP);
    if_eeprom_write(_ADRSTEE+20,RSDT1.outPG);
    if_eeprom_write(_ADRSTEE+21,M.CONTSETP);
    if_eeprom_write(_ADRSTEE+22,RSDT1.outZW);    
}*/
