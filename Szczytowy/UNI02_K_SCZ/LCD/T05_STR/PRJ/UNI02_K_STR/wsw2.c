...
//Ustawia do wykonania pozycje kolejki 
void SetActiveKolPoz(const unsigned char nrkol,const unsigned char nrpoz)
{
    if(nrkol>=NKOL) return;	//obsluga sytuacji wyjatkowej
    if(nrpoz>=Klj[nrkol].nPzc) return;
    if((*Klj[nrkol].KL).KFc[nrpoz].focus==0) return;
    
    KInd[nrkol].indeks=nrpoz;											
	KTim[nrkol].ncS=0;
	KTim[nrkol].ndS=0;
}
#endif
