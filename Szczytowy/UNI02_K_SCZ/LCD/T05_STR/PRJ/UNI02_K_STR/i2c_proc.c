...
void inline SetTData(void)
{
    ...
    */
#if _TSPRZ_CO==1    
    BufI2C.Bf[1]=PrCO.impCO%100;//imp/sek
	BufI2C.Bf[2]=PrCO.impCO/100;//

	BufI2C.Bf[3]=PrCO.przCCO%100;//ltr/min
	BufI2C.Bf[4]=PrCO.przCCO/100;//
    //dt=(PrCO.S_CO==1 ? 10 : 0);
    
    dt=(InRdPrt(S_PRZ_CO)==1 ? 10 : 0);
...
//	BufI2C.Bf[4]=DtI2C.CW;//(M.rVVNT)/100;//DtI2C.CW;//Kvnt[0].dpwm/100;//DtI2C.CW;//RSDT1.inECO;//M.FL1.BT1._pob_PGDRT;//DtI2C.CW;//DtCO.PGDPCO;//DtI2C.CW;//PWM.BufPWM/100;//DtI2C.CW;//(DtCO.FL0.BT0._ad_CO)?1:0;//DtI2C.CW;//mmm/100;//(vvvv<=0)?(-1)*vvvv:0;//DtI2C.CW;//DtIN.DIN;//DtI2C.CW;
	BufI2C.Bf[3]=DtCO.MXCO;//DtKNF.mxco;//moc nastawiana (dla odbioru automatycznego)
	BufI2C.Bf[4]=0;//
...
#else
	InBitSet(&DtI2C.ST_AD1,b_NEW_PCO,DtCO._newMXCO);    
#endif
...
//        {
//            InBitSet(&DtI2C.ST_GLOBAL3,b_RUSZ_CIAG,1);	
//            if(M._KKOM)     //nacisnieto przycisk specjalny?
//            {
//                InBitSet(&DtI2C.ST_AD3,b_F_LO_CSN,1);    
//            }
...
//        }
    ...
    InBitSet(&DtI2C.PORTC,b_PRZEPLYW_CO,PrCO.S_CO);    
#else
	InBitSet(&DtI2C.PORTC,b_PLOMIEN, M.STN_PLM);
	InBitSet(&DtI2C.PORTC,b_ALARM,1);
	InBitSet(&DtI2C.PORTC,b_VNT, M.POB_VNT);   
#if _NO_SYM_PRZ==0
    InBitSet(&DtI2C.PORTC,b_PRZEPLYW_CO,M.STN_PRZ);
#else
    InBitSet(&DtI2C.PORTC,b_PRZEPLYW_CO,PrCO.S_CO);
#endif
#endif
    ...
    if(!M._AKTYWNE_SRW)
    {
        if(M._pob_KCO&&M._pob_KCO2)
        {
            d=1;
        }
        else
        {
            d=(InRdPrt(S_UP_TMP))?1:0;
        }
    }
    //InBitSet(&DtI2C.ST_GLOBAL0,b_S_CW,(!M._AKTYWNE_SRW&&!M._pob_KCO)?InRdPrt(S_UP_TMP):0);
    InBitSet(&DtI2C.ST_GLOBAL0,b_S_CW,d);
//	if(DtKNF.tpkt!=_UNICO)
//	{
