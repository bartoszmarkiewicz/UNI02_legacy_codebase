...
#define	_NVPORT	28		/*liczba obslugiwanych portow/sygnalow*/
...
static const Opoznienie Nastawa[_NVPORT]={	
									...
									10,10,		//port nr 1 czas lo/hi oraz hi\lo
									...
									20,20,		//port nr 24 czas lo/hi oraz hi\lo  $$$ 
									1,1,		//port nr 25 czas lo/hi oraz hi\lo
									1,1,        //port nr 26 czas lo/hi oraz hi\lo  
									20,20		//port nr 22 czas lo/hi oraz hi\lo                                             
								 };
/*---------------------------------------------------------------------------------*/
...
void inline RefStan(void)
{
    ...
    SPort[1].Input=(PrCO.S_CO)?1:0;						//port/sygnal poddany filtracji
#else
#if _NO_SYM_PRZ==0
    SPort[1].Input=(M.STN_PRZ)?1:0;						//port/sygnal poddany filtracji
#else
    //SPort[1].Input=(_PRZ_CO_GetValue())?1:0;
    SPort[1].Input=(PrCO.S_CO)?1:0;						//port/sygnal poddany filtracji
#endif
#endif
	//port 2
...
#else
    SPort[5].Input=(M.STN_PLM)?0:1;						//port/sygnal poddany filtracji
#endif
	//port 6  
	//SPort[6].Input=(_PRZ_CW_GetValue())?0:1;						//port/sygnal poddany filtracji
    SPort[6].Input=(_PRZ_CO_GetValue())?1:0;						//port/sygnal poddany filtracji        
	//port 7
    ...
    SPort[11].Input=((_PRZ_CO_GetValue())?1:0)||((M._LIN_CW)?1:0)||((M._IOT_CW)?1:0);			//port/sygnal poddany filtracji        
	//port 12
#if _ZR_SCZ==0
    if(!M._LIN_BUF)
    {
    ...
    else    //praca na bufor ciepla
    {
        SPort[12].Input=((M._LIN_RT)?1:0);
    }
#else
    //SPort[12].Input=((_RT_GetValue())?1:0)||((_PRZ_CW_GetValue())?0:1);				//port/sygnal poddany filtracji
    SPort[12].Input=(((_RT_GetValue())?1:0)||(_PRZ_CO_GetValue()?1:0)||(M._LIN_UP)?1:0||(M._LIN_RT)?1:0||(M._IOT_RT)?1:0||((M._I10V_RT)?1:0));				//port/sygnal poddany filtracji
#endif
    //port 13
...
#else   
    //SPort[22].Input=((_RT_GetValue())?1:0)||((_PRZ_CW_GetValue())?0:1);				//port/sygnal poddany filtracji
    SPort[22].Input=((_RT_GetValue())?1:0)||((_PRZ_CO_GetValue())?1:0);				//port/sygnal poddany filtracji            
#endif
	//port 23
	SPort[23].Input=(DtZW._en_ZW)?1:0;					//port/sygnal poddany filtracji    
	//port 24
	SPort[24].Input=(DtIN._hot_IN)?1:0;					//port/sygnal poddany filtracji    
	//port 25
	SPort[25].Input=(_KY1_GetValue()) ? 0 : 1;							//_KEY port/sygnal poddany filtracji
	//port 26
	SPort[26].Input=(_KY2_GetValue()) ? 0 : 1;							//port/sygnal poddany filtracji  
    //port 27    
    //SPort[27].Input=(_PRZ_CW_GetValue())?0:1;	//port/sygnal poddany filtracji
    SPort[27].Input=(_PRZ_CO_GetValue()||M._LIN_UP)?1:0;	//port/sygnal poddany filtracji
    //SPort[27].Input=((_PRZ_CW_GetValue())?0:1)||((DtKNF.wymCW)?1:0);	//port/sygnal poddany filtracji
	//...tak do Prt[NVPORT-1]
}
/*---------------------------------------------------------------------------------*/
...
void inline FrsStan(void)
{
	...
	//port 24
	SPort[24].Output=SPort[24].Input;	//stan zgodny ze stanem fizycznym	
	SPort[24].Timer=0;     
    
	//port 25
	SPort[25].Output=SPort[25].Input;	//stan zgodny ze stanem fizycznym	
	SPort[25].Timer=0;  
    
	//port 26
	SPort[26].Output=SPort[26].Input;	//stan zgodny ze stanem fizycznym	
	SPort[26].Timer=0;    
    
    //port 27
	SPort[27].Output=SPort[27].Input;	//stan zgodny ze stanem fizycznym	
	SPort[27].Timer=0; 
