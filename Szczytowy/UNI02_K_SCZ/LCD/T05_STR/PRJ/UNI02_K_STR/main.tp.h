...
typedef struct	{
        ...
        union   {
            ...
            {   
                ...
                unsigned _LIN_PWD:1;        //znacznik przewyzszenia temperaturowego
                unsigned _LIN_BUF:1;        //znacznik pracy na bufor ciepla
                unsigned _LIN_UP:1;         //znacznik pobudzenia grzania na wysokich paramertach                 
            };
        };
        union   {
            unsigned char flag9;
            struct
            {   
				unsigned _pob_DW:1;         //pobudzenie grzania na wysokich parametrach
				unsigned _pob_UP:1;			//pobudzenie grzania na niskich parametrach
                unsigned _KPOT:1;           //plytka LCD z potencjometrami          
                unsigned _pob_KCO2:1;       //znacznik pobudzenia CO (dla ogrzewania CW) z procedury konfiguracji         
                unsigned _fl9_bt4:1;		
                unsigned _fl9_bt5:1;        
                unsigned _fl9_bt6:1;        
                unsigned _fl9_bt7:1;                        
        ...
        unsigned char ODP;
        unsigned char SRV_DW;  
        unsigned char PRN_SRV;
...
#if _DEMO!=0
        unsigned char STN_PLM;      //symulacja stanu PLOMIEN
        unsigned char STN_PRZ;      //symulacja stanu PRZEPLYW CO        
        unsigned char POB_GRZ;      //symulacja pobudzenia GRZEJ     
        unsigned char POB_PMP;      //symulacja pobudzenia POMPA    
        unsigned char POB_VNT;      //symulacja pobudzenia WENTYLATOR
        unsigned char POB_KCOCW;    //symulacja pobudzenia KCOCW        
#endif        
        float VVSU;
        float VVUP;
	}tpMAIN;
