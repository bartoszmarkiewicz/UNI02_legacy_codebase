...
//			}
//		}
		if(((M.ERR_BTY>=0x01)&&(M.ERR_BTY<=0x03))||M.ERR_BTY==0x05||(M.ERR_BTY==0x07)||(M.ERR_BTY==0x0D)||(M.ERR_BTY==0x0E))	//dozwolony kod bledu? //
		{
			PrintErr(M.ERR_BTY,1);			//wyswietl kod bledu
			ErrPTG();						//przejdz do petli wylaczenia awaryjnego
		}
...
//------------------	
    ...
    PomiarPrzCO();          //detekcja przeplywu w obiegu CO
	DtCO._newPCO=0;			//deaktywacja wyswietlania nastawy CO
    DtCO._newMXCO=0;			//deaktywacja wyswietlania nastawy MXCO
    DtCO._wrtPCO=0;			//deaktywacja zapisu nowej nastawy
    DtCO._wrtMXCO=0;			//deaktywacja zapisu nowej nastawy MXCO  
...
#else
        WriteMXCO();            //zapisz nastawe do pamieci
#endif
#endif
	//----------------
...
#if _DEMO==1 && _NO_SYM_PRZ==0
        SymulacjaPrzeplywuWG();
#endif         
...
//    //PrintCO(73,0);
//    
//    if((M._ROZRUCH||_GRZEJ_GetValue())&&!RdPrt(S_PLM))     //trwa zapalanie plomienia?     
//    {
//        PrintOFF(0);  //zapobieganie migotaniu wyswietlacza w czasie iskrzenia
//        EndNst();        
//    }
//	else 
//	{        
//        //if(!M._AKTYWNE_SRW&&!M._AKTYWNE_KNF)
//        {
//            if(!RdPrt(S_PLM))   //brak sygnalu plomien?
//            {
//                PrintOFF(M._BLOKUJ_3M);   
//            }        
//            else
//            {
//                EndOFF();
//                if(RdPrt(S_UP_TMP))     //wymuszenie CW?     
//                {
//                    krp=1;  //kropka na najmlodszej cyfrze
//                }
//                else
//                {
//                    krp=0;
//                }                
//                PrintCO(DtCO.SCO,krp);	//wymuszenie CW            
//            }		
//#if _NST_P3==0            
//            if(DtCO._newPCO)    //zmiana nastawy?
//            {
//                EndOFF(); 
//                if(M._AKTYWNE_PGD)
//                {
//                    krp=1;  //kropka na starszej cyfrze
//                }
//                else
//                {
//                    krp=0;
//                }
//                PrintNstCO(DtCO.PCO,krp);
//            } 
//#else
//            if(DtCO._newMXCO)    //zmiana nastawy?
//            {
//                EndOFF(); 
//                krp=0;
//                PrintNstCO(DtCO.MXCO,krp);
//            }             
//#endif
//        }
//
...
////        {
////            EndOFF();        
////            //PrintNstCO(M.PRN_SRV,(M._AKTYWNE_SRW)?1:0);
////            PrintSRV(M.PRN_SRV);
////        }       
//	}
//
...
//	InBitSet(&RSDT1.outSTAT0,_PLMCW,M._PRACA&&M._RPM_CW&&RdPrt(S_PLM));		//plomien CW
	InBitSet(&RSDT1.outSTAT0,_PLMCO,M._PRACA&&M._pob_DW&&RdPrt(S_PLM));		//plomien CO
	InBitSet(&RSDT1.outSTAT0,_PLMCW,M._PRACA&&M._pob_UP&&RdPrt(S_PLM));		//plomien CW
...
//	InBitSet(&RSDT1.outSTAT1,_OBGCW,M._PRACA&&M._RPM_CW);					//aktywny obieg CW
    ...
    InBitSet(&RSDT1.outSTAT1,_OBGCO,M._PRACA&&M._pob_DW);					//aktywny obieg CO
	InBitSet(&RSDT1.outSTAT1,_OBGCW,M._PRACA&&M._pob_UP);					//aktywny obieg CW
    if(RSDT1.outSTAT1)
    {
        Nop();
        Nop();
...
//    }
    InBitSet(&RSDT1.outSTAT3,_TIM_ZAS,(RdPrt(S_UP_TMP)!=0)?1:0);				//pobudzenie CW
    ...
    InBitSet(&RSDT1.outSTAT5,_STK_CW2,(_PRZ_CO_GetValue())?1:0);                        //styk wejscia pobudzenia CW/timer zasobnika (0-rozwarty, 1-zwarty)
    ...
    InBitSet(&RSDT1.outKSTAT0,_CWIMP,0);                                    //rodzaj czujnika przeplywu CW (0-stykowy, 1-impulsowy) 
#if _DEMO==1 && _NO_SYM_PRZ==0
    if(M.STN_PRZ)
    {
        wr=(unsigned char)(4.5*10);     //4.5 [l/min]
    }
    else
    {
        wr=0;
...
#else    
    if(PrCO.przCO<=25.5)                                                    //przeplyw ponizej wartosci granicznej?
    {
        wr=(unsigned char)(PrCO.przCO*10);
    }
    else
    {
        wr=255;                                       //dla wiekszych przeplywow nadaj wartosc graniczna
    }          												//procentowy wydatek pompy
#endif
    RSDT1.outPRZ=wr;                                 //przeplyw co 	
...
//                        10V---
    ...
    M._LIN_UP=InGetBit(RSDT1.inINSTAT,_LN_UP)&&M._LIN;   					//ustaw pobudzenie RT z interfejsu LIN
    
    if(M._LIN_PWD) M._LIN_BUF=0;    
...
#endif    
    InBitSet(&RSDT1.outKSTAT0,_PWD,0);
    InBitSet(&RSDT1.outKSTAT0,_BUF,0);
...
//				}
    ...
    M._pob_DzCO=0;
    M._pob_DzCW=0;
    M._pob_DW=((M._pob_KCO&&!M._pob_KCO2)||M._pob_SRW||(M._pob_CO&&!RdPrt(S_UP_TMP)))?1:0;       //zbiorczy sygnal pobudzenia dla grzania na niskich paramertach
    M._pob_UP=(!M._pob_SRW&&(M._pob_KCO&&M._pob_KCO2)||(M._pob_CO&&RdPrt(S_UP_TMP)))?1:0;      //zbiorczy sygnal pobudzenia dla grzania na wysokich parametrach
...
//					{ 
//						M._RUSZ_NTC_CW=0;
//						if((DtKNF.rdpm!=0)&&!M._LIN_KSK&&!M._LIN_PWD&&!M._LIN_BUF)
//						{
...
void DecSTR(void)
{
...
#endif
    {
        if(!M._ZMKCOCW)
        {
            ...
            if(!M._PRACA&&M._WYLACZ&&!M._PROC_ODP&&(DtKNF.tpkt==_UNICO)&&(!RdPrt(S_ERR_CO)&&!DtCO._ptwp_CO)&&((PFN.poz==_PFLTO)||(PFN.poz==_PFZMA))&&M._pob_UP) ToObiegCW();
            else
                ...
                else                                        //pozycja ZIMA
                {
                    if(DtKNF.tpkt==_UNICO)
                    {
                        ...
                        if(M._HWB_CW)
                        {
                            ...
                            if(!M._pob_UP&&M._pob_DW) ToObiegCO();                //brak pobudzenia CW, pobudzenie RT, temp CO poza zakresem
                            else ToObiegCW();                            
                        }
                        else
                        { 
                            //if(!M._pob_KCW && !M._pob_CW && !M._pob_DzCW && !M._pob_AL) ToObiegCO();                //brak pobudzenia CW
                            if(!M._pob_UP) ToObiegCO();                //brak pobudzenia CW
                            else ToObiegCW();
                        } 
                    }
                    else
            ...
            if(!M._PRACA&&!M._BLOKUJ_15S&&!M._PROC_ODP&&M._pob_DW&&!DtCO._su_CO) ToObiegCO();
...
//                        else        //nieaktywna pompa?
...
#endif
	{
		...
		{
			if(PWM._TMAXCO||DtKNF.mxco!=DtCO.MXCO)
			{
                DtKNF.mxco=DtCO.MXCO;
				ModParam();					//oblicz parametry kluczowe modulatora
			}
			PWM._TMAXCO=0;
		}        
	...
	else
	{
		...
		PWM._TMAXCO=0;
#else
        if(PWM._TMAXCO||DtKNF.mxco!=DtKNF.def_mxco)
        {
            DtKNF.mxco=DtKNF.def_mxco;		//praca z domyslna moca maksymalna zdefiniowana w konfiguracji
            ModParam();						//oblicz parametry kluczowe modulatora
        }        
...
#else
    if(M._pob_KCO&&M._pob_KCO2&&!M._ROZRUCH&&_GRZEJ_GetValue()) WritePWM3H(ProcToMod(DtKNF.dmxcw));
    if(M._pob_KCO&&!M._pob_KCO2&&!M._ROZRUCH&&_GRZEJ_GetValue()) WritePWM3H(ProcToMod(DtKNF.dmxco));
#endif
...
#else
                    if(!RdPrt(S_UP_TMP)&&(RdPrt(S_ADD_CO)&&!M._BLOKUJ_3M&&!M._BLOKUJ_15S))    
#endif
					{
...
#else
            if(PFN.poz!=_PFZMA||RdPrt(S_UP_TMP))
#endif
			{
				M._BLOKUJ_3M=0;
				M.BL3=0;
	...
	switch(M.ROZ)
	{
		...
		case 1:
		{
			if(DtKNF.robg==_OTWARTY) //uklad z obiegiem otwartym?
			{
				...
				M.ERR_BTY=0x05;
				PrintErr(0x05,1,1);								//wyswietl kod bledu
                ToWriteESTAT();
...
#endif
#if _ZR_SCZ==0            
			if(M._RPM_CW)                   //dla obiegu CW zakoncz wybieg4
#else
            if(RdPrt(S_UP_TMP))             //dla obiegu CW zakoncz wybieg4    
#endif
			{
				M._ROZRUCH=0;
				M.ROZ=0;
...
{
	volatile unsigned char pmp,prz;
#if _ZR_SCZ==0
#if _DEMO==0
...
#else
            //if(prz&&pmp||!(M._pob_KCO||M._pob_SRW||M._pob_CO))					//powrocil przeplyw? 
            //if(prz||!(M._pob_KCO||M._pob_SRW||M._pob_CO))
            if(prz||!(M._pob_DW||M._pob_UP))     
#endif          
			{
				M.RPR=0;
				M._RUSZ_POMPE=0;
...
#if _DEMO!=0 && _NO_SYM_PRZ==0
void SymulacjaPrzeplywuWG(void)
{
    volatile unsigned char pmp;
#if _ZR_SCZ==0
	pmp=(M.POB_PMP)?1:0;    
#else  
    pmp=((M._pob_DW||M._pob_UP))?1:0;
#endif    
    if(!pmp)     //brak sygnalu grzej i brak plomienia
    {
        M.STN_PRZ=0;         
        StartRTS(_RTSPRZ);
    }
    ...
    {
        if(RTS(_RTSPRZ)>=1) //po 1 sek. zasymuluj przeplyw WG
        {
            M.STN_PRZ=1;     
        }
    }
...
//		}
//	}														
	if(((M.ERR_BTY>=0x01)&&(M.ERR_BTY<=0x03))||M.ERR_BTY==0x05||(M.ERR_BTY==0x07)||(M.ERR_BTY==0x0D)||(M.ERR_BTY==0x0E))
    {
        if_WriteEEPROM(_ADRERR,M.ERR_BTY);//zapis kodu bledu do pamieci $$$
    }
	M._RUSZ_POMPE=0;
	M._ROZRUCH=0;
	M._MODULUJ=0;
