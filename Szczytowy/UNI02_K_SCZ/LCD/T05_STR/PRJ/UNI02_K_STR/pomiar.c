...
//    else
//    {
//        iavr = iavr + ((int)AC_DTA - iavr) / 50;             //wartosc srednia pomiaru
//    }    
//    AC_AVR=(unsigned char)((iavr)>>8);
    
    AC_AVR=(unsigned char)(AC_DTA>>8);
    ...
    //return;
#else
    volatile static unsigned char b_tmp=0;   
    volatile unsigned char n_tmp=0,step;    
    
    DtCO.HstCo=DtKNF.hsco;
	DtCO.F3PPCO=DtKNF.conw;    
    DtCO.AC_CO=_NTC_MIN+(_NTC_MAX-_NTC_MIN)/4;;
    //DtCO.CO=21;
    DtCO._err_CO=0;
    
    if(DtCO.CO<21) DtCO.CO=21;
    if(!M.POB_GRZ)
    {
        n_tmp=21;
    }
    ...
    {     
#if _ZR_SCZ==0            
		if(!M._RPM_CW)  //praca w obiegu CO?
#else
	...
	{
		StartRTdS(_RTDCO);
        if(!M._ROZRUCH&&M.POB_GRZ) step=2;
        else step=1;
        if(b_tmp!=n_tmp)
        {
            if(b_tmp>n_tmp)
...
#else 
                        if(RdPrt(S_UP_TMP))             //dla obiegu CW 
                        {
                            if((DtCO.CO-M.CONTSETP)>=F3_AD_CO) DtCO._ad_CO=1;
                            else DtCO._ad_CO=0;                                                                    
                        }
                        else
                        {
...
void PomiarIN(void)
{
...
#else
    DtIN.AC_IN=_NTC_MIN+(_NTC_MAX-_NTC_MIN)/4;
    DtIN._err_IN=0;
    //DtIN.IN=20; 
    
    if(DtIN.IN<21) DtIN.IN=21;
    
    if(!M._RPM_CW&&!M._KCW_PMP&&M.POB_PMP)
    {
        if((DtCO.CO-21)>=3) 
        {
            DtIN.IN=DtCO.CO-3;
    ...
    //$$$
    if(DtIN.IN>=_HOT_IN_CO)     //wysoka temperatura na powrocie obiegu CO?
	{
		DtIN._hot_IN=1;
	}
	else DtIN._hot_IN=0;
...
void PomiarCW(void)
{
	...
	DtCW.AC_CW=_NTC_MIN+(_NTC_MAX-_NTC_MIN)/4;
    DtCW._err_CW=0;
	//DtCW.CW=21;  
    if(DtCW.CW<21) DtCW.CW=21; 
    ...
    {
        if(M._RPM_CW)       //praca w obiegu CW?
        {
...
#else
            DtZW.AC_ZW=_ZNTC_MIN+(_ZNTC_MAX-_ZNTC_MIN)/4;;
            DtZW.ZW=5;            
#endif
            RSDT1.outZW=DtZW.ZW;
...
//        }
        ...
        switch(DtKNF.opdg)
        {
            ...
            case 1:
            {
                if(ppc>_MXPPW_CO) ppc=_MXPPW_CO;          //wartosci >60 sa sprowadzane do wartosci 60
                break;
            }
            default:
            {
...
//        }
        ...
        {
            DtCO.PPCO=DtKNF.conw;
            RSDT1.outPPCO=DtKNF.conw;
            M.CONTSETP=DtCO.PPCO;
            M.SETPmax=DtCO.PPCO;
            M.SETPPmax=M.SETPmax;
        }        
...
void WritePCO(void)
{
	...
	if(!M._AKTYWNE_KNF)
	{
		if(DtCO._wrtPCO&&RTdS(_RTPCO)>=_TWRTPCO)
		{
			DtCO._wrtPCO=0;
			StartRTdS(_RTPCO);
            if(!M._AKTYWNE_PGD)
...
/*---------------------------------------------------------------------------------*/
//Odczyt nastawy MXCO z pamieci EEPROM
unsigned char ReadMXCO(void)
{
    return ReadEEMXCO();   //odczyt nastawy z pamieci EEPROM
}
/*---------------------------------------------------------------------------------*/
//Zapis nastawy MXCO po ustalonym czasie do EEPROM
void WriteMXCO(void)
{
#if _KEY_PCO==0
	if(DtCO._wrtMXCO&&RTdS(_RTPCO)>=_TWRTPCO)
	{
		DtCO._wrtMXCO=0;
		StartRTdS(_RTPCO);
        WriteEEMXCO();   //zapis nastawy w pamieci EEPROM
	}    
#else
	if(!M._AKTYWNE_KNF)
	{
...
void PomiarPCO(void)
{
	...
	{
		if(!M._RUSZ_PLOMIEN&&!M._RUSZ_STB&&!M._RUSZ_POMPE&&!M._RUSZ_NTC_CO)
		{
			if(!M._AKTYWNE_KNF&&!M._AKTYWNE_SRW)
			{
				if(!RdPrt(S_KDW))
				{
	 				ReadDataXONLI(RdPrt(S_KUP),&DtCO.PCO,DtCO.maxPCO,1,1);
					if(RdPrt(S_KUP)&&(DtCO.PCO==DtCO.maxPCO)) DtCO._newPCO=1;
				}
				if(!RdPrt(S_KUP))
				{
	 				ReadDataXONLD(RdPrt(S_KDW),&DtCO.PCO,DtCO.minPCO,1,1);
					if(RdPrt(S_KDW)&&(DtCO.PCO==DtCO.minPCO)) DtCO._newPCO=1;
				}
				if(!RdPrt(S_KDW)&&!RdPrt(S_KUP)) //oba przyciski wycisniete
				{
					if(DtCO._wrtPCO)
					{
						if(RTdS(_RTPCO)>_TFLPCO)	DtCO._newPCO=0;	 //podtrzymanie wyswietlania nastawy przez czas _TWRTPCW-1
					}
					else DtCO._newPCO=0;
				}
				if(DtCO.PCO!=pnst)
				{
					...
					RNst(DtCO.PCO);
					//-------------------
					DtCO._wrtPCO=1;									//zapis nowej nastawy w pamieci
					StartRTdS(_RTPCO);
				}
			}else DtCO._newPCO=0;
		}else DtCO._newPCO=0;
    ...
    DtCO._wrtPCO=0;
    //----------------------------------------------------------------------------------------------------
	//ustaw wartosc nastawy PCO oraz jej wartosci graniczne
    SetMinMaxPCO();          
    DtCO.PCO=DtKNF.pgmx;            //nastawa PCO jest rowna parametrowi P29 
    
    //je?eli P29 jest wieksze od gornej wartosci granicznej dla nastaw wynikajacej z wyboru wartosci parametru P8, to PCO bedzie rowne gornej wartosci granicznej
    if(DtCO.PCO<DtCO.minPCO)
    {
        DtCO.PCO=DtCO.minPCO;
    }
    //je?eli P29 jest mniejsze od dolnej wartosci granicznej dla nastaw wynikajacej z wyboru wartosci parametru P8, to PCO bedzie rowne dolnej wartosci granicznej
    if(DtCO.PCO>DtCO.maxPCO)
    {
        DtCO.PCO=DtCO.maxPCO;
    } 
    DtCO.PGDPCO=DtCO.PCO;
    //RSDT1.outPCO=DtCO.PCO;  
    ...
    //ustaw wartosc nastawy MXCO oraz jej wartosci graniczne
#if _KEY_PCO==0
    DtCO.minMXCO=0;
    DtCO.maxMXCO=DtKNF.def_mxco;    
    RSDT1.outPCOmn=DtCO.minMXCO;
	RSDT1.outPCOmx=DtCO.maxMXCO;  
    
	if(!InGetBit(RSDT1.inSTAT0,_INITDTA)&&MGSD._CON&&!M._PW_START&&!M._RS_START)
	{
        pnst=RSDT1.inPCO;
        if(pnst<DtCO.minMXCO) pnst=DtCO.minMXCO;	//nastawa w dozwolonym zakresie?
        if(pnst>DtCO.maxMXCO) pnst=DtCO.maxMXCO;	//nastawa w dozwolonym zakresie?
        if(DtCO.MXCO!=pnst)	//nowa wartosc nastawy?
        {
            DtCO.MXCO=pnst;
            DtCO._wrtMXCO=1;
            StartRTdS(_RTPCO);
        ...
        RSDT1.outPCO=DtCO.MXCO;  
	}
	else
	{
		DtCO.MXCO=ReadMXCO();
		if(DtCO.MXCO<DtCO.minMXCO) pnst=DtCO.minMXCO;	//nastawa w dozwolonym zakresie?
		if(DtCO.MXCO>DtCO.maxMXCO) pnst=DtCO.maxMXCO;	//nastawa w dozwolonym zakresie?
		RSDT1.outPCO=DtCO.MXCO;      
	}     
#else    
    DtCO.minMXCO=0;
	...
	if(!(RdPrt(S_KDW)&&RdPrt(S_KUP))) 					//nie nacisnieto jednoczesnie obu przyciskow
	{
		if(!M._RUSZ_PLOMIEN&&!M._RUSZ_STB&&!M._RUSZ_POMPE&&!M._RUSZ_NTC_CO)
		{
			if(!M._AKTYWNE_KNF&&!M._AKTYWNE_SRW)
			{
				if(!RdPrt(S_KDW))
				{
	 				ReadDataXONLI(RdPrt(S_KUP),&DtCO.MXCO,DtCO.maxMXCO,1,1);
					if(RdPrt(S_KUP)&&(DtCO.MXCO==DtCO.maxMXCO)) DtCO._newMXCO=1;
				}
				if(!RdPrt(S_KUP))
				{
	 				ReadDataXONLD(RdPrt(S_KDW),&DtCO.MXCO,DtCO.minMXCO,1,1);
					if(RdPrt(S_KDW)&&(DtCO.MXCO==DtCO.minMXCO)) DtCO._newMXCO=1;
				}
				if(!RdPrt(S_KDW)&&!RdPrt(S_KUP)) //oba przyciski wycisniete
				{
					if(DtCO._wrtMXCO)
					{
						if(RTdS(_RTPCO)>_TFLPCO)	DtCO._newMXCO=0;	 //podtrzymanie wyswietlania nastawy przez czas _TWRTPCW-1
					}
					else DtCO._newMXCO=0;
				}
				if(DtCO.MXCO!=pnst)
				{
					pnst=DtCO.MXCO;
					...
					RNst(DtCO.MXCO);
					//-------------------
					DtCO._wrtMXCO=1;									//zapis nowej nastawy w pamieci
					StartRTdS(_RTPCO);
				}
			}else DtCO._newMXCO=0;
		}else DtCO._newMXCO=0;
...
//#else
//    DtCW.PCW=DtCW.CW;
//    DtCW.minPCW=DtCW.PCW;
//    DtCW.maxPCW=DtCW.PCW;
//    RSDT1.outPCW=DtCW.PCW;
//	RSDT1.outPCWmn=DtCW.minPCW;
//	RSDT1.outPCWmx=DtCW.maxPCW;    
...
#endif
#if _ZR_SCZ==1
    DtCW.PCW=DtCW.minPCW;//ReadEEPCW();
    RSDT1.outPCW=DtCW.PCW;
	RSDT1.outPCWmn=DtCW.minPCW;
	RSDT1.outPCWmx=DtCW.maxPCW;      
#endif
