...
void OTInterpreter(void)
{
    if(OT.ToACK)                                     //odebrano paczke danych, przejdz do interpretacji   
    {
        ...
        if(MGSDf_NO_ODB||(PFNf_LIN_KSK&&PFNf_FLIN)||(PFNf_LIN_STR&&PFNf_FLIN))    //brak polaczenia z UNI-02 lub wersja oprogramowania UNI-02<09 lub aktywny interfejs LIN w trybie sterowania?  
        {
            OTMaster.Ref.war=0;
...
unsigned char OTMSGReadData(void)
{
    ...
    switch(OT.RFrame.data_id)
    {
        ...
        case 3:     //Slave configuration
        {
            ...
            OT.TFrame.byte1.b0=0;   //DHW present [ dhw not present, dhw is present ]
...
//            else OT.TFrame.byte1.b3=1;
            OT.TFrame.byte1.b3=1;   //DHW config [instantaneous or not-specified,storage tank]           
...
//            OT.TFrame.byte0.b1=1;   //max CHsetpoint [ read-only, read/write ]        
            OT.TFrame.byte1.b0=0;   //0: DHW Setpoint [ transfer disabled, transfer enabled ]
            OT.TFrame.byte1.b1=0;   //max CHsetpoint [ transfer disabled, transfer enabled ]   
            OT.TFrame.byte0.war=0;  //Remote-parameter transfer-enable flags
            OT.TFrame.byte0.b0=0;   //DHW Setpoint [ read-only, read/write ]
            OT.TFrame.byte0.b1=0;   //max CHsetpoint [ read-only, read/write ]            
            return _OT_TO_RESP;            
        }
...
//            if((OTMaster.Control_Setpoint>=PFN.SETPmin)&&(OTMaster.Control_Setpoint<=PFN.SETPmax))   //dolna granica =minPCO, gorna granica =PCO
//            {
//                PFN.INSETP=(unsigned char) OTMaster.Control_Setpoint;
//            }
//            else
//            if(OTMaster.Control_Setpoint<PFN.SETPmin)    
//            {
//                //PFN.PCO=PFN.minPCO;
//                PFN.INSETP=PFN.SETPmin;
//            }
//            else
//            if(OTMaster.Control_Setpoint>PFN.SETPmax)
//            {
//                //PFN.PCO=PFN.maxPCO;
...
//            if((OTMaster.DHW_Setpoint>=PFN.minPCW)&&(OTMaster.DHW_Setpoint<=PFN.maxPCW))
//            {
//                PFN.PCW=(unsigned int)OTMaster.DHW_Setpoint;
//            } 
//            //***********************
//            else
//            if(OTMaster.DHW_Setpoint<PFN.minPCW)
//            {
//                PFN.PCW=PFN.minPCW;
//            }
//            else
//            if(OTMaster.DHW_Setpoint>PFN.maxPCW)
//            {
//                PFN.PCW=PFN.maxPCW;
//            }  
            SetUkownAckRespData();
...
//            {
//                SetWriteAckRespData();
//                if((OTMaster.maxCH_Setpoint>=PFN.minPCO)&&(OTMaster.maxCH_Setpoint<=PFN.maxPCO))
//                {
//                    PFN.PCO=(unsigned int)OTMaster.maxCH_Setpoint;
//                }
//                //******************************
//                else
//                if(OTMaster.maxCH_Setpoint<PFN.minPCO)    
//                {
//                    PFN.PCO=PFN.minPCO;
//                }
//                else
//                if(OTMaster.maxCH_Setpoint>PFN.maxPCO)
//                {
//                    PFN.PCO=PFN.maxPCO;      
//                }  
//                //*****************************
