...
//            }
        ...
        if(!k&&!j&&!PFNf_opgres)
        {
            ...
            BitSet3(&RSDT1.inSTAT1,_POT,0);                        //znacznik interfejsu z potencjometrami             
#endif
#if _KEY_BBOARD==1  
            if(!k&&!j)
            {            
...
//        }
     ...
     //--------------------------- 
    PFNf_cwIMP=0;     //flaga obecnosci impulsowego czujnika przeplywu CW
    PFNf_PWD=0;         //znacznik trybu pracy ze stabilizacja temperatury
    PFNf_BUF=0;         //znacznik trybu pracy na bufor ciepla    
...
//STANDARD
    ...
    if(!PFNf_EnF10V&&!PFNf_LIN_KSK&&!PFNf_LIN_STR&&!PFNf_LIN_PWD&&!PFNf_LIN_BUF&&!(OTMaster.Ref._Control_Setpoint&&PFNf_FOT))
    {
    ...
    {
        if((PFNf_LIN_KSK||PFNf_LIN_STR||PFNf_LIN_PWD||PFNf_LIN_BUF)&&PFNf_FLIN)             //interfejs LIN w trybie STR lub KSK?
        {
            if(!PFNf_LIN_PWD)
            {
            ...
            {
                PFN.CONTSETP=FormatSETPP(PFN.INSETPP);               //natychmiastowa aktualizacja SETP 
                if(!PFNf_EnF10V&&!(OTMaster.Ref._Control_Setpoint&&PFNf_FOT)) //interfejsy 10V i OT nie sa aktywne
                {
                    PFN.INSETP=PFN.SETPmax;                         //wartosc nastawy dla przewyzszenia
                }
            }
        }
...
#if _TEST_UP==0
    BitSet3(&RSDT1.inINSTAT,_LN_UP,(PFNf_LIN_UP&&PFNf_FLIN));                  //pobudzenie grzania na wysokich parametrach
#else
    BitSet3(&RSDT1.inLNSTAT,_FLIN,1);        //odswiez znacznik komunikacji LIN dla ukladu UNI-02
    StartRTdS(_RTFLIN);
    PFNf_FLIN=1;   
    PFNf_LIN_UP=1;
    BitSet3(&RSDT1.inINSTAT,_LN_UP,1);                  //pobudzenie grzania na wysokich parametrach
#endif   

#if _TEST_UP==0 && _TEST_DW==1
    BitSet3(&RSDT1.inLNSTAT,_FLIN,1);        //odswiez znacznik komunikacji LIN dla ukladu UNI-02
    StartRTdS(_RTFLIN);
    PFNf_FLIN=1;  
    PFNf_LIN_RT=1;
    BitSet3(&RSDT1.inINSTAT,_LN_RT,1);                  //pobudzenie grzania na wysokich parametrach
#endif
    ...
    BitSet3(&RSDT1.inINSTAT,_LN_UP,0);                //pobudzenie grzania na wysokich parametrach
    PFN.STRB=0;
#endif    
...
{   
	Mark(_GZ_MNCW,shCW&&(PFN.PCO<=PFN.minPCO));
	Mark(_GZ_MXCW,shCW&&(PFN.PCO>=PFN.maxPCO));    
...
//            }
            PrintVER(0x1F,(nr)? nr:(-1),1);				//wyswietl 1F dla kotla jednofunkcyjnego
#else
            if(PFN.vUNI02<=2)
            {
                if(!PFNf_EnAleg) PrintVER(0x2F,0,1);	//wyswietl 2F dla kotla dwufunkcyjnego
...
{
	volatile static unsigned char obg_cw=0,obg_co=0,zmn2=0,pcw,inst=0;      //mmm
...
//-----------------
	if(!PFNf_set_nco&&!obg_cw&&PFNf_obgCW)	//uruchomienie w obiegu CW?
	{
		PFNf_str_ncw=1;
		PFNf_b_str_ncw=1;
        PFNf_str_nco=0;
...
//-----------------
	if(PFNf_set_nco)	//wybrano nastawe CO?
	{
        ...
        bfdta=(signed int)PFN.PCO;//kkk
        ReadParam(&bfdta,&zmn,&zps,PFN.minPCO,PFN.maxPCO,0,1);        //kkk
        PFN.PCO=(unsigned int)bfdta;//kkk        
#else
...
#endif        
#endif
		if(!PFNf_b_set_nco)	//stan po pierwszym nacisnieciu przycisku DW/UP (bez zmiany nastawy)?
		{
			PFNf_b_set_nco=1;
			EndNst();
			zmn2=1;
        ...
        //if(zmn)
		{
			...
			zmn2=0;
			StartRTdS(_RTSET);
            //if(!(PFNf_str_nco||PFNf_str_ncw))
            {
                PrintNstMXCO(PFN.PCO,0);
            }
		}
	}
//-----------------
...
//							}
//							else
                                if(PFNf_prn_pg) PrintPG(PFN.VNT);//PrintPG(PFN.PG);						//wylaczny wskaz PG
                                else
                                if(PFNf_prn_pp) PrintFL(PFN.PRZ);//PrintPP(PFN.PP);						//wylaczny wskaz PP
                                else
								if(PFNf_prn_in) PrintCo(PFN.rCO,PFNf_errCO);//PrintIN(PFN.IN);						//wylaczny wskaz IN                       
						}
						else 
						{
//                            if(PFNf_fL3) 
...
//							}
					...
					{
						if(PFNf_obgCO||PFNf_obgCW)													//aktywny obieg CO?
						{
							if(PFNf_prn_res)
							{
//								if(PFNf_prn_cs)
...
//								else
							...
							else														//wskaz CO
							{
                                ...
                                if(PFNf_obgCW)
                                {
                                    PrintGCO(PFN.PG, PFN.CO, 1, PFNf_errCO);
                                }
                                else
                                {
                                    PrintGCO(PFN.PG, PFN.CO, 0, PFNf_errCO);
                                }
//								if(!PFNf_errCO) 
//								{
//                                    PrintCO(PFN.CO,1,PFNf_fL3);							//wylaczny wskaz temp CO
////									if(!PFNf_EnAleg)
////									{
...
//                                //if(PFNf_obgCW) 
//								//wskaz temp CW
//                                //if(!(PFNf_ofnserw||PFNf_fnserw||PFNf_set_nco)) ClrDig(1,0,0);											//wygaszony wskaz WU
////								if(!PFNf_EnAleg)
////								{
...
//                                    if(!PFNf_errCO) 
//                                    {
//                                        PrintCo(PFN.rCO);//PrintIN(PFN.IN);						//wylaczny wskaz IN
//                                    }
//                                    else
//                                    {
//                                        PrintPG(PFN.PG);						//wylaczny wskaz PG
//                                    }
//                                }
//								else
//								if(PFNf_prn_pp) PrintPP(PFN.PRZ);						//wylaczny wskaz PP
//								else
//								if(PFNf_prn_pg) PrintPG(PFN.PG);						//wylaczny wskaz PG                                
//							}
//							else														//wskaz CW
...
//						}                        
				...
				{
					if(PFNf_ofnserw||PFNf_fnserw) 									//aktywna funkcja serwisowa?								
					{
                        //ClrDig(0,0,0);
						if(!PFNf_fshowLpulse) SetLightLLed(1); 												//rozswietlenie maksymalne
...
//                        }
                        ...
                        if(PFNf_fns_max)
                        {
                            PrintSRV(99, PFN.CO,PFNf_errCO);
                        }
                        else
                        {
                            PrintSRV(0, PFN.CO,PFNf_errCO);
                        }
					}
...
//    else
    ...
    if(PFNf_ofzima&&!PFNf_ofoff&&!PFNf_opKNF&&!PFNf_aktywne_LKNF)       //kkk    
    {
        if(PFNf_fL3)
        {
            fl=2;       //migajacy kaloryfer
        }
        else
        {
            fl=1;       //staly kaloryfer
        }
    }
    else 
    {
...
#endif
    //Mark(_GZ_DCW,(PFNf_plmCW||PFNf_obgCW)&&(PFNf_ofzima||PFNf_oflato)&&!PFNf_ofoff&&!PFNf_opKNF);
    //if((PFNf_plmCW||PFNf_obgCW)&&(PFNf_ofzima||PFNf_oflato)&&!PFNf_ofoff&&!PFNf_opKNF)
    if((PFNf_plmCW||PFNf_obgCW)&&(PFNf_ofzima||PFNf_oflato)&&!PFNf_ofoff&&!PFNf_opKNF&&!PFNf_aktywne_LKNF)    //kkk  
    {
        if(PFNf_LIN_UP&&PFNf_FLIN)
        {
            fl=2;       //zdalne pobudzenie - migajacy kaloryfer
        }
        else
        {
            fl=1;       //staly kaloryfer
    ...
    if(!PFNf_opKNF&&!PFNf_aktywne_LKNF)    //kkk
    {
        Mark(_GZ_OFF,PFNf_ofoff&&!PFNf_opKNF&&!PFNf_aktywne_LKNF&&!PFNf_fsetLaddr);					//znacznik OFF  //kkk
    }      
...
void DecWsw(void)
{
    ...
    if(!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF&&!PFNf_ferr)        //kkk    
	{
		...
		else
		{
			...
			if(PFNf_set_ncw)	//wybrano nastawe CW?
			{
                ...
                bfdta=(signed int)PFN.PCW;//kkk
                ReadParam(&bfdta,&zmn,&zps,PFN.minPCW,PFN.maxPCW,0,1);    //kkk
                PFN.PCW=(unsigned int)bfdta;//kkk
    ...
    Mark(_GZ_PGD,PFNf_pgd&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF);								//znacznik funkcja pogodowa    //kkk
	//Mark(_GZ_DCO,(PFNf_zima||(PFNf_lato&&PFNf_tdzCO)||(PFNf_off&&PFNf_tdzCO))&&!PFNf_aktywne_KNF);					//znacznik KALORYFER
    Mark(_GZ_DCO,(PFNf_zima||(PFNf_lato&&PFNf_tdzCO)||(PFNf_off&&PFNf_tdzCO))&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF);					//znacznik KALORYFER    //kkk
	//Mark(_GZ_DCW,(PFNf_zima||PFNf_lato||(PFNf_off&&PFNf_tdzCW))&&!PFNf_aktywne_KNF);								//znacznik KRAN
    Mark(_GZ_DCW,(PFNf_zima||PFNf_lato||(PFNf_off&&PFNf_tdzCW))&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF);            //kkk
    ...
    Mark(_GZ_OFF,PFNf_off&&!PFNf_aktywne_KNF&&!PFNf_aktywne_LKNF);		//znacznik OFF      //kkk
}
//********************************************************************************************************
#endif
//---------------------------------------------------------------
