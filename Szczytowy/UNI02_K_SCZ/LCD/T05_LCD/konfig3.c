...
const CDtPZK CPZK[_KNPAR]=
{
	...
	12,20,0b01100100,			//3 dolna granica predkosci obrotowej wentylatora
	25,95,0b01100100,			//4 gorna granica predkosci obrotowej wentylatora

	1 , 2,0b01000000,			//5 kod kotla

	0 , 1,0b01000100,			//6 rodzaj obiegu
	0 , 2,0b01000100,			//7 rodzaj ogrzewania (0-ogrzewanie tradycyjne, 1-ogrzewanie podlogowe, 2-pe³en zakres)
	0 , 1,0b01000100,			//8 wybor rodzaju czujnika cisnienia
	0 , 1,0b01000100,			//9 aktywacja zegara antylegionella

	1 , 4,0b01000100,			//10 tachometr - liczba okresow na sek.
	0 , 1,0b01000100,			//11 rodzaj pompy
	5 ,25,0b01010100,			//12 delta T dla pompy
	15,99,0b01001100,           //13 minimalny wydatek pompy
	0 , 1,0b01000100,			//14 aktywacja/deaktywacja trybu pracy ECO

	0 , 5,0b01000100,			//15 czas pracy przy obnizonej mocy w trybie CO/deaktywacja dla wartosci 0
	0 ,25,0b01001100,			//16 wartosc obnizonej mocy w trybie CO
	25,99,0b01001100,			//17 maksymalny wydatek pompy
	25,99,0b01001100,			//18 maksymalny wydatek pompy dla obiegu CW     

	0 ,15,0b01010100,			//19 histereza wylaczenia grzania CO (ponad nastawe)
	50,80,0b01010100,			//20 temp CO przy grzaniu CW zasobnika
	0 ,20,0b01010100,			//21 poprawka dla regulagtora pogodowego  
    0 , 2,0b01000100,			//22 wybor rodzaju wentylatora
    2 ,30,0b01000100,			//23 czas wybiegu na mocy startowej dla obiegu CO   
    1 ,60,0b01000100,			//24 czas blokady L3 dla obiegu CO     
    
    0 , 3,0b01000100,			//25 rodzaj regulatora pogodowego
    10,21,0b01010100,			//26 wartosc graniczna pobudzenia od regulatora pogodowego
    0 ,20,0b01010100,			//27 obnizenie nocne dla regulatora pogodowego  
    _MIN_PDCO ,_MAX_PCO,0b01010100,	//28 maksymalna wartosc graniczna CO
    0 , 1,0b01000100,			//29 histereza wylaczenia CW(2F)
    0 , 1,0b01000100,			//30 we. sygnalu timera zasobnika (1F)     
...
//---------------------------------------------------------
//Decyzja o dostepnosci parametru
unsigned char ChkAvailablePar(const unsigned char nr)
{
    if(nr>=_KNPAR) return 0;
    switch(nr)
    {
        case 5://5 kod kotla
        case 6://6 rodzaj obiegu
        case 8://8 wybor rodzaju czujnika cisnienia
        case 9://9 aktywacja zegara antylegionella
        case 11://11 rodzaj pompy    
        case 12://12 delta T dla pompy
        case 13://13 minimalny wydatek pompy
        case 14://14 aktywacja/dezaktywacja trybu ECO     
        case 17://17 maksymalny wydatek pompy
        case 18://18 maksymalny wydatek pompy dla obiegu CW    
        case 21://21 poprawka dla regulatora pogodowego  
        case 25://25 rodzaj regulatora pogodowego
        case 26://26 wartosc graniczna pobudzenia od regulatora pogodowego    
        case 27://27 obnizenie nocne dla regulatora pogodowego   
        case 29://29 histereza wylaczenia CW(2F)
        case 30://30 we. sygnalu timera zasobnika (1F)    
        case 31://31 przewyzszenie temperaturowe            
        {
            return 0;       //parametr niedostepny
        }
        default:
        {
            return 1;       //parametr dostepny
        }
    }
}
//------------------------------------------------------------------------------------
...
void DecShowHide(void)
{
    ...
    {   
        PZK[i].fl.war=CPZK[i].flwar;  
        if(ChkAvailablePar(i))            //parametr dostepny?
        {
            PZK[i].fl.shw=1;        //pokaz parametr
        }
        else
        {
            PZK[i].fl.shw=0;        //ukryj parametr
        }
    }
    //-----------------------------------------------
    //Zaktualizuj znaczniki zwiazane z ukrywaniem parametrow (0-ukryty,1-widoczny))
//    PZK[0].fl.shw=1;//0 moc startowa
//    PZK[1].fl.shw=1;//1 moc maksymalna CW
//    PZK[2].fl.shw=1;//2 moc maksymalna CO
//    PZK[3].fl.shw=1;//3 dolna granica predkosci obrotowej wentylatora
//    PZK[4].fl.shw=1;//4 gorna granica predkosci obrotowej wentylatora    
//    
//    PZK[5].fl.shw=0;//5 kod kotla
//    PZK[6].fl.shw=0;//6 rodzaj obiegu
//    PZK[7].fl.shw=1;//7 ogrzewanie podlogowe   
//    PZK[8].fl.shw=0;//8 wybor rodzaju czujnika cisnienia
//    PZK[9].fl.shw=0;//9 aktywacja zegara antylegionella
//    PZK[10].fl.shw=1;//10 tachometr - liczba okresow na sek.
//    
//    PZK[11].fl.shw=0;//11 rodzaj pompy
//    PZK[12].fl.shw=0;//12 delta T dla pompy
//    PZK[13].fl.shw=0;//13 minimalny wydatek pompy
//    PZK[14].fl.shw=0;//14 aktywacja/dezaktywacja trybu ECO
//    
//    PZK[15].fl.shw=1;//15 czas pracy przy obnizonej mocy w trybie CO/deaktywacja dla wartosci 0
//    PZK[16].fl.shw=1;//16 wartosc obnizonej mocy w trybie CO    
//    
//    PZK[17].fl.shw=0;//17 maksymalny wydatek pompy
//    PZK[18].fl.shw=0;//18 maksymalny wydatek pompy dla obiegu CW
//    
//    PZK[19].fl.shw=1;//19 histereza wylaczenia grzania CO (ponad nastawe)
//    PZK[20].fl.shw=1;//20 temp CO przy grzaniu CW zasobnika
//    
//    PZK[21].fl.shw=0;//21 poprawka dla regulatora pogodowego
//
//    PZK[22].fl.shw=1;//22 rodzaj wentylatora 
//    PZK[23].fl.shw=1;//23 czas wybiegu na mocy startowej w trakcie grzania CO 
//    PZK[24].fl.shw=1;//24 czas blokady L3 dla obiegu CO    
//    
//    PZK[25].fl.shw=0;//25 rodzaj regulatora pogodowego
//    PZK[26].fl.shw=0;//26 wartosc graniczna pobudzenia od regulatora pogodowego
//    PZK[27].fl.shw=0;//27 obnizenie nocne dla regulatora pogodowego
//    
//    PZK[28].fl.shw=1;//28 maksymalna wartosc graniczna CO dla grzania w oborgu CO (nastawa CO)
//    
//    PZK[29].fl.shw=0;//29 histereza wylaczenia CW(2F) 
//    PZK[30].fl.shw=0;//30 we. sygnalu timera zasobnika (1F) 
//    PZK[31].fl.shw=0;//31 przewyzszenie temperaturowe    
    
    //dynamiczna zmiana widocznosci parametru    
	//16 wartosc obnizonej mocy w trybie CO
    if(PZK[15].fl.shw&&PZK[16].fl.shw)      //parametry dostepne?
    {
        if(RSDT1.inTMMAXG2==0)				//15 czas pracy przy obnizonej mocy w trybie CO/deaktywacja dla wartosci 0
        {
            PZK[16].fl.shw=0;				//16 wartosc obnizonej mocy w trybie CO
        }
        else
        {
            PZK[16].fl.shw=1;				//16 wartosc obnizonej mocy w trybie CO
...
#if _KEY_BBOARD==0           
            kk=(PFNf_aktywne_LKNF||RdPrt(S_KUP))?1:0;            
			//key=PresKeyEX(RdPrt(S_OFF)&&RdPrt(S_SET),&ky[SS_OFF_SET]);
            key=PresKeyEX(RdPrt(S_OFF)&&RdPrt(S_SET)&&!kk,&ky[SS_OFF_SET]);     //kkk
#endif
#if _KEY_BBOARD==1       
            kk=(PFNf_aktywne_LKNF||RdPrt(S_KUP_CO))?1:0;
...
//				Mark(_GZ_TCO,PZK[DtKNF.PRM].fl.T);
                PrintWPR2(PZK[DtKNF.PRM].bfdta,PZK[DtKNF.PRM].fl.war);
