...
//        }
        ...
        if(!RdPrt(S_OFF)&&!RdPrt(S_SET)&&!PFNf_off&&!PFNf_fnserw&&!PFNf_aleg&&!PFNf_set_fnserw&&!PFNf_set_aleg&&!PFNf_set_nco&&!PFNf_set_ncw&&!PFNf_set_eco&&!PFNf_prODP&&!PFNf_obgCW)
        {
            key=PresKeyEX(RdPrt(S_KDW),&ky[SS_KDW]);
            key2=PresKeyEX(RdPrt(S_KUP),&ky[SS_KUP]);
            EnEco=0;
...
//            }	
            ...
            if((RTdS(_RTPCS)>=(tm/3))&&(RTdS(_RTPCS)<((2*tm)/3)))
            {
                PFNf_prn_cs=0;
                PFNf_prn_in=0;
                PFNf_prn_pp=1;
