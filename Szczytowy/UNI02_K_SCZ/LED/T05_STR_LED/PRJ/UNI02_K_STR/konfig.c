...
{
	_ADRMST, _KMINMST, _KMAXMST,1,				//0 moc startowa
	_ADRMCW, _KMINMCW, _KMAXMCW,1,				//1 moc maksymalna CW
	_ADRMCO, _KMINMCO, _KMAXMCO,1, 				//2 moc maksymalna CO
	_ADRMDG, _KMINVMD, _KMAXVMD,1,				//3 dolna granica predkosci obrotowej wentylatora
	_ADRMGG, _KMINVMG, _KMAXVMG,1,				//4 gorna granica predkosci obrotowej wentylatora

	_ADRWKK, _KMINKK,  _KMAXKK,0,					//5 kod kotla

	_ADROBG, _KMINOBG, _KMAXOBG,0,				//6 rodzaj obiegu
	_ADRPDG, _KMINOGP, _KMAXOGP,1,				//7 ogrzewanie podlogowe
	_ADRCS5, _KMINTPC, _KMAXTPC,0,				//8 wybor rodzaju czujnika cisnienia
	_ADRTAL, _KMINEAL, _KMAXEAL,0,				//9 aktywacja zegara antylegionella

	_ADRNOK, _KMINNOK, _KMAXNOK,1,				//10 tachometr - liczba okresow na sek.
	_ADRRDPM, _KMINRDP,_KMAXRDP,0,				//11 rodzaj pompy
	_ADRDLPM, _KMINDLP,_KMAXDLP,0,				//12 delta T dla pompy
	_ADRMNPM, _KMINMNP,_KMAXMNP,0,				//13 minimalny wydatek pompy
	_ADRTECO, _KMINTEC,_KMAXTEC,0,				//14 aktywacja/dezaktywacja trybu ECO
	_ADRTMCO2,_KMINTMCO2,_KMAXTMCO2,1,          //15 czas pracy przy obnizonej mocy w trybie CO/deaktywacja dla wartosci 0
	_ADRMCO2, _KMINMCO2,_KMAXMCO2,1,			//16 wartosc obnizonej mocy w trybie CO
	_ADRMXPM, _KMINMXP,_KMAXMXP,0,				//17 maksymalny wydatek pompy
	_ADRMXPM2,_KMINMXP2,_KMAXMXP2,0,			//18 maksymalny wydatek pompy dla obiegu CW

	_ADRHSCO,_KMINHSCO,_KMAXHSCO,1,				//19 histereza wylaczenia grzania CO (ponad nastawe)
	_ADRCONW,_KMINCONW,_KMAXCONW,1,				//20 temp CO przy grzaniu CW zasobnika
	_ADRPROP,_KMINPROP,_KMAXPROP,0,				//21 poprawka dla regulatora pogodowego
	_ADRRFAN,_KMINRFAN,_KMAXRFAN,1,				//22 rodzaj wentylatora    
	_ADRTSTR,_KMINTSTR,_KMAXTSTR,1,				//23 czas wybiegu na mocy startowej w trakcie grzania CO 
	_ADRTL3, _KMINTL3, _KMAXTL3,1,				//24 czas blokady L3 dla obiegu CO    
    
	_ADRPGDR, _KMINPGDR, _KMAXPGDR,0,			//25 rodzaj regulatora pogodowego
	_ADRPGDG, _KMINPGDG, _KMAXPGDG,0,			//26 wartosc graniczna pobudzenia od regulatora pogodowego
	_ADRPGDO, _KMINPGDO, _KMAXPGDO,0,			//27 obnizenie nocne dla regulatora pogodowego
	_ADRPGMX, _KMINPGMX, _KMAXPGMX,1,			//28 maksymalna wartosc graniczna CO dla regulatora pogodowego 
	_ADRCWMG, _KMINCWMG, _KMAXCWMG,0,			//29 histereza wylaczenia CW(2F) 
	_ADRCWTM, _KMINCWTM, _KMAXCWTM,0,			//30 we. sygnalu timera zasobnika (1F)  
	_ADRPWTM, _KMINPWTM, _KMAXPWTM,0			//31 przewyzszenie temperaturowe    
};
//-------------------------------------------------------------------------------
...
{
	volatile static unsigned char bf_AKTYWNE_KNF=0, bf_SAVE_PAR=0, wpar=0;
    volatile unsigned char mrg;
...
//	}	
	...
	if(DtKNF.KRK&&(RTM(_RTKONF3)>=_TKNFEXT2))			//uplynol maksymalny dozwolony czas aktywnosci procedury?
	{
		ExitKonf();
		return;
	}    
...
//		RSDT1.outKRK=DtKNF.KRK;								//informacja zwrotna o aktualnym kroku
		switch(DtKNF.KRK)
		{
            ...
            case 1:
            {
                ...
                {
                    DtKNF.PRM++;
                    if(DtKNF.PRM>9&&DtKNF.PRM!=_KNPAR1)
                    {
                        UpdatePR(DtKNF.PRM);
                    }
                    while(DtKNF.PRM<_KNPAR1&&!MinMax[DtKNF.PRM-1].shw)
                    {
                        DtKNF.PRM++;
                    }
                    if(DtKNF.PRM>_KNPAR1) DtKNF.PRM=1;
                }
                if(DtKNF.PRM<_KNPAR1) PrintPR(led_P,DtKNF.PRM,0);			//kod rx
                ...
                if(DtKNF.PRM!=_KNPAR1)										//parametr rx?
                {
                    if(PresKey1(RdPrt(S_KDW)))
                    {
                        ...
                        wpar=ReadEEPROM(MinMax[DtKNF.PRM-1].addr);
                    }
                }
                break;                
            }
...
//				break;	
//			}
			case 2: //tryb modyfikacji parametru
			{
				if(!DtKNF.PRM&&DtKNF.PRM>3)								//sterowanie wymuszeniami dla parametrow statycznych
				{
					DtKNF.wymCO=0;							//wylacz wymuszenie CO
				...
				switch(DtKNF.PRM)
				{
                    ...
                    case 1:
                    {
                        if(!ReadDataONL(RdPrt(S_KUP),&wpar,MinMax[DtKNF.PRM-1].min,MinMax[DtKNF.PRM-1].max,1,PresKey1(RdPrt(S_KDW))))
                        {
                            DtKNF.dmstr==wpar;
                            PrintWPR(wpar);
                        ...
                        {
                            DtKNF.wymCO=0;							//wylacz wymuszenie CW								//wylacz wymuszenie CO
                            if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,wpar);
                            DtKNF.mstr=wpar;
                            DtKNF.KRK=1;		
                        }
                        break;
                    ...
                    case 2:
                    {
                        if(!ReadDataONL(RdPrt(S_KUP),&wpar,MinMax[DtKNF.PRM-1].min,MinMax[DtKNF.PRM-1].max,1,PresKey1(RdPrt(S_KDW))))
                        {
                            DtKNF.dmxcw==wpar;
                            PrintWPR(wpar);
                            DtKNF.wymCO=0;						//zalacz wymuszenie CO
#if _ZR_SCZ==0                            
                    ...
                    case 3:
                    {
                        ...
                        else 
                        {
                            ...
                            if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,wpar);
                            DtKNF.mxco=wpar;
                            if(DtCO.MXCO>DtKNF.mxco)
                            {
                                DtCO.MXCO=DtKNF.mxco;
                                WriteEEMXCO();
                            }
                            DtCO.minMXCO=0;
                            DtCO.maxMXCO=DtKNF.mxco;                            
                            DtKNF.KRK=1;		
                        }
                        break;
                    }  
                    //predkosc minimalna wentylatora
                    case 4:
                    {
                        if(!ReadDataONL(RdPrt(S_KUP),&wpar,MinMax[DtKNF.PRM-1].min,MinMax[DtKNF.PRM-1].max,1,PresKey1(RdPrt(S_KDW))))
                        {
                        ...
                        {
                            mrg=ReadEEPROM(_ADRMGG);
                            if(wpar>mrg) wpar=mrg;
                            if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,wpar);
                            DtKNF.mmin=wpar;
                            ModParam();
                            DtKNF.KRK=1;		
                    ...
                    //predkosc maksymalna wentylatora
                    case 5:
                    {
                        if(!ReadDataONL(RdPrt(S_KUP),&wpar,MinMax[DtKNF.PRM-1].min,MinMax[DtKNF.PRM-1].max,1,PresKey1(RdPrt(S_KDW))))
                        {
                    ...
                    case 6:
                    {
                        ...
                        {
                            DtKNF.kodkt=wpar;
                            if(DtKNF.kodkt!=ReadEEPROM(MinMax[DtKNF.PRM-1].addr))		//zmieniono typ kotla
                            {
                                SetTpKtParam();							//ustaw param. ukryte dla wybr. kotla
                                ModParam();								//oblicz parametry kluczowe modulatora
                                if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,DtKNF.kodkt);
                            }                            
                            DtKNF.KRK=1;		
                        }
                        break;
                    ...
                    case 10:
                    {
                        ...
                        {
                            DtKNF.aleg=wpar;
                            if(DtKNF.aleg!=ReadEEPROM(MinMax[DtKNF.PRM-1].addr))
                            {
                                StartRTG(_RTANL);
                                if_WriteEEPROM(_ADRRTA,0);
                                if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,DtKNF.aleg);
                            }                         
                            DtKNF.KRK=1;		
                        }
                        break;
                    ...
                    case 23:
                    {
                        ...
                        {
                            DtKNF.rfan=wpar;
                            if(DtKNF.rfan!=ReadEEPROM(MinMax[DtKNF.PRM-1].addr))
                            {
                                if_WriteEEPROM(MinMax[DtKNF.PRM-1].addr,DtKNF.rfan);                          
...
void RdEEParam(void)
{
    ...
    M.RGDZ=GetRGDZ();
    DtCO.MXCO=ReadEEMXCO();
    if(DtCO.MXCO>DtKNF.def_mxco)
    {
        DtCO.MXCO=DtKNF.def_mxco;
        WriteEEMXCO();
    }
    DtCO.minMXCO=0;
    DtCO.maxMXCO=DtKNF.def_mxco;
}
/*---------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------*/
//Zapamietaj/odczytaj nastawe MXCO
unsigned char ReadEEMXCO(void)
{
	return ReadEEPROM(_ADRMXCO);
}
void WriteEEMXCO(void)
{   
    if_WriteEEPROM(_ADRMXCO,DtCO.MXCO);
}
/*---------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------*/
