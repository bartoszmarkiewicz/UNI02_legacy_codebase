...
//-------------
    ...
    if(RTdS(_RTMOD2)>1)
    {
        StartRTdS(_RTMOD2);
        NewPop_MIN(); 	//poprawka do wartosci granicznej minimalnej
        if((M._pob_KCO&&M._pob_KCO2)||(M._pob_CO&&RdPrt(S_UP_TMP)))			//obieg na zasobnik 
        {
            NewPop_MAX_CW(); //poprawka do wartosci granicznej maksymalnej
        }
        else
...
#else
    PWM.MINPWM=PWM.MDMOD;
    if(M._pob_CO&&RdPrt(S_UP_TMP))			//obieg na zasobnik    
    {
        PWM.MAXPWM=PWM.MMAXCW; 
    }
    else
...
unsigned char FirstXMOD(void)
{
	...
	//-------
	//ze wzgledu na wartosc docelowa (nastawe)
	switch(DtKNF.opdg)
    {
        case 0:             
        {  
        ...
        case 2:            
        {  
            if(M.CONTSETP>=_MIN_PDCO&&M.CONTSETP<=_MAX_PCO)					//wartosc docelowa w zakresie nastaw?
            {
                m=(signed long)(M.CONTSETP-_MIN_PDCO)*10;
                m=m/(signed long)(_MAX_PCO-_MIN_PDCO);
                m=m*(signed long)(maxmod-PWM.MINPWM);
                if(m<0) m=0;
                m=m/10+PWM.MINPWM;
                if(m>0xff)	p2=maxmod;										//przepelnienie 
