...
//------------------
	...
	if(M.ERR_BTY=ReadEEPROM(_ADRERR)) 		//uruchomienie po wylaczeniu awaryjnym?
	{
		if(DtKNF.robg==_OTWARTY) 			//obieg otwarty?
		{
			...
			{
				PrintErr(M.ERR_BTY,1,1);			//wyswietl kod bledu
...
//--------BEZWZGLEDNE WYLACZENIE BEZ BLOKADY
	...
	else
	{
		...
		if(M._RUSZ_POMPE)
		{
			...
			{
				if(M._prb_PRZ) PrintErr(0x05,0,0);
			}
			M._RUSZ_CISN=0;
			M._RUSZ_NCISN=0;
			M._RUSZ_NTC_CW=0;
...
//	if(M._AKTYWNE_SRW&&RdPrt(S_KDW)) M.SRV_DW=1;  
//	if(M._AKTYWNE_SRW&&RdPrt(S_KUP)) M.SRV_DW=0;    
	switch(M.SRV)
	{
		case 0:
		{
#if _NST_P3==0
			if(RdPrt(S_KUP)&&(DtCO.PCO==DtCO.maxPCO))
#else
            if(RdPrt(S_KUP)&&(DtCO.MXCO==DtCO.maxMXCO))
#endif
			{
				M.SRV=1;
				StartRTS(_RTSRV0);
	...
	switch(M.ROZ)
	{
		...
		case 3:
		{
			...
			if(RdPrt(S_PLM)&&!_GRZEJ_GetValue())			//obecny plomien mimo ze _GRZEJ=0?
			{
				...
				M.ERR_BTY=0x06;
				PrintErr(0x06,1,1);					//wyswietl kod bledu
                ToWriteESTAT();
				ErrPTG();
			}                    
#else
...
void SymulacjaPrzeplywuWG(void)
{
	...
	pmp=(M.POB_PMP)?1:0;    
#else
    pmp=((M._pob_KCO||M._pob_SRW||M._pob_CO))?1:0;    
#endif    
