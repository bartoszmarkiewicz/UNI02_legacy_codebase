...
{
    volatile float d;
    d=((float)PWM.BufPWM3/(float)PWM.HIPWM3)*100.0;
...
//Procentowy wydatek na modulatorze wzgledem zakresu dla wody uzytkowej
unsigned char ProcToMOD(unsigned char prc)
{
	unsigned int tmp;

	if(prc>PWM.GWMMAXCW) prc=PWM.GWMMAXCW;
	tmp=((PWM.MGMOD-PWM.MDMOD)*10)/PWM.GWMMAXCW;
	return PWM.MDMOD+(unsigned char)((prc*tmp)/10);    
}
//Procentowy wydatek na modulatorze wzgledem pelnego zakresu 
...
//gdzie tp-moc grzejnika, nr-dlugosc przewodu kominowego,ind-kolejny punkt charakterystyki
...
unsigned char GetPPWM(const unsigned char tp, const unsigned char nr,const unsigned char ind)
{
    ...
    switch(nr)
    {
        case 0:
        {
            if(!tp) return vnt_pwm0[ind].PPWM-pop;
            return Hvnt_pwm0[ind].PPWM-pop;
        }            
        case 1:            
        {
            if(!tp) return vnt_pwm1[ind].PPWM-pop;
            return Hvnt_pwm1[ind].PPWM-pop;            
        }
        case 2:
        {
            if(!tp) return vnt_pwm2[ind].PPWM-pop;
            return Hvnt_pwm2[ind].PPWM-pop;            
        }
        case 3:
        {
            if(!tp) return vnt_pwm3[ind].PPWM-pop;
            return Hvnt_pwm3[ind].PPWM-pop;            
        }
        case 4:
        {
            if(!tp) return vnt_pwm4[ind].PPWM-pop;
            return Hvnt_pwm4[ind].PPWM-pop;            
        }
        case 5:
        {
            if(!tp) return vnt_pwm5[ind].PPWM-pop;
            return Hvnt_pwm5[ind].PPWM-pop;            
        }    
        case 6:
        {
            if(!tp) return vnt_pwm6[ind].PPWM-pop;
            return Hvnt_pwm6[ind].PPWM-pop;            
        }     
        case 7:
        {
            if(!tp) return vnt_pwm7[ind].PPWM-pop;
            return Hvnt_pwm7[ind].PPWM-pop;            
        }        
        case 8:
        {
            if(!tp) return vnt_pwm8[ind].PPWM-pop;
            return Hvnt_pwm8[ind].PPWM-pop;            
        }         
        default:
        {
            if(!tp) return vnt_pwm8[ind].PPWM-pop;  
...
unsigned char VNTtoPPWM(unsigned char dvnt)
{
	...
	{
        Kvnt[i].dpwm=GetPPWM(DtKNF.pwr,DtKNF.lkm,i);
        Kvnt[i].dvnt=GetVNT(DtKNF.pwr,DtKNF.lkm,i);        
    } 
	for(i=0;i<_NVNTPWM;i++)
	{     
...
//                return Hpwm_vnt6p[ind].VNT;
            ...
            case 7:
            {
                switch(tp)
                {
                    case 0:
                    {
                        return Hpwm_vnt7[ind].VNT;
                    }
                    case 1:
                    {
                        return Hpwm_vnt7p[ind].VNT;
                    }
                    case 2:
                    {
                        return Hpwm_vnt7p2[ind].VNT;
                    } 
                    default:
                    {
                        return Hpwm_vnt7[ind].VNT;
...
unsigned char MPPWMtoVNT(unsigned char dpwm)
{
	...
	{
//        Kvnt[i].dpwm=MGetPPWM(PWM.tpGZ, DtKNF.lkm,i);DtKNF.rdgz
//        Kvnt[i].dvnt=MGetVNT(PWM.tpGZ, DtKNF.lkm,i);
        Kvnt[i].dpwm=MGetPPWM(DtKNF.rdgz, DtKNF.lkm,i);
        Kvnt[i].dvnt=MGetVNT(DtKNF.rdgz, DtKNF.lkm,i);        
    } 
	for(i=0;i<_NPWMVNT;i++)
	{     
		p2=(float)Kvnt[i].dpwm;
...
void VNTtoProc(const unsigned int pv)
{
    volatile unsigned char prc;
    volatile float dt;
...
{          
    volatile float prc,vpwm;
    volatile unsigned char mc,mv;
    ...
    if(_VNT_GetValue()&&!_GRZEJ_GetValue())         //zalaczony wentylator, wylaczona czesc zapalacza
    {
        ...
        StartPID(1);
        */
        if(M._ROZRUCH&&M._initVNT)       //praca wentylatora podczas w czasie wzbudzenia presostatu
            //if(!M._ROZRUCH||(M._ROZRUCH&&M._initVNT))
        {
            ...
            */
            prc=(float)VNTtoPPWM(30)/99.0;         //oczekiwana predkosc wentylatora na procent wypelnienia na wentylatorze
            vpwm=((float)PWM.HIPWM3)*prc;
            WritePWM3H((unsigned int)vpwm);
            pw2=VNTtoPPWM(30);            
            pw1=0;
        ...
        if(M._ROZRUCH)   //praca na mocy startowej w czasie proby osiagniecia oczekiwanej predkosci 
        {
#if _VPID==1
        ...
        else    //praca wentylatora w czasie wybiegu
        {
#if _VPID==1
            WritePWM3H(SetVNT(DtKNF.minv));
#else            
            //prc=(float)VNTtoPPWM(DtKNF.maxv)/99.0;         //oczekiwana predkosc wentylatora na procent wypelnienia na wentylatorze
            //vpwm=((float)PWM.HIPWM3)*prc;
            //WritePWM3H((unsigned int)vpwm);
            mc=VNTtoPPWM(DtKNF.maxv);
            ProcToVNT(mc);            
#endif            
            pw2=VNTtoPPWM(DtKNF.minv); 
            pw1=0;
        }
    }
    else
    if(_VNT_GetValue()&&_GRZEJ_GetValue())          //zalaczony wentylator, zalaczona czesc zapalacza
    {
...
#else                
                StartPID(0);
                mc=VNTtoPPWM(DtKNF.minv);
                ProcToVNT(mc);
#endif
            }
            else    //trwa rozruch, wentylator pracuje na mocy startowej
            {
                ...
                /*if(RdPrt(S_PLM)) 					//stwierdzono plomien?
                {
                    ...
                    WritePWM3H(SetVNT(DtKNF.strv+10));                    
                }
                else
                {
                    StartMProc(DtKNF.strv);                       
        ...
        else                                        //grzanie wody uzytkowej?
        {
            ...
            {
                //modyfikacja granicy minimalnej dla lagodzenia dynamicznej redukcji wydatku gazu po wyjsciu z procedury rozruchu
                if(RTdS(_RTMIP)<50)
                {
                    if(!PWM.tpGZ) 				//gaz ziemny
                    {
                        PWM.MDMOD=_MD_MOD1+10; //50
                    }
                    else
                    {
                        PWM.MDMOD=_MD_PMOD1+10; //50
                    }                  
                }
                else
                {
                ...
                //Po wyjsciu z procedury rozruchu czasowe utrzymanie wydatku na wentylatorze
                if(M._SVWB) //wybieg na podwyzszonej mocy startowej?
                {
                    StartPID(1);
                    if(!DtKNF.rdgz)
                    {
                        ...
                        if(DtKNF.pwr==1) //Grzejnik wysokiej mocy?
                        {
                            dt=20;//20
                            if(DtKNF.lkm>4)
                            {
...
//                            }                            
                        ...
                        if(dt>DtKNF.maxv) dt=DtKNF.maxv; 
                        StartMProc(dt);
                        pw2=VNTtoPPWM(dt);                    
                        pw1=MocMod(); 
                        mc=VNTtoPPWM(dt);
                    ...
                    else
                    {
                        ...
                        if(DtKNF.pwr==0) //Grzejnik niskiej mocy?
                        {
                            if((DtKNF.rdgz==1)||(DtKNF.rdgz==2)) //Gaz plynny propan-butan?
                            //if((DtKNF.rdgz==2)||(!DtKNF.lkm)) //Gaz plynny propan-butan?                            
                            {
                                dt=10;
                            }
                        }
 ...
 */
                ...
                else
                {
                    ...
                    pw1=MocMod();
                    mv=MPPWMtoVNT(pw1);
                    if(mv<DtKNF.minv) mv=DtKNF.minv;
                    if(mv>DtKNF.maxv) mv=DtKNF.maxv;
...
void StartPID(const unsigned char tp)
{
    ...
    if(tp)
    {
        ...
        //prc=(float)VNTtoPPWM(50)/99.0;         //oczekiwana predkosc wentylatora na procent wypelnienia na wentylatorze
        prc=(float)VNTtoPPWM(DtKNF.strv)/99.0;         //oczekiwana predkosc wentylatora na procent wypelnienia na wentylatorze
    }
    else
    {
...
//------------------------------------------------------------
void StartDM(const unsigned char wr)
{
    StartRTdS(_RTMOD2);
}
...
//Wartosc startowa PWM
//gdzie tp-rodzaj gazu, ln-dlugosc przewodu kominowego, pwr - moc podgrzewacza
unsigned char GetModSTR(const unsigned char tp, unsigned char ln, const unsigned char pwr)
{
    ...
    switch(tp)
    {
        case 0:
        {
            if(!pwr)
            {
                return r1_r6[ln].PPWM;
            }
            else
            {
                return Hr1_r6[ln].PPWM;        
            }            
        }
        case 1:
        {
            if(!pwr)
            {
                return r1_r6p[ln].PPWM;
            }
            else
            {
                return Hr1_r6p[ln].PPWM;        
            }            
        } 
        case 2:
        {
            if(!pwr)
            {
                return r1_r6p2[ln].PPWM;
            }
            else
            {
                return Hr1_r6p2[ln].PPWM;        
            }            
        }        
        default:
        {
...
unsigned char GetVntSTR(const unsigned char tp, unsigned char ln, const unsigned char pwr)
{
    ...
    switch(tp)
    {
        ...
        case 1:
        {
            if(!pwr)
            {
                return r1_r6p[ln].VNT;
            }
            else
            {
                return Hr1_r6p[ln].VNT;        
            }             
        }
        case 2:
        {
            if(!pwr)
            {
                return r1_r6p2[ln].VNT;
            }
            else
            {
                return Hr1_r6p2[ln].VNT;        
            }             
        }        
        default:
        {
