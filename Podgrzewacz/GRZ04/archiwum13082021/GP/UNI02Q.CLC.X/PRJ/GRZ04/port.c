...
#define	_NVPORT	12		/*liczba obslugiwanych portow/sygnalow*/
...
static const Opoznienie Nastawa[_NVPORT]={	
									...
									10,10,		//port nr 7 czas lo/hi oraz hi\lo
									2,2,		//port nr 8 czas lo/hi oraz hi\lo
									2,10,		//port nr 9 czas lo/hi oraz hi\lo
									10,4, 		//2,2port nr 10 czas lo/hi oraz hi\lo 
									30,4 		//2,2port nr 10 czas lo/hi oraz hi\lo                                            
								 };
/*---------------------------------------------------------------------------------*/
...
{
	//port 0
	SPort[0].Input=(_KY1_GetValue()) ? 0 : 1;							//_KEY port/sygnal poddany filtracji
	//port 1
	SPort[1].Input=(_KY2_GetValue()) ? 0 : 1;							//port/sygnal poddany filtracji
	//port 2
	...
	//port 9
	SPort[9].Input=(_CIG_NO_GetValue()) ? 1 : 0;								//port/sygnal poddany filtracji
	//port 10
	SPort[10].Input=(_PRZ_CO_GetValue()) ? 1 : 0;								//port/sygnal poddany filtracji    
	//port 11
	SPort[11].Input=(DtCW._cold_CW) ? 1 : 0;								//port/sygnal poddany filtracji    
	//...tak do Prt[NVPORT-1]
}
/*---------------------------------------------------------------------------------*/
