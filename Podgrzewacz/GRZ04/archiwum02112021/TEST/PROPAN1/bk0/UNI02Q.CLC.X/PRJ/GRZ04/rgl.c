...
void ModPrzCW(void)
{
	unsigned char p,m,v,maxmod,mdmod,i;
	signed int r,q;
		...
		//if(rgl.fz)					//faza 2 ?
		{
			if(!Tmaleje(0))			//(Tcw nie maleje ?)
			{
				if(Twyzsza(8))		//Tcw-Tnst>4 ?
				{
					...
					{
						{
							if(rgl.srp-15<PWM.MDMOD) rgl.srp=PWM.MDMOD;//15
							else rgl.srp=rgl.srp-15;
						}
					}*/
//#if _RNMOD==1
...
volatile unsigned char nmc;
void StartDMod(const unsigned char proc)
{
    StartRTdS(_RTDM1);
    nmc=proc;
}
void SetDMod()
{
    ...
    WriteOPWM(npwm);		//wymus minimum plomienia
}
//************************************************************
//------------------------------------------------------------
//------------------------------------------------------------
