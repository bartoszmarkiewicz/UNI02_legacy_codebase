...
static inline void InPWM6_LoadDutyValue(uint16_t dutyValue)
{
    ...
    {
        // Writing to 8 MSBs of PWM duty cycle in PWMDCH register
        PWM6DCH = (dutyValue & 0x03FE)>>1;

        // Writing to 2 LSBs of PWM duty cycle in PWMDCL register
        PWM6DCL = (dutyValue & 0x0001)<<8;
    }    
}
/*---------------------------------------------------------------------------------*/
/*-----------------------------PUBLIC DATA-----------------------------------------*/
...
#endif    
    InPWM6_LoadDutyValue((uint16_t)PWM.NewPWM3);
/*
#if _NEG_PWM2==1
