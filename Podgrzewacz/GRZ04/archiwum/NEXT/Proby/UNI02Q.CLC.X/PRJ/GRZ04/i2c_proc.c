...
void inline SetTData(void)
{
	...
	BufI2C.Bf[1]=pw2;//M.rVVNT%100;//pw2;//DtI2C.PCO;//M.LICZNIK_CIG3;//DtI2C.PCO;//M.LICZNIK_CIG3;
	BufI2C.Bf[2]=pw1;//M.rVVNT/100;//pw1;//DtI2C.CO;//M.LICZNIK_CIG;//DtI2C.CO;//M.LICZNIK_CIG;
	BufI2C.Bf[3]=rgl.nmod;//pw3;//rgl.nmod;//rgl._mdm;//rgl.nmod;//dt;//DtI2C.PCW;//RTS(_RTRSCI2)/10;//DtI2C.PCW;//RTS(_RTRSCI2)/10;
...
void inline FormatDataIMY(void)
{
    ...
    if(DtKNF.rdgz!=0)
        InBitSet(&DtI2C.ST_GLOBAL2,b_GP,1);
    else
        InBitSet(&DtI2C.ST_GLOBAL2,b_GP,0);
