...
extern DtRgl rgl;
extern StPin sp;
extern unsigned char pw1,pw2,pw3;
...
#if _DZ_CW==1        
        BlkDzCW();              //timer blokady dla pobudzenia od temp. dyzurnej
#endif        
#if _SRVTRB!=1
...
#if _DZ_CW==1
    if(!M._pob_KCW&&!M._pob_CW)     //brak tradycyjnych pobudzen
    {
        if(!M._blkDzCW)             //nie jest aktywna blokada dla pobudzenia od niskiej temperatury
        {
            if(RdPrt(S_COLD))       //temperatura <=5C ?
            {
                M._pob_DzCW=1;      //uaktywnij pobudzenie dyzurne
            }
            if(!M._ROZRUCH)         //gdy trwa rozruch to podtrzymaj pobudzenie do jego zakonczenia
            {
                if(!RdPrt(S_COLD))       //temperatura >6C ?
                {
                    M._pob_DzCW=0;      //dezaktywuj pobudzenie dyzurne
                }                
            }
        }
        else
...
void DecWSW(void)
{
    ...
    */
	...
	//if(_GRZEJ_GetValue()&&!RdPrt(S_PLM))          //zapobieganie migotaniu wyswietlacza w czasie iskrzenia
    if((M._ROZRUCH||_GRZEJ_GetValue())&&!RdPrt(S_PLM))          //zapobieganie migotaniu wyswietlacza w czasie iskrzenia
    {
        PrintOFF();  
        EndNst();        
    }
	else 
	{
        ...
        {
            EndOFF();        
            PrintNstCW(M.PRN_SRV,(M._AKTYWNE_SRW)?1:0);
        }       
	}
	//nastawa PCW
	//if(DtCW._newPCW) PrintNstCW(DtCW.PCW,(M._AKTYWNE_SRW)?1:0);
...
void DecSTR(void)
{
	...
	if(M._AKTYWNE_SRW&&!M._ROZRUCH&&_GRZEJ_GetValue()) 
    {
        //SetSrvPWM(0,99); 
        //SetSrvMod(0,99);
        if(M.SRV_DW)
        {
            //SetSrvMod(0,ProcToMOD2(00));
            M.PRN_SRV=SetSrvMod(0,M.SRV_HD,0);
        }
        else
        {
            //SetSrvMod(0,ProcToMOD2(99));
            M.PRN_SRV=SetSrvMod(0,M.SRV_HD,99);            
        }
    }
#endif
}
...
void DecRozruch(void)
{
	...
	if(!M._RUSZ_PLOMIEN&&!M._RUSZ_CIAG&&!M._RUSZ_STB&&!M._RUSZ_NTC_CW)
	{
		...
		if(M._pob_DzCW&&!M._blkDzCW)						//grzanie obiegu CW
		{
 				SetRozruch();					
		}	
#endif         
...
void DecWylacz(void)
{
	...
	if(M._ROZRUCH||_GRZEJ_GetValue())
	{
...
#else
		if(!M._pob_KCW&&!M._pob_CW&&!M._pob_DzCW) SetWylacz();
		else
		if(M._RUSZ_PLOMIEN||M._RUSZ_CIAG||M._RUSZ_STB||M._RUSZ_NTC_CW) SetWylacz();
		else
		if(M._pob_KCW)														//konfiguracja obieg CW
		...
		if(M._pob_DzCW)											//grzanie obiegu CW odtemp dyzurnej
		{
 			if(M._blkDzCW||DtCW._ad_CW) SetWylacz();						
		}    
#endif
	}        
}		
	...
	switch(M.SRV)
	{
		...
		case 1:
		{
			if(RdPrt(S_KUP)&&(DtCW.PCW==F1_MAX_PCW))
			{
                ...
                M.SRV_DW=0;
                M.SRV_HD=0;
                //SetSrvMod(1, ProcToMOD2(99));
                M.PRN_SRV=SetSrvMod(1,0, 99);
                
				StartRTM(_RTSRV1);
				StartRTS(_RTSRV0);
				//PrintNstCW(DtCW.PCW,1);
                PrintNstCW(M.PRN_SRV,1);
			}
			else
			{
				M.SRV=0;
		...
		case 3:
		{
			...
			{
				if((RTS(_RTSRV0)>=_TSRV0)&&RdPrt(S_KUP))
                {
                    //PrintNstCW(DtCW.PCW,1);
                    PrintNstCW(M.PRN_SRV,1);
                }
		...
		case 5:								
		{
			...
			if(M._TURBO&&!RdPrt(S_CIG_NO_1S))		//nieobecny sygnal ciagu?
			{
                ...
                if(M._pob_DzCW) 
                {
                    M._blkDzCW=1;
                    M._pob_DzCW=0;
                    StartRTM(_RTBLCW);       //aktywacja timera blokady dla pobudzenia od temp. dyz.
                }
				return;
			}
#endif
            ...
            if(M._pob_DzCW)                 //pobudzenie od niskiej temperatury?
            {
                if(RTS(_RTROZ)<_TDZCW) return;   //5 sek. praca i wylaczenie z blokada grzania od temp.dyz.
                M._blkDzCW=1;
                M._pob_DzCW=0;
                StartRTM(_RTBLCW);       //aktywacja timera blokady dla pobudzenia od temp. dyz.
            }
