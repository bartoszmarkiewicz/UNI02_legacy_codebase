...
unsigned char GetNPrc(unsigned char wr)
{  
    if(!DtKNF.rdgz)
    {
        if(RTdS(_RTDM0)<30)         //czasowa praca na mocy NMOD
...
void SetPWM_CW(void)
{
    volatile unsigned char mc,prc,mprc;
    volatile unsigned char dt;
	if(!rgl.nmod)					//nie jest w trybie nmod?
	{
		//WriteOPWM(rgl.tmp_pwm);
    ...
    {
        mprc=MocMod();            //procentowy stan modulatora
//        prc=GetNPrc(_STR_NPRC);
//        if(mprc<_STR_NPRC)
//        {
//            prc=mprc;
//        }
...
void SetMod(unsigned char ref, unsigned char npwm)
{
    volatile unsigned char v,nv,m;
    volatile static unsigned char dpwm;
    ...
    v=M.VVNT;              //wartosc aktualna predkosci wentylatora
    nv=MPPWMtoVNT(m);      //wartosc oczekiwana predkosci wentylatora
    if(nv>DtKNF.maxv) nv=DtKNF.maxv;
    if(nv<DtKNF.minv) nv=DtKNF.minv;
...
*/
//********************************************************************************
/*void SetSrvMod(unsigned char ref, unsigned char npwm)
{
    volatile unsigned char v,nv,m;
    volatile static unsigned char dpwm;
    ...
    else
    {
        ...
        {
            StartRTdS(_RTDM2);
            if(dpwm&&dpwm>npwm) dpwm--;
            if(dpwm<99&&dpwm<npwm) dpwm++;              
        }
        //WriteOPWM(dpwm);
        ProcToMOD2(dpwm);
    }
}*/
//volatile unsigned char nprc2;
//unsigned char GetSrvNPrc(unsigned char ref, unsigned char wr, unsigned char dw)
//{  
//    if(ref)
//    {
//        StartRTdS(_RTDM1);
...
//            if(dw)
//            {
//                if(nprc2>0) nprc2--;      //stopniowe obnizenie wartosci procentowej
//            }
//            else
//            {
//                if(nprc2<99) nprc2++;      //stopniowe obnizenie wartosci procentowej                
//            }
//                    
//        }
//    }
...
//    return nprc2;
//}
//void SetSrvPWM(const unsigned char ref, const unsigned char dw)
//{
//    volatile unsigned char v,nv,m;
//    volatile unsigned char prc,mprc;
//	if(ref)					//nie jest w trybie nmod?
//	{
//        if(dw)
//        {
//            SetSrvMod(1, 0);         
...
unsigned char SetSrvMod(unsigned char ref, unsigned char hold, unsigned char npwm)
{
    //volatile unsigned char v,nv,m;
    volatile static unsigned char dpwm;
