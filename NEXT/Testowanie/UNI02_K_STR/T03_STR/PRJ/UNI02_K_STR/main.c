...
//        RSDT1.outRDZ2=wr;
    ...
    RSDT1.outPRZ=(unsigned char)PrCW.przCW;                                  //przeplyw cw
    //------------------------------------------
...
void DecSTR(void)
{
...
#endif
    {
        if(!M._ZMKCOCW)
        {
            ...
            if(M._PRACA&&!M._BLOKUJ_15S&&(M._pob_KCO||M._pob_SRW||M._pob_UP||M._pob_CO||M._pob_DzCO)) ToObiegCO();
            else
			...
			switch(M.WYL)
			{
				...
				{	
					...
					else			//obieg CO
					{
                        if(!M._pob_UP)
                        {
                            if(RTS(_RTWYL)<DtKNF.tmpm) return;  				//uplynal maksymalny wybieg pompy?
                            M.WYL=2;
                        }
                        else
                        {
				...
				case 2:
				{
					if(!M._RPM_CW)	//obieg CO
					{
                        ...
                        if(M._pob_UP)
                        {
                            if(!RdPrt(S_ERR_CO)&&!DtCO._su_UP) return; 		//osiagnieto wartosc wylaczenia po pracy w ukladzie CO? 
                        }                        
					}
					else
					{
