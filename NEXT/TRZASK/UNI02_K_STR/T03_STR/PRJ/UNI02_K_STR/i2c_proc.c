...
void inline SetTData(void)
{
    ...
    */
	...
	BufI2C.Bf[3]=PMP.APROC;//DtI2C.PCW;//(M.rVVNT)%100;//dt;//DtI2C.PCW;//Kvnt[0].dpwm%100;//DtI2C.PCW;//((!((M.FL3.BT3._AKTYWNE_PGD2)?1:0))&&(((_RT_GetValue())?1:0)||((M.FL5.BT5._LIN_RT)?1:0)||((M.FL6.BT6._I10V_RT)?1:0)||((M.FL7.BT7._IOT_RT)?1:0)));//DtZW.ZW;//DtI2C.PCW;//RSDT1.inCONTSETP;//M.CONTSETP;//DtI2C.PCW;//M.CONTSETP%100;//DtI2C.PCW;//PWM.BufPWM%100;//DtI2C.PCW;//DtCO.HstCo;//DtI2C.PCW;//mmm%100;//(vvvv>=0)?vvvv:0;//DtI2C.PCW;//PMP.APROC;//DtI2C.PCW;
	BufI2C.Bf[4]=(M._LOW_PMP)?1:0;//DtI2C.CW;//(M.rVVNT)/100;//DtI2C.CW;//Kvnt[0].dpwm/100;//DtI2C.CW;//RSDT1.inECO;//M.FL1.BT1._pob_PGDRT;//DtI2C.CW;//DtCO.PGDPCO;//DtI2C.CW;//PWM.BufPWM/100;//DtI2C.CW;//(DtCO.FL0.BT0._ad_CO)?1:0;//DtI2C.CW;//mmm/100;//(vvvv<=0)?(-1)*vvvv:0;//DtI2C.CW;//DtIN.DIN;//DtI2C.CW;
