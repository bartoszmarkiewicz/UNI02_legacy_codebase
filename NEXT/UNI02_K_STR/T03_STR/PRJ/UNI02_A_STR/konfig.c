...
DtMINMAX MinMax[_KNPAR1]=
{
    ...
    _ADRPWTM, _KMINPWTM, _KMAXPWTM,				//21 przewyzszenie temperaturowe
};
//-------------------------------------------------------------------------------
...
unsigned char RefKnfPar(void)
{
	if(RSDT1.RD_KNF)
	{
		...
		switch(RSDT1.vKNF)
		{
            case 6:                                            //(31)dziesiata wersja paczki danych plyty LCD  
...
unsigned char SaveKnfPar(void)
{
	if(RefKnfPar())
	{
        ...
        if((DtKNF.pwtm>MinMax[21].min)&&(DtKNF.pwtm<MinMax[21].max)||DtKNF.pwtm==MinMax[21].min||DtKNF.pwtm==MinMax[21].max)
        {
