...
unsigned char LIN1SingleResponse_RD(void)
{
    ...
    {      
        ...
        case _CLIN_RRD_SETP2:                                   //informacja zwrotna pobudzen i wartosci oczekiwanych (na potrzeby BYPASS)
        {
            FTL1.PCI_LENGTH=0x02;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40
            FTL1.WRD0.byte1=PFN.CONTSETP&0b01111111;            //odeslij wartosc oczekiwana
            if(FTL1.WRD0.byte1)
            {
                if(PFN._FOUTRT)
                {
