...
{          
    ...
    if(_VNT_GetValue()&&_GRZEJ_GetValue())          //zalaczony wentylator, zalaczona czesc zapalacza
    {
...
#else
            WritePWM3((unsigned char)SetVNT(DtKNF.strv));            
#endif
        ...
        else                                        //grzanie wody uzytkowej?
        {
            ...
            //ProcToVNT(MocModToVNT());               //predkosc proporcjonalna wentylatora
#if _HIRESVNT==1            
            WritePWM3H(MProcToVNT());                //predkosc wentylatora zalezna od wydatku na modulatorze zaworu gazowego
#else
            WritePWM3((unsigned char)MProcToVNT());                //predkosc wentylatora zalezna od wydatku na modulatorze zaworu gazowego            
#endif
            //pw2=VNTtoPPWM(M.VVNT);
            pw1=MocMod();
            pw2=VNTtoPPWM(MPPWMtoVNT(pw1));
            
#else
            //ProcToVNT(DtKNF.maxv);                  //predkosc zadana wentylatora
#if _HIRESVNT==1            
            WritePWM3H(SetVNT(DtKNF.maxv));
#else
            WritePWM3((unsigned char)SetVNT(DtKNF.maxv));            
#endif
