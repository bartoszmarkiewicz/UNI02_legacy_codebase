...
void InitUart(void)
{
	...
	RCSTA1bits.CREN=1;			//aktywacja odbioru
	U1OD=0;
...
{
	if(RCSTA1bits.ADDEN&&RCSTA1bits.RX9D)							//RX9D - ustawiony 9 ty bit? (znacznik adresu)
	{
		RSerial=RCREG1;
		if(RSerial==_MY_ADDR)				//parawidlowy adres?
		{
			//if(zzz++>99) zzz=0;
			RCSTA1bits.ADDEN=0;						//wylacz detekcje adresu
	...
	if (UADt.indeks<_NRS_RBF)				//mozliwe przepelnienienie bufora odbioru?
	{
		...
		if(VerifyNDatCHK(UADt.RNData))		//kontroluj poprawnosc danej
		{
			...
			{	
				RCSTA1bits.ADDEN=1;						//zalacz detekcje adresu
		...
		{
			RCSTA1bits.ADDEN=1;							//uaktywnij detekcje adresu
		 	UADtf_RData=0;						//sygnalizuj deaktywacje odbioru danych
...
{
	if(!UADt.indeks) TXSTA1bits.TX9D=1;		//dla zerowego indeksu ustaw znacznik adresu
	else TXSTA1bits.TX9D=0;					//dla pozostalych danych usun znacznik adresu
	if(UADt.TNData>_NRS_TBF) UADt.TNData=_NRS_TBF;
	if(UADt.indeks>_NRS_TBF-1) UADt.indeks=_NRS_TBF-1;    
	TXREG1=bufTUART[UADt.indeks++];	//rozpoczyna transmisje
	...
	else
	{
		...
		PIE1bits.TX1IE=0;						//deaktywacja transmisji
	}
}	
//------------------------------------------------------------------------------------
//Aktywacja modulu odbioru danych
...
{
	if(PIE1bits.TX1IE) return;				//trwa transmisja danych?
	...
	PIE1bits.TX1IE=1;							//aktywacja transmisji
}
