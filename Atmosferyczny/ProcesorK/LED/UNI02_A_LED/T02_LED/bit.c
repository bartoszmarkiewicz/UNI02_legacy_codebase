...
//do wywolania w petli glownej

bit GetBit2(const unsigned char data, const unsigned char nbit)
{
    if(data & (1 << nbit)) return 1;
    return 0;
}
...
void BitSet3(volatile unsigned char *data, const unsigned char nbit, const unsigned char bt)
{
    if(bt) *data |= 1 << nbit;
    else *data &= ~(1 << nbit);
}
...
void BitSet2(unsigned char *data, const unsigned char nbit, const unsigned char bt)
{
    if(bt) *data |= 1 << nbit;
    else *data &= ~(1 << nbit);
}
