/*---------------------------------------------------------------------------------*/
/*wsw.c*/
/*obsluga 2 cyfrowego wyswietlacza typu LED z czterema diodami sygnalizacyjnymi*/
...
volatile static bit mux; //znacznik aktywnej cyfry
volatile static unsigned char wbty[2]; //bajty rejestrow szeregowych wyswietlacza
volatile static unsigned char wsdid; //slowo wysterowania kolenych diod sygnalizacyjnych
...
void FormatWswData(void)
{
        ...
        if(WswData.St.wbdid && (WswData.St.sdiode || WswData.St.rdiode))
    {
        ...
        if(WswData.St.sdiode) Swbty(WswData.St.wbdid); //zapal wybrane diody
        else
            if(WswData.St.rdiode) Rwbty(WswData.St.wbdid); //wygas wybrane diody
    }
    else
    {
        //zastosuj ogolne slowo wysterowania diod
...
void PrintWswData(void)
{
...
#endif
#else
    //Interfejs szeregowy (wysylanie bajtow do rejestrow bit po bicie.)
    mux = !mux;
#if WSWAP==0							//zamienic cyfry miejscami?
    if(mux)
    ...
    if(mux)
        ...
        wbty[0] = LedData.dLed0;
#endif
    if(mux) wbty[NWBTY - 1] |= (1 << SMUX);
    else wbty[NWBTY - 1] &= ~(1 << SMUX);
    for(k = 0; k < NWBTY; k++)
    {
        for(j = 0; j < NRBIT; j++)
        {
            _DTA = (wbty[NWBTY - 1 - k]&(1 << (NRBIT - 1 - j))) ? 1 : 0;
...
void SetData(volatile WPack *wData, const unsigned char wdt1, const unsigned char wdt0)
{
    (*wData).Dt.Data1 = wdt1;
    (*wData).Dt.Data0 = wdt0;
}
...
void SetStatus(volatile WPack *wData, const unsigned int wst, const unsigned int mask)
{
