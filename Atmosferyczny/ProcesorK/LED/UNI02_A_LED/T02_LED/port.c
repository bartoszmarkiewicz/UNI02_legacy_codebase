...
#define	_NVPORT	26		/*liczba obslugiwanych portow/sygnalow*/
...
static const Opoznienie Nastawa[_NVPORT] = {
    ...
    2, 10, //port nr 21 czas lo/hi oraz hi\lo
    1, 1, //port nr 22 czas lo/hi oraz hi\lo
    1, 1, //port nr 23 czas lo/hi oraz hi\lo
    0, 0, //port nr 24 czas lo/hi oraz hi\lo
    0, 0 //port nr 25 czas lo/hi oraz hi\lo
};
/*---------------------------------------------------------------------------------*/
...
{
    //port 0
    SPort[0].Input = 0; //_KEY port/sygnal poddany filtracji
    //port 1
    SPort[1].Input = (_PRZ_CO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 2
    ...
    //port 6
    SPort[6].Input = 0; //port/sygnal poddany filtracji
    //port 7
    SPort[7].Input = 0; //port/sygnal poddany filtracji
    //port 8
    SPort[8].Input = (_MOD_ON) ? 0 : 1; //port/sygnal poddany filtracji
    //port 9
    SPort[9].Input = (DtCOf_on_DZ_CO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 10
    SPort[10].Input = (DtCWf_on_DZ_CW) ? 1 : 0; //port/sygnal poddany filtracji
    //port 11
    SPort[11].Input = (_PRZ_CW) ? 0 : 1; //port/sygnal poddany filtracji
    //port 12
    SPort[12].Input = (_RT) ? 1 : 0; //port/sygnal poddany filtracji
    //port 13
    SPort[13].Input = (DtCOf_cold_CO || DtCOf_hot_CO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 14
    SPort[14].Input = (DtCSf_error) ? 1 : 0; //port/sygnal poddany filtracji
    //port 15
    SPort[15].Input = (DtCSf_work) ? 1 : 0; //port/sygnal poddany filtracji
    //port 16
    SPort[16].Input = (DtCSf_lo) ? 1 : 0; //port/sygnal poddany filtracji
    //port 17
    SPort[17].Input = (DtCWf_cold_CW) ? 1 : 0; //port/sygnal poddany filtracji
    //port 18
    SPort[18].Input = (DtCOf_err_CO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 19
    SPort[19].Input = (DtCOf_ad_CO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 20
    SPort[20].Input = (DtZWf_on_ZW) ? 1 : 0; //port/sygnal poddany filtracji
    //port 21
    SPort[21].Input = (_CIG_NO) ? 1 : 0; //port/sygnal poddany filtracji
    //port 22
    SPort[22].Input = (_OFF) ? 0 : 1; //port/sygnal poddany filtracji
    //port 23
    SPort[23].Input = (_SET) ? 0 : 1; //port/sygnal poddany filtracji
    //port 24
    SPort[24].Input = (_KUP) ? 0 : 1; //port/sygnal poddany filtracji
    //port 25
    SPort[25].Input = (_KDW) ? 0 : 1; //port/sygnal poddany filtracji
    //...tak do Prt[NVPORT-1]
}
/*---------------------------------------------------------------------------------*/
