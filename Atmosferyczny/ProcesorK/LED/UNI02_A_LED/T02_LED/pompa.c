...
unsigned char ModPmp1(unsigned char p)
{
    ...
    if(rglf_fz || ppwm != PMP.MGPROC) //aktywny modulator pomy i faza 2 modulacji?
    {
        ...
        if(DtIN.DIN2 > DtIN.DDLT)
        {
            //tim=50;
            tim = 100;
            if(DtIN.DIN2 - DtIN.DDLT == 2) tim = 120; //tim=100;
            else
                if(DtIN.DIN2 - DtIN.DDLT == 1) tim = 150; //tim=150;
        }
        else
        {
            //tim=80;
            tim = 120;
            if(DtIN.DDLT - DtIN.DIN2 == 2) tim = 150; //tim=100;
            else
                if(DtIN.DDLT - DtIN.DIN2 == 1) tim = 180; //tim=150;
        }
        pup = 0;
        pdw = 0;
...
void RglPmp1(void)
{
    ...
    if(!Mf_PRACA)
    {
        ...
        if(_POMPA)
        {
            PMP.APROC = PMP.MGPROC; //pelny wybieg
        }
        else
        {
    ...
    {
        if(Mf_MODULUJ && !DtINf_err_IN && !Mf_RPM_CW && RTS(_RTMPMP) > _TPMPBLK) //obieg CO, po rozruchu po ustalonym czasie rozpocznij modulacje pompa
        {
            if(rglf_fz)		//dla fazy 1 narost lub spadek obrotow w rgl.c (IncMaxCO())
			{
...
unsigned char ModPmp2(unsigned char p)
{
    ...
    if(DtIN.PPEco)
    {
        DtIN.HI = RetHI(DtCO.PPCO, DtIN.PPEco - 1, _MAXT); //dlona granica IN dla parametru ECO i nastawy PPCO
        DtIN.LO = RetLO(DtCO.PPCO, DtIN.PPEco - 1, _MINT); //gorna granica IN dla parametru ECO i nastawy PPCO
        DtIN.delta = RetDelta(DtCO.PPCO, DtIN.PPEco - 1); //wartosc delta dla parametru ECO i nastawy PPCO
    }
    else
    {
        DtIN.HI = 0;
    ...
    if(rglf_fz || ppwm != PMP.MGPROC) //aktywny modulator pompy i faza 2 modulacji?
    {
...
void PmpToUP(void)
{
    ...
    //else PMP.APROC = PMP.MGPROC;   
}
//Funkcja wywolywana w proc regulacji przy naroscie mocy (FAZA1)
void PmpToDW(void)
{
    static unsigned char tm;
...
void RglPmp(void)
{
    if(DtKNF.rdpm) //zdecyduj ze wzgledu na rodzaj pompy
    {
        if(!DtKNF.teco)
        {
    ...
    else //minimalizuj straty energii/zapobiegnij postojowi w przypadku pomylki konfiguracyjnej
    {
        if(!Mf_PRACA)
        {
            ...
            if(_POMPA)
            {
                PMP.APROC = 15; //wybieg z predkoscia bezpieczenstwa
            }
            else
            {
                PMP.APROC = 0; //minimalizuj straty energii
