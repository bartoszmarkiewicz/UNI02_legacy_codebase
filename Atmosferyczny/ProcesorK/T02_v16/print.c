...
/*
void PrintErrTm(const unsigned char kod,const unsigned char tm, const unsigned char pls)
{
	SetData(&DWD[0],0,kod);
	SetData(&DWD[1],0,tm);
	SetStatus(&DWD[0],tHEX|tSDID|_DKM,tALL);					//kod bledu/cisnienie
	if(pls)	SetStatus(&DWD[1],tDEC|tRDID|_DKM,tALL);				//cisnienie			
	else SetStatus(&DWD[1],tDEC|tSDID|_DKM,tALL);	
	SetPoz(_WS_ERR,0);
	SetPoz(_WS_ERR,1);
}
*/
...
void PrintNstCW(const unsigned char nst)
{
	...
	SetStatus(&DWD[3],tDEC|tSDID|_DCW,tALL);
	SetStatus(&DWD[4],tDEC|tRDID|_DCW,tALL);
	RestartKol(_WS_NCWCO);
}
//wyswietla nastawe temp. CO
void PrintNstCO(const unsigned char nst)
{
	SetData(&DWD[3],0,nst);							//nastawa CO
	SetData(&DWD[4],0,nst);							//nastawa CO
	SetStatus(&DWD[3],tDEC|tSDID|_DCO,tALL);
	SetStatus(&DWD[4],tDEC|tRDID|_DCO,tALL);
	RestartKol(_WS_NCWCO);
}
//wyswietla nastawe Kt
void PrintNstKt(const unsigned char nst)
{
	SetData(&DWD[3],0,nst);							//nastawa Kt
	SetData(&DWD[4],0,nst);							//nastawa Kt
	SetStatus(&DWD[3],tDEC|tKROPKA1|tSDID|_DCO,tALL);
	SetStatus(&DWD[4],tDEC|tKROPKA1|tRDID|_DCO,tALL);
	RestartKol(_WS_NCWCO);
}
...
{
	SetStatus(&PWD[_WS_TCW],tDEC|tFOCUS,tALL);
