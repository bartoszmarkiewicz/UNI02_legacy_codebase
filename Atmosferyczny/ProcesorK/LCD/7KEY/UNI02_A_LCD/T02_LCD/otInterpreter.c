...
void OTInterpreter(void)
{
    ...
    {
        if(MGSDf_NO_ODB||!PFN.vUNI02||(PFNf_LIN_KSK&&PFNf_FLIN)||(PFNf_LIN_STR&&PFNf_FLIN))    //brak polaczenia z UNI-02 lub wersja oprogramowania UNI-02<09 lub aktywny interfejs LIN w trybie sterowania?  
        {
            OTMaster.Ref.war=0;
    ...
    */
    ...
    switch(OT.RFrame.data_id)
    {
        ...
        case 1:     //Control Setpoint f8.8
        {
            ...
            if((OTMaster.Control_Setpoint>=PFN.SETPmin)&&(OTMaster.Control_Setpoint<=PFN.SETPmax))   //dolna granica =minPCO, gorna granica =PCO
            {
                PFN.CONTSETP=(unsigned char) OTMaster.Control_Setpoint;
            }
            else
            if(OTMaster.Control_Setpoint<PFN.SETPmin)    
            {
