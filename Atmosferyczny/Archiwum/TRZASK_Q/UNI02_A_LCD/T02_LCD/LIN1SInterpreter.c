...
unsigned char LIN1SingleResponse_WR(void)
{
    ...
    switch(FRL1.IDN)                        //identyfikator komendy
    {
        ...
        {
            if((PFNf_LIN_STR&&PFNf_FLIN)||(PFNf_LIN_KSK&&PFNf_FLIN))    //LIN w trybie pracy sterowanie?
            {
            ...
            /*if((PFNf_LIN_STR&&PFNf_FLIN)||(PFNf_LIN_KSK&&PFNf_FLIN))    //LIN w trybie pracy sterowanie?
            {
                ...
                else PFN.CONTSETP=FRL1.WRD1.byte0;
            }
            else return _LIN1_ERRODP;*/   
            break;
        }   
...
unsigned char LIN1SingleResponse_RD(void)
{
    ...
    {      
        case _CLIN_RRD_ACK:                 //wygeneruj potwierdzenie w odpowiedzi na wywolanie adresem
        {
            ...
            FTL1.WRD0.byte1=_LIN_OID;                           //identyfikator odbiornika (0x02 - identyfikator kotla atmosferycznego)
            //*******************************
            ...
            //*******************************
            //data2: aktualna konfiguracja sterownika
             //(0-Unico,1-miniterm,2-maxiterm,3-minimax plus,4-nieznany) 
        ...
        case _CLIN_RRD_TRB:                 //wygeneruj tryb pracy
        {
            FTL1.PCI_LENGTH=0x04;                               //Rozmiar ramki(RSID+liczba bajtow danych bez CHK)
            FTL1.WRD0.byte0=0xB2+0x40;                          //RSID=SID+0x40
            if(PFNf_off)
                ...
                case 3:         //3.Przygotowanie danych dla KSK
                {
                    ...
                    if(!FRL1f_NextFr)                                                    //w trakcie wysylki do UNI-02 nie bylo aktualizacji ramki LIN?
                    {
                        ...
                        if(RSDT1.inLDTA1<GetKNPAR())
                        {
                            ...
                            FTL1.WRD2.byte0=0;
                            /*if(PZK[RSDT1.inLDTA1].fl.T) FTL1.WRD2.byte0=1;
                            if(PZK[RSDT1.inLDTA1].fl.P) FTL1.WRD2.byte0=2;*/ 
        ...
        case _CLIN_RRD_INT:                                     //wygeneruj informacje zbiorcza o aktywnosci interfejsach 10V,OT,LIN
        {
            ...
            if((PFNf_LIN_KSK&&PFNf_FLIN)||(PFNf_LIN_STR&&PFNf_FLIN))
            {
...
//------------------------------------------------------------
//******************************
void ifInitAutoMFrame(const unsigned char nr)
{
    static unsigned char pchk[10];
    unsigned char chk;
...
unsigned char LIN1StartMultiResponse_RD(void)
{
    ...
    switch(FRL1.IDN)                        //identyfikator komendy
    {
        ...
        {          
            ...
            tbuf[8]=RSDT1.outCS;
            tbuf[9]=RSDT1.outIN;
            tbuf[10]=(unsigned char)PFN.PCO;//RSDT1.outPCO;//**********************
            tbuf[11]=(unsigned char)PFN.PCW;//RSDT1.outPCW;//**********************
            tbuf[12]=RSDT1.outPPCO;
            tbuf[13]=RSDT1.outPPCW;
            tbuf[14]=RSDT1.outERR;
            tbuf[15]=RSDT1.outAWR;
            tbuf[16]=RSDT1.outPP;
            tbuf[17]=RSDT1.outPG;//23
            if(RSDT1.vSTD<3)
            {
                tbuf[18]=RSDT1.outCO;
                tbuf[19]=RSDT1.outCW;                
            ...
            {
                tbuf[18]=RSDT1.outrCO;
                tbuf[19]=RSDT1.outrCW; 
            }
            if(RSDT1.vSTD>=3)
            {
                tbuf[20]=RSDT1.outZW; 
        ...
        case _CLIN_RRD_STD2:                 //odeslij do KSK rozszerzona paczke danych opisujaca stan sterownika
        {
            ...
            if(PFNf_ex_pgd)
            {
                tbuf[5]|=0b01000000;                             //ustaw znacznik aktywnosci zewnetrznego regulatora pogodowego
            }
            else
            {
                tbuf[5]&=0b10111111;                             //zeruj znacznik aktywnosci zewnetrznego regulatora pogodowego
            }            
            tbuf[6]=RSDT1.outCO;
            tbuf[7]=RSDT1.outCW;
            tbuf[8]=RSDT1.outCS;
            ...
            tbuf[20]=PFN.CONTSETP;                                                                  //docelowa wartosc modulacji dla temp zasilania
            tbuf[21]=RSDT1.outZW;                                                                   
            //******************************
            ifInitAutoMFrame(1);      
            //******************************
        ...
        case _CLIN_RRD_KNF:
        {
            ...
            tbuf[3]=RSDT1.outRGZ;                        
            tbuf[4]=RSDT1.outWANL;
            tbuf[5]=RSDT1.outKODKT;
            tbuf[6]=RSDT1.outWOBG;                        
            tbuf[7]=RSDT1.outOPDG;                        
            tbuf[8]=RSDT1.outTPCS;
            tbuf[9]=RSDT1.outRDPM;
            tbuf[10]=RSDT1.outDTPM;
            tbuf[11]=RSDT1.outMDPM;
            tbuf[12]=RSDT1.outMGPM;
            tbuf[13]=RSDT1.outMGPM2;  
            tbuf[14]=RSDT1.outHSCO;
            tbuf[15]=RSDT1.outCONW;
            tbuf[16]=RSDT1.outPROP;
            tbuf[17]=RSDT1.outPGMX;   
            tbuf[18]=RSDT1.outPGDR;
            tbuf[19]=RSDT1.outCWMG;   
            tbuf[20]=RSDT1.outCWTM;                 
            //******************************
            ifInitAutoMFrame(3);      
            //******************************       
            break;
            ...
            switch(krok)
            {
                ...
                case 3:         //3.Przygotowanie danych dla KSK
                {
                    ...
                    if(!FRL1f_NextFr)                                                    //w trakcie wysylki do UNI-02 nie bylo aktualizacji ramki LIN?
                    {
                        ...
                        tbuf[10]=RSDT1.outOPDG;                        
                        tbuf[11]=RSDT1.outTPCS;
                        tbuf[12]=RSDT1.outRDPM;
                        tbuf[13]=RSDT1.outDTPM;
                        tbuf[14]=RSDT1.outMDPM;
                        tbuf[15]=RSDT1.outMGPM;
                        tbuf[16]=RSDT1.outMGPM2;  
                        tbuf[17]=RSDT1.outHSCO;
                        tbuf[18]=RSDT1.outCONW;                        
                    }
                    else                                                                //aktualizowano ramke - odpowiedz negatywna
                    {
                        Nop();
    ...
    switch(FRL1.RS_KOM)
    {
        ...
        {        
            switch(FRL1.RS_IDN&0b01111111) //IDN paczki RS
            {
                ...
                case 0x04://Tryb pracy , wartosci graniczne
                {
                    FRL1.IDN=_CLIN_RRD_TRB;
                    return 1;                    
                }     
                case 0x05://Stan przycisku reset
                {
                    FRL1.IDN=_CLIN_RRD_RES;
                    return 1;                    
                }   
                case 0x06://Blokada awaryjna
                {
                    FRL1.IDN=_CLIN_RRD_BRES;
                    return 1;                    
                }     
                case 0x07://Funkcja serwisowa
                {
                    FRL1.IDN=_CLIN_RRD_FSR;
                    return 1;                    
                }  
                case 0x08://Funkcja antylegionella
                {
                    FRL1.IDN=_CLIN_RRD_ANL;
                    return 1;                    
                }  
                case 0x09://Pobudzenie RT
                {
                    FRL1.IDN=_CLIN_RRD_RT;
                    return 1;                    
                }
                case 0x0A://Pobudzenie CW / timera zasobnika
                {
                    FRL1.IDN=_CLIN_RRD_CW;
                    return 1;                    
                }   
                case 0x0B://Status interfejsu 10V
                {
                    FRL1.IDN=_CLIN_RRD_10V;
                    return 1;                    
                }  
                case 0x0C://Docelowa wartosc modulacji przy grzaniu obiegu WG (SETP)
                {
                    FRL1.IDN=_CLIN_RRD_SETP;
                    return 1;                    
                } 
                case 0x0D://Temp zewnetrzna, reg pogodowy
                {
                    FRL1.IDN=_CLIN_RRD_ZW;
                    return 1;                    
                }  
                case 0x0E://Status interfejsów
                {
                    FRL1.IDN=_CLIN_RRD_INT;
                    return 1;                    
                } 
                case 0x0F://Specjalny tryb pracy sterownika
                case 0x10:                    
                {
                    FRL1.IDN=_CLIN_RRD_PRD;
                    return 1;                    
                ...
                {
                    if(PFN.vUNI02<12) return 0;
                    FRL1.IDN=_CLIN_RRD_STH;
                    return 1;
                }
                case 0x12://odczyt liczba godzin pracy
                {
                    if(PFN.vUNI02<18) return 0;
    ...
    switch(FRL1.RS_KOM)
    {
        ...
        case 0x87:  //Pozostale parametry
        {
            switch(FRL1.RS_IDN&0b01111111) //IDN paczki RS
            {
                ...
                case 0x03://Zapisz tryb pracy
                {
                    FRL1.IDN=_CLIN_WRD_TRB;
                    return 1;                    
                }     
                case 0x04://Chwilowa aktywacja przycisku reset
                {
                    FRL1.IDN=_CLIN_WRD_RES;
                    return 1;                    
                }   
                case 0x05://Zniesienie blokady awaryjnej
                {
                    FRL1.IDN=_CLIN_WRD_BRES;
                    return 0;                       //brak mozliwosci zdalnego resetu 
                }     
                case 0x06://Sterowanie funkcj± serwisow±
                {
                    FRL1.IDN=_CLIN_WRD_FSR;
                    return 1;                    
                }  
                case 0x07://Sterowanie funkcj± antylegionella
                {
                    FRL1.IDN=_CLIN_WRD_ANL;
                    return 1;                    
                }   
                case 0x08://Zdalne pobudzenie RT
                {
                    FRL1.IDN=_CLIN_WRD_RT;
                    return 1;                    
                }
                case 0x09://Zdalne pobudzenie od timera zasobnika
                {
                    FRL1.IDN=_CLIN_WRD_CW;
                    return 1;                    
                }
                case 0x0A://Specjalny tryb pracy sterownika
                {
                    FRL1.IDN=_CLIN_WRD_PRD;
                    return 1;                    
                }  
                case 0x0B://zerowanie licznika flagi serwisowej historii zdarzen awaryjnych
                {
                    FRL1.IDN=_CLIN_WRD_HFLG;
                    return 1;                    
                }
                case 0x0C://zerowanie liczby godzin pracy
                {
                    if(PFN.vUNI02<18) return 0;
