...
#define	_NVPORT	23		/*liczba obslugiwanych portow/sygnalow*/
...
void RefStan(void)
{
	...
	//port 11
	SPort[11].Input=((_PRZ_CW)?0:1)||((M._LIN_CW)?1:0)||((M._IOT_CW)?1:0);			//port/sygnal poddany filtracji
	//port 12
	SPort[12].Input=((!((M._AKTYWNE_PGD2)?1:0))&&(((_RT)?1:0)||((M._LIN_RT)?1:0)||((M._I10V_RT)?1:0)||((M._IOT_RT)?1:0)))||(((M._AKTYWNE_PGD2)?1:0)&&((M._pob_PGDRT)?1:0));                     //port/sygnal poddany filtracji
	//port 13
