...
//UWAGA!!!
//Modulacja w dol nastepuje gdy temp CO-IN<DELTA 
//Modulacja w gore gdy zagrozone jest utrzymanie temperatury zadanej
unsigned char ModPmp1(unsigned char p)
{
	...
	if(rgl.fz||ppwm!=PMP.MGPROC)		//aktywny modulator pompy i faza 2 modulacji?
	{
		...
		if(DtIN.DIN2>DtIN.DDLT)     //DIN2=CO-IN, DDLT=DELTA
		{
			tim=50;
...
void RglPmp1(void)
{
	...
	{
		if(M._MODULUJ&&!DtIN._err_IN&&!M._RPM_CW&&RTS(_RTMPMP)>_TPMPBLK)			//obieg CO, po rozruchu po ustalonym czasie rozpocznij modulacje pompa
		{
	...
	if(DtKNF.rdpm)					//zdecyduj ze wzgledu na rodzaj pompy
	{
        ...
        {        
            if(DtKNF.robg)				//obieg zamkniety?
            {
                ...
                {
                    RglPmp2();				//moduluj algorytmem ECO dla atmo. zablokowane!
                }
            }
            else					   //obieg otwarty
            {
