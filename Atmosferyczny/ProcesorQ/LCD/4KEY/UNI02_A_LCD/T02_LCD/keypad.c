...
unsigned char DoublePresKey(unsigned char k,volatile StPin *ky)
{
	(*ky).c=PresKey(k,ky); 
...
unsigned char ExPresKey(unsigned char k,volatile StPin *ky,unsigned char dTim,unsigned char pTim)
{
	(*ky).bk=(k)?1:0;
...
void ExResKey(volatile StPin *ky)
{
	(*ky).bk=0;
	(*ky).sbk=0;
	(*ky).ap=0;
