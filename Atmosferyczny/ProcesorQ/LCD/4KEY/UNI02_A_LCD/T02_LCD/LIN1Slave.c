...
unsigned char LIN1SRecept(unsigned char ref)
{
    ...
    switch(krok)
    {
        ...
        case 1:
        {
            ...
            {
               UAD1tf_RFExt=0;
               LIN1f_ACK=0;                      //ignoruj paczke
               krok=3;
               return 4;    //blad naglowka
            }
            if((!RCSTA1bits.CREN||!PIE1bits.RC1IE))                      //deaktywowany odbior RS w trakcie oczekiwania lub podczas procesu odbioru?
            {
                if(!UAD1tf_RFEnd||UAD1tf_RFData) return 2;               //restart procedury
            }            
            ...
            {
                LIN1f_CHK=1;
                if(MGSDf_NO_ODB||!PFN.vUNI02||!PFN.ALIN)           //brak komunikacji z plytka UNI-02 lub wersja oprogramowania UNI-02<09 lub nieprawidlowy adres wlasny LIN?
                {
                    LIN1f_ACK=0;                      //ignoruj paczke
                    krok=3;
                ...
                //if((!bufRUART1[1]&&(bufRUART1[0]&0b00111111)!=_PIF_WRITE)) //adres zerowy i zadanie wysylki danych do MASTER?    
